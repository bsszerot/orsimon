...
&print_html_head("Привелегии - объектные") ;
...
print "<FORM ACTION=\"$base_url/cgi/get_privs_objects.cgi\" METHOD=\"GET\">
       <TABLE BORDER=\"0\" CELLSPACING=\"0\" CELLPADDING=\"0\" STYLE=\"width: 100%;\">" ;
print "<TR><TD>&nbsp;источник: </TD><TD><SELECT NAME=\"source\" STYLE=\"width: 100%;\">" ;
$is_selected = "" ; if ( $pv{source} eq "DBA_TAB_PRIVS" ) { $is_selected = " SELECTED" ; } print "<OPTION VALUE=\"DBA_TAB_PRIVS\" $is_selected>Пользователи и группы (роли)</OPTION>" ;
$is_selected = "" ; if ( $pv{source} eq "USER_TAB_PRIVS" ) { $is_selected = " SELECTED" ; } print "<OPTION VALUE=\"USER_TAB_PRIVS\" $is_selected>Привелегии текущего пользователя</OPTION>" ;
$is_selected = "" ; if ( $pv{source} eq "ROLE_TAB_PRIVS" ) { $is_selected = " SELECTED" ; } print "<OPTION VALUE=\"ROLE_TAB_PRIVS\" $is_selected>Привелегии групп (ролей) текущего пользователя</OPTION>" ;
print "</SELECT></TD></TR>" ;
print "<TR><TD>&nbsp;Владелец: </TD><TD><INPUT TYPE=\"INPUT\" NAME=\"owner\" VALUE=\"$pv{owner}\" STYLE=\"width: 100%\"></TD></TR>" ;
print "<TR><TD>&nbsp;Таблица: </TD><TD><INPUT TYPE=\"INPUT\" NAME=\"table_name\" VALUE=\"$pv{table_name}\" STYLE=\"width: 100%\"></TD></TR>" ;
print "<TR><TD>&nbsp;Привелегия: </TD><TD><INPUT TYPE=\"INPUT\" NAME=\"privilege\" VALUE=\"$pv{privilege}\" STYLE=\"width: 100%\"></TD></TR>" ;
...
$where_ext = "" ; 
if ( $pv{owner} ne "" ) { if ( $where_ext eq "" ) { $where_ext .= " WHERE " ; } else { $where_ext .= " AND " ; } $where_ext .= " OWNER = '$pv{owner}' " ; }
if ( $pv{table_name} ne "" ) { if ( $where_ext eq "" ) { $where_ext .= " WHERE " ; } else { $where_ext .= " AND " ; } $where_ext .= " TABLE_NAME = '$pv{table_name}' " ; }
if ( $pv{privilege} ne "" ) { if ( $where_ext eq "" ) { $where_ext .= " WHERE " ; } else { $where_ext .= " AND " ; } $where_ext .= " PRIVILEGE = '$pv{privilege}' " ; }
# актуально только для DBA_TAB_PRIVS
if ( $pv{grantee} ne "" ) { if ( $where_ext eq "" ) { $where_ext .= " WHERE " ; } else { $where_ext .= " AND " ; } $where_ext .= " GRANTEE = '$pv{grantee}' " ; }

# обработка для источника - DBA_ или USER__
if ( $pv{source} eq "DBA_TAB_PRIVS" || $pv{source} eq "USER_TAB_PRIVS" ) { 
   $request = "SELECT GRANTEE, OWNER, TABLE_NAME, GRANTOR, PRIVILEGE, GRANTABLE, HIERARCHY FROM $pv{source} $where_ext order by $pv{order_field}" ;
...
# - вывести непосредственно контент
   ...
   print "<TR><TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_privs_objects.cgi?order_field=GRANTEE&source=$pv{source}\&privilege=$pv{privilege}\&grantee=$pv{grantee}\&owner=$pv{owner}\&table_name=$pv{table_name}\" TARGET=\"cont\">Субъект</A></TD>
              <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_privs_objects.cgi?order_field=OWNER&source=$pv{source}\&privilege=$pv{privilege}\&grantee=$pv{grantee}\&owner=$pv{owner}\&table_name=$pv{table_name}\" TARGET=\"cont\">Владелец</A></TD>
              <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_privs_objects.cgi?order_field=TABLE_NAME&source=$pv{source}\&privilege=$pv{privilege}\&grantee=$pv{grantee}\&owner=$pv{owner}\&table_name=$pv{table_name}\" TARGET=\"cont\">Таблица</A></TD>
              <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_privs_objects.cgi?order_field=GRANTOR&source=$pv{source}\&privilege=$pv{privilege}\&grantee=$pv{grantee}\&owner=$pv{owner}\&table_name=$pv{table_name}\" TARGET=\"cont\">GRANTOR</A></TD>
              <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_privs_objects.cgi?order_field=PRIVILEGE&source=$pv{source}\&privilege=$pv{privilege}\&grantee=$pv{grantee}\&owner=$pv{owner}\&table_name=$pv{table_name}\" TARGET=\"cont\">Привелегия</A></TD>
              <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_privs_objects.cgi?order_field=GRANTABLE&source=$pv{source}\&privilege=$pv{privilege}\&grantee=$pv{grantee}\&owner=$pv{owner}\&table_name=$pv{table_name}\" TARGET=\"cont\">GRANTABLE</A></TD>
              <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_privs_objects.cgi?order_field=HIERARCHY&source=$pv{source}\&privilege=$pv{privilege}\&grantee=$pv{grantee}\&owner=$pv{owner}\&table_name=$pv{table_name}\" TARGET=\"cont\">Иерархия</A></TD>
              </TR>" ;
   $cnt_rows = 0 ;
   while (my ($grantee,$owner,$table_name,$grantor,$privilege,$grantable,$hierarchy) = $sth->fetchrow_array() ) { $cnt_rows++ ;
   ...
   printf ("<TR><TD CLASS=\"SZDATA\">Итого</TD><TD CLASS=\"NUMDATA\" COLSPAN=\"6\">%s</TD></TR>\n", $cnt_rows) ;
   print "</TABLE>" ;
   }

# обработка для источника - ROLE_
if ( $pv{source} eq "ROLE_TAB_PRIVS" ) { 
   if ( $pv{order_field} eq "GRANTEE" ) { $pv{order_field} = "ROLE" ; }
   $request = "SELECT ROLE, OWNER, TABLE_NAME, COLUMN_NAME, PRIVILEGE, GRANTABLE FROM $pv{source} $where_ext order by $pv{order_field}" ;
   my $sth = $dbh->prepare($request) ; $sth->execute() ;
# - вывести непосредственно контент
   print "<TABLE BORDER=\"1\" CELLSPACING=\"0\" CELLPADDING=\"0\" STYLE=\"width: 100%;\">" ;
   print "<TR><TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_privs_objects.cgi?order_field=ROLE&source=$pv{source}\&privilege=$pv{privilege}\&grantee=$pv{grantee}\&owner=$pv{owner}\&table_name=$pv{table_name}\" TARGET=\"cont\">Роль</A></TD>
              <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_privs_objects.cgi?order_field=OWNER&source=$pv{source}\&privilege=$pv{privilege}\&grantee=$pv{grantee}\&owner=$pv{owner}\&table_name=$pv{table_name}\" TARGET=\"cont\">Владелец</A></TD>
              <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_privs_objects.cgi?order_field=TABLE_NAME&source=$pv{source}\&privilege=$pv{privilege}\&grantee=$pv{grantee}\&owner=$pv{owner}\&table_name=$pv{table_name}\" TARGET=\"cont\">Таблица</A></TD>
              <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_privs_objects.cgi?order_field=COLUMN_NAME&source=$pv{source}\&privilege=$pv{privilege}\&grantee=$pv{grantee}\&owner=$pv{owner}\&table_name=$pv{table_name}\" TARGET=\"cont\">Колонка</A></TD>
              <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_privs_objects.cgi?order_field=PRIVILEGE&source=$pv{source}\&privilege=$pv{privilege}\&grantee=$pv{grantee}\&owner=$pv{owner}\&table_name=$pv{table_name}\" TARGET=\"cont\">Привелегия</A></TD>
              <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_privs_objects.cgi?order_field=ADMIN_OPTION&source=$pv{source}\&privilege=$pv{privilege}\&grantee=$pv{grantee}\&owner=$pv{owner}\&table_name=$pv{table_name}\" TARGET=\"cont\">GRANTABLE</A></TD>
              </TR>" ;
   $cnt_rows = 0 ;
   while (my ($role,$owner,$table_name,$column_name,$privilege,$grantable) = $sth->fetchrow_array() ) { $cnt_rows++ ;
         printf("<TR><TD CLASS=\"SZDATA\">%s</TD>
                     <TD CLASS=\"SZDATA\">%s</TD>
                     <TD CLASS=\"SZDATA\">%s</TD>
                     <TD CLASS=\"SZDATA\">%s</TD>
