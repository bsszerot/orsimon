...
$request = 'select PROFILE, RESOURCE_NAME, RESOURCE_TYPE, LIMIT from dba_profiles ' . "order by $order_field asc" ;
                                 
print "Content-Type: text/html\n\n"; 
&print_html_head("Информация о профилях системы") ;

my $sth = $dbh->prepare($request) ;
$sth->execute();
print "<TABLE BORDER=\"1\" CELLSPACING=\"0\" CELLPADDING=\"0\" STYLE=\"width: 100%;\">" ;
print "<TR><TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_profiles_list.cgi?order_field=PROFILE\" TARGET=\"cont\">Имя профиля</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_profiles_list.cgi?order_field=RESOURCE_NAME\" TARGET=\"cont\">Ресурс</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_profiles_list.cgi?order_field=RESOURCE_TYPE\" TARGET=\"cont\">Тип ресурса</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_profiles_list.cgi?order_field=LIMIT\" TARGET=\"cont\">Лимит</A></TD>
           </TR>" ;

$profiles_count_all = 0 ;
while (my ($profile,$resource_name,$resource_type,$limit) = $sth->fetchrow_array() ) {
      $profiles_count_all += 1 ;

      if ( $resource_type eq "" ) { $resource_type = "&nbsp;" ; }
      if ( $limit eq "" ) { $limit = "&nbsp;" ; }
      printf ("<TR><TD CLASS=\"SZDATA\"><A HREF=\"$base_url/cgi/get_profile_info.cgi?profile=%s&order_field=RESOURCE_TYPE\" TARGET=\"cont\">%s</A></TD>",$profile,$profile) ;
      printf ("<TD CLASS=\"SZDATA\">%s</TD>
               <TD CLASS=\"SZDATA\">%s</TD>
               <TD CLASS=\"SZDATA\">%s</TD>",$resource_name,$resource_type,$limit) ;
      }
# форматирование вложенной таблицей оставлено для единобразия
print "<TR><TD COLSPAN=\"4\">
           <TABLE BORDER=\"0\"><TR><TD>Записей лимитов профилей всего:</TD><TD>$profiles_count_all</TD></TR>" ;
print "</TABLE></TD></TR>" ;
