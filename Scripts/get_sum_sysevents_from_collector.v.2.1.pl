...
sub print_system_events_navigation($) { $active_tab{1} = "" ; $active_tab{2} = "" ; $active_tab{3} = "" ; 
    ...
    print "<TABLE CELLSPACING=\"0\" CELLPADDING=\"0\" STYLE=\"border: 0pt none; width: 100%;\">
           ...
           <TD STYLE=\"border: 2pt navy; border-style: solid $active_tab{3}; text-align: center;\">
               <A HREF=\"$base_url/cgi/get_system_events.cgi?order_field=NAME\" TARGET=\"cont\">Аналитика<BR>топовых<BR>событий</A></TD>
           <TD STYLE=\"border: 2pt navy; border-style: none none solid none;\">&nbsp;&nbsp;&nbsp;&nbsp;</TD>
           <TD STYLE=\"border: 2pt navy; border-style: solid $active_tab{4}; text-align: center;\">
               <A HREF=\"$base_url/cgi/get_sum_sysevents_from_collector.cgi?order_field=CR_SNAP_ID\" TARGET=\"cont\">Суммарная<BR>аналитика<BR>событий</A></TD>
           <TD STYLE=\"border: 2pt navy; border-style: none none solid none;\">&nbsp;&nbsp;&nbsp;&nbsp;</TD>
           </TR></TABLE>" ;
    }
...
print "<FORM ACTION=\"$base_url/cgi/get_sum_sysevents_from_collector.cgi\" METHOD=\"GET\">
...
if ( $pv{is_second} eq "yes" ) {
   ...
   print "<TR><TD ROWSPAN=\"2\" CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_sum_sysevents_from_collector.cgi?order_field=POINT_ID&from_range=$pv{from_range}&to_range=$pv{to_range}&report_type=$pv{report_type}&is_second=$pv{is_second}\" TARGET=\"cont\">ID объекта контроля (POINT ID)</A></TD>
              <TD ROWSPAN=\"2\" CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_sum_sysevents_from_collector.cgi?order_field=CR_SNAP_ID&from_range=$pv{from_range}&to_range=$pv{to_range}&report_type=$pv{report_type}&is_second=$pv{is_second}\" TARGET=\"cont\">ID текущего среза</A></TD>
              <TD ROWSPAN=\"2\" CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_sum_sysevents_from_collector.cgi?order_field=CR_SNAP_TIME&from_range=$pv{from_range}&to_range=$pv{to_range}&report_type=$pv{report_type}&is_second=$pv{is_second}\" TARGET=\"cont\">Дата текущего среза</A></TD>
              <TD ROWSPAN=\"2\" CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_sum_sysevents_from_collector.cgi?order_field=PR_SNAP_ID&from_range=$pv{from_range}&to_range=$pv{to_range}&report_type=$pv{report_type}&is_second=$pv{is_second}\" TARGET=\"cont\">ID предыдущего среза</A></TD>
              <TD ROWSPAN=\"2\" CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_sum_sysevents_from_collector.cgi?order_field=PR_SNAP_TIME&from_range=$pv{from_range}&to_range=$pv{to_range}&report_type=$pv{report_type}&is_second=$pv{is_second}\" TARGET=\"cont\">Дата предыдущего среза</A></TD>
              <TD ROWSPAN=\"2\" CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_sum_sysevents_from_collector.cgi?order_field=TOTAL_TIME_MICRO&from_range=$pv{from_range}&to_range=$pv{to_range}&report_type=$pv{report_type}&is_second=$pv{is_second}\" TARGET=\"cont\">Длительность среза, микросекунд</A></TD>
              <TD COLSPAN=\"2\" CLASS=\"HEAD\">Длительность нефиктивных ожиданий</TD>
              <TD COLSPAN=\"2\" CLASS=\"HEAD\">Длительность очередей (блокировок)</TD>
              </TR><TR>
              <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_sum_sysevents_from_collector.cgi?order_field=TOTAL_NOIDLE_WAIT&from_range=$pv{from_range}&to_range=$pv{to_range}&report_type=$pv{report_type}&is_second=$pv{is_second}\" TARGET=\"cont\">микросекунд</A></TD>
              <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_sum_sysevents_from_collector.cgi?order_field=TOTAL_NOIDLE_PRCNT&from_range=$pv{from_range}&to_range=$pv{to_range}&report_type=$pv{report_type}&is_second=$pv{is_second}\" TARGET=\"cont\">%</A></TD>
              <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_sum_sysevents_from_collector.cgi?order_field=QUEUE_WAIT_VAL&from_range=$pv{from_range}&to_range=$pv{to_range}&report_type=$pv{report_type}&is_second=$pv{is_second}\" TARGET=\"cont\">микросекунд</A></TD>
              <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_sum_sysevents_from_collector.cgi?order_field=QUEUE_WAIT_PRCNT&from_range=$pv{from_range}&to_range=$pv{to_range}&report_type=$pv{report_type}&is_second=$pv{is_second}\" TARGET=\"cont\">%</A></TD>
              </TR>" ;
