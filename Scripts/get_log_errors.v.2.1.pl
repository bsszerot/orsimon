...
print "<FORM ACTION=\"$base_url/cgi/get_log_errors.cgi\" METHOD=\"GET\">
...
# сформировать уточняющие фильтры для запроса статистики
   ...
   $request = 'select TO_CHAR(TMST_RECORD,\'YYYY-MM-DD HH24:MI:SS\'), USERNAME, INSTANCE, DBNAME, ERROR_STACK FROM BESTAT_ERROR_LOGS ' . $where_ext . " order by $pv{order_field} asc" ;
   ...
   print "<TR><TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_log_errors.cgi?order_field=TMST_RECORD&isfirst=$pv{isfirst}&srcptr=$pv{srcptr}&tbs_filter=$pv{tbs_filter}&name_filter=$pv{name_filter}&owner_filter=$pv{owner_filter}&type_filter=$pv{type_filter}\" TARGET=\"cont\">Дата ошибки</A></TD>
              <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_log_errors.cgi?order_field=USERNAME&isfirst=$pv{isfirst}&srcptr=$pv{srcptr}&tbs_filter=$pv{tbs_filter}&name_filter=$pv{name_filter}&owner_filter=$pv{owner_filter}&type_filter=$pv{type_filter}\" TARGET=\"cont\">Пользователь</A></TD>
              <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_log_errors.cgi?order_field=INSTANCE&isfirst=$pv{isfirst}&srcptr=$pv{srcptr}&tbs_filter=$pv{tbs_filter}&name_filter=$pv{name_filter}&owner_filter=$pv{owner_filter}&type_filter=$pv{type_filter}\" TARGET=\"cont\">Инстанс</A></TD>
              <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_log_errors.cgi?order_field=DBNAME&isfirst=$pv{isfirst}&srcptr=$pv{srcptr}&tbs_filter=$pv{tbs_filter}&name_filter=$pv{name_filter}&owner_filter=$pv{owner_filter}&type_filter=$pv{type_filter}\" TARGET=\"cont\">База</A></TD>
              <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_log_errors.cgi?order_field=ERROR_STACK&isfirst=$pv{isfirst}&srcptr=$pv{srcptr}&tbs_filter=$pv{tbs_filter}&name_filter=$pv{name_filter}&owner_filter=$pv{owner_filter}&type_filter=$pv{type_filter}\" TARGET=\"cont\">Стэк ошибки</A></TD>
              </TR>" ;

   $count_all = 0 ;
   while (my ( $tmst_record,$username,$instance,$dbname,$error_stack ) = $sth->fetchrow_array() ) {
         if ( $count_by_users{$username} eq "" || $count_by_users{$username} < 1 ) { $count_by_users{$username} = 0 ; } 
         $count_by_users{$username} += 1 ; $count_all += 1 ;
         if ( $tmst_record eq "" ) { $tmst_record = "&nbsp;" ; } $tmst_record =~ s/[\s\t]+/&nbsp;/g ;
         if ( $username eq "" ) { $username = "&nbsp;" ; }
         $error_stack =~ s/[\r\n]+/<BR>/g ; $error_stack =~ s/[\s\t]+/&nbsp;/g ;
# $error_stack =~ s/^[\r\n\s\t]+$//mg ;
#print "===$error_stack===" ;
         printf("<TR><TD CLASS=\"SZDATA\">%s</TD>
                     ...
                     <TD CLASS=\"SZDATA\">%s</TD>
                    </TR>\n",$tmst_record,$username,$instance,$error_stack ) ;
         }
