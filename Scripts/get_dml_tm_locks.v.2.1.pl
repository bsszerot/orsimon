...
$request = 'SELECT /*+ ORDERED */ l.sid SID, s.serial#, o.OWNER, o.OBJECT_NAME, o.OBJECT_TYPE, decode(l.LMODE,0,\'none\',1,\'null (NULL)\',2,\'row-S (SS)\',3,\'row-X (SX)\',4,\'share (X)\',5,\'S/Row-X (SSX)\',6,\'executive (X)\',l.LMODE), 
            ...
            FROM V$LOCK l, V$SESSION s, DBA_OBJECTS o WHERE l.SID = s.SID (+) AND l.ID1 = o.OBJECT_ID (+) AND l.TYPE = \'TM\' ORDER BY ' . $order_field ;

my $sth = $dbh->prepare($request) ;
$sth->execute();
print "Content-Type: text/html\n\n"; 
&print_html_head("Текущие TM DML блокировки") ;
# добавить определение навигационно - оформительского механизма закладок
&print_locks_navigation(2) ;
...
print "<TR><TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_dml_tm_locks.cgi?order_field=l.SID\" TARGET=\"cont\">Сессия</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_dml_tm_locks.cgi?order_field=o.OWNER\" TARGET=\"cont\">Владелец объекта</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_dml_tm_locks.cgi?order_field=o.OBJECT_NAME\" TARGET=\"cont\">Имя объекта</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_dml_tm_locks.cgi?order_field=o.OBJECT_TYPE\" TARGET=\"cont\">Тип объекта</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_dml_tm_locks.cgi?order_field=l.LMODE\" TARGET=\"cont\">Режим блокировки</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_dml_tm_locks.cgi?order_field=l.REQUEST\" TARGET=\"cont\">Режим ожидания</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_dml_tm_locks.cgi?order_field=l.TYPE\" TARGET=\"cont\">Тип блокировки</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_dml_tm_locks.cgi?order_field=l.BLOCK\" TARGET=\"cont\">Блокирует ли других</A></TD>
           </TR>" ;
$is_not_null_answer = "no" ; $sum_records = 0 ;
while (my ($sid,$serial,$owner,$object_name,$object_type,$lmode,$request,$type,$block) = $sth->fetchrow_array() ) { $sum_records += 1 ;
      ...
      printf("<TR><TD CLASS=\"SZDATA\"><A HREF=\"$base_url/cgi/get_session_info.cgi?sid=%s&serial=%s\" TARGET=\"cont\">%s,%s</A></TD>
                  ...
                  <TD CLASS=\"CENTERDATA\">%s</TD>
                  <TD CLASS=\"CENTERDATA\">%s</TD>
              </TR>\n",$sid,$serial,$sid,$serial,$owner,$object_name,$object_type,$lmode,$request,$type,$block) ;
      $is_not_null_answer = "yes" ;
      }
if ( $is_not_null_answer eq "no" ) { print "<TR><TD CLASS=\"SZDATA\" COLSPAN=\"8\">Блокировки TM DML</TD></TR>" ;}
