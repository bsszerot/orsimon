...
&print_pga_navigation(10) ;

my $dbh = DBI->connect( $connector_definition{$current_connector}, $connector_credentials{$current_connector}, '');
$request = "SELECT NAME, VALUE FROM V\$SYSSTAT WHERE NAME IN ('sorts (memory)', 'sorts (disk)') ORDER BY $pv{order_field}" ;
...
print "<TR><TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_pga_sorts_by_type.cgi?order_field=NAME\" TARGET=\"cont\">Вид сортировки</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_pga_sorts_by_type.cgi?order_field=VALUE\" TARGET=\"cont\">Значение</A></TD>
           </TR>" ;
...
while (my ($name,$value) = $sth->fetchrow_array() ) {
      ...
      printf("<TR><TD CLASS=\"SZDATA\">%s</TD>
              ...
              </TR>\n",$name,&show_razryads($value)) ;
      }
printf("<TR><TD CLASS=\"SZDATA\">Итого строк: </TD><TD CLASS=\"NUMDATA\">%s</TD></TR>\n",$count_rows) ;
print "</TABLE>" ;
print "</TD></TR></TABLE>" ;
$sth->finish();
$dbh->disconnect();                                        
