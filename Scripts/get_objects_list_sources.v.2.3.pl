...
$pv{order_field} = "NAME" ; $report_type = "" ;
&get_forms_param() ;

$ENV{ORACLE_SID} = $current_connector ;
                                 
print "Content-Type: text/html\n\n"; 
&print_html_head("Информация источников исходного кода") ;

# добавить определение навигационно - оформительского механизма закладок
&print_objects_navigation(7) ;
...
print "<FORM ACTION=\"$base_url/cgi/get_objects_list_sources.cgi\" METHOD=\"GET\">
...
# --- вычислить диапазон выводимых значений при необходимости
   ...
   if ( $pv{object_type} ne "" ) { if ( $where_class ne "" ) { $where_class .= " AND " } else { $where_class .= " WHERE " } $where_class .= ' TYPE = ' . "'$pv{object_type}' " ; }
   if ( $pv{name_include} ne "" ) { if ( $where_class ne "" ) { $where_class .= " AND " } else { $where_class .= " WHERE " } $where_class .= " NAME LIKE '%$pv{name_include}%' " ; } 
   $request = 'select OWNER, NAME, TYPE from all_source ' . $where_class . ' GROUP BY OWNER,NAME,TYPE ORDER BY ' . $pv{order_field} ;
...
# вывести непосредственно контент
   ...
   print "<TR><TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_objects_list_sources.cgi?order_field=OWNER\&report_type=$pv{report_type}\&owner=$pv{owner}\&object_type=$pv{object_type}\&name_include=$pv{name_include}\&isfirst=$pv{isfirst}\" TARGET=\"cont\">Владелец</A></TD>
              <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_objects_list_sources.cgi?order_field=NAME\&report_type=$pv{report_type}\&owner=$pv{owner}\&object_type=$pv{object_type}\&name_include=$pv{name_include}\&isfirst=$pv{isfirst}\" TARGET=\"cont\">Объект</A></TD>
              <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_objects_list_sources.cgi?order_field=TYPE\&report_type=$pv{report_type}\&owner=$pv{owner}\&object_type=$pv{object_type}\&name_include=$pv{name_include}\&isfirst=$pv{isfirst}\" TARGET=\"cont\">Тип</A></TD>
              </TR>" ;
   $count_rows = 0 ;
   while (my ($owner,$object_name,$object_type) = $sth->fetchrow_array() ) {
         $count_rows += 1 ;
         $created =~ s/\s/&nbsp;/ ;
         $last_ddl_time =~ s/\s/&nbsp;/ ;
         $object_type =~ s/\s/&nbsp;/ ;
         $object_type_web = $object_type ; if ( $object_type_web =~ /PACKAGE.+BODY/ ) { $object_type_web = 'PACKAGE%20BODY' ; }
         printf("<TR><TD CLASS=\"SZDATA\">%s</TD>
                     <TD CLASS=\"SZDATA\"><A HREF=\"get_object_source.cgi?owner=%s\&name=%s\&type=%s\" TARGET=\"cont\">%s</A></TD>
                     <TD CLASS=\"SZDATA\">%s</TD>
                 </TR>\n",$owner,$owner,$object_name,$object_type_web,$object_name,$object_type) ;
         }
   print "<TR><TD CLASS=\"SZDATA\">Итого</TD><TD CLASS=\"NUMDATA\" COLSPAN=\"2\">$count_rows</TD></TR>\n" ;
   print "</TABLE>" ;
   $sth->finish();
   $dbh->disconnect();
   print "</TD></TR></TABLE>" ;
