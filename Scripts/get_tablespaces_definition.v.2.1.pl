...
&print_html_head("Табличные пространства") ;
...
&print_tablespaces_navigation(1) ;
...
print "<FORM ACTION=\"$base_url/cgi/get_tablespaces_definition.cgi\" METHOD=\"GET\">
...
if ( $pv{srcptr} eq "0" || $pv{srcptr} eq "" ) { $request = "select TABLESPACE_NAME, BLOCK_SIZE, STATUS, CONTENTS, LOGGING, FORCE_LOGGING, EXTENT_MANAGEMENT, ALLOCATION_TYPE, PLUGGED_IN, SEGMENT_SPACE_MANAGEMENT from dba_tablespaces " . $where_ext . " order by $pv{order_field}" ; }
else { if ( $where_ext ne "" ) { $where_ext =~ s/WHERE/AND/g ; } $request = "select TABLESPACE_NAME, BLOCK_SIZE, STATUS, CONTENTS, LOGGING, FORCE_LOGGING, EXTENT_MANAGEMENT, ALLOCATION_TYPE, PLUGGED_IN, SEGMENT_SPACE_MANAGEMENT from BESTAT_DBA_TABLESPACES WHERE POINT = $pv{srcptr} " . $where_ext . "  order by $pv{order_field}" ; }
...
print "<TR><TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_tablespaces_definition.cgi?order_field=TABLESPACE_NAME&srcptr=$pv{srcptr}&tbs_filter=$pv{tbs_filter}&status_filter=$pv{status_filter}&type_filter=$pv{type_filter}\" TARGET=\"cont\">Имя ТС</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_tablespaces_definition.cgi?order_field=BLOCK_SIZE&srcptr=$pv{srcptr}&tbs_filter=$pv{tbs_filter}&status_filter=$pv{status_filter}&type_filter=$pv{type_filter}\" TARGET=\"cont\">Размер блока</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_tablespaces_definition.cgi?order_field=STATUS&srcptr=$pv{srcptr}&tbs_filter=$pv{tbs_filter}&status_filter=$pv{status_filter}&type_filter=$pv{type_filter}\" TARGET=\"cont\">Статус</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_tablespaces_definition.cgi?order_field=CONTENTS&srcptr=$pv{srcptr}&tbs_filter=$pv{tbs_filter}&status_filter=$pv{status_filter}&type_filter=$pv{type_filter}\" TARGET=\"cont\">Контент</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_tablespaces_definition.cgi?order_field=LOGGING&srcptr=$pv{srcptr}&tbs_filter=$pv{tbs_filter}&status_filter=$pv{status_filter}&type_filter=$pv{type_filter}\" TARGET=\"cont\">LOGGING</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_tablespaces_definition.cgi?order_field=FORCE_LOGGING&srcptr=$pv{srcptr}&tbs_filter=$pv{tbs_filter}&status_filter=$pv{status_filter}&type_filter=$pv{type_filter}\" TARGET=\"cont\">Force LOGGING</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_tablespaces_definition.cgi?order_field=EXTENT_MANAGEMENT&srcptr=$pv{srcptr}&tbs_filter=$pv{tbs_filter}&status_filter=$pv{status_filter}&type_filter=$pv{type_filter}\" TARGET=\"cont\">Управление экстентами</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_tablespaces_definition.cgi?order_field=ALLOCATION_TYPE&srcptr=$pv{srcptr}&tbs_filter=$pv{tbs_filter}&status_filter=$pv{status_filter}&type_filter=$pv{type_filter}\" TARGET=\"cont\">ALLOCATION_TYPE</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_tablespaces_definition.cgi?order_field=PLUGGED_IN&srcptr=$pv{srcptr}&tbs_filter=$pv{tbs_filter}&status_filter=$pv{status_filter}&type_filter=$pv{type_filter}\" TARGET=\"cont\">PLUGGED_IN</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_tablespaces_definition.cgi?order_field=SEGMENT_SPACE_MANAGEMENT&srcptr=$pv{srcptr}&tbs_filter=$pv{tbs_filter}&status_filter=$pv{status_filter}&type_filter=$pv{type_filter}\" TARGET=\"cont\">Управление сегментами</A></TD>
           </TR>" ;
$count_rows = 0 ;
while (my ($ts_name,$block_size,$status,$contentc,$logging,$force_logging,$extent_management,$allocation_type,$plugged_in,$segment_space_management) = $sth->fetchrow_array() ) {
      $count_rows += 1 ;
      printf("<TR><TD CLASS=\"SZDATA\"><A HREF=\"$base_url/cgi/get_files_definition.cgi?order_field=FILE_NAME&tbs_filter=%s&srcptr=$pv{srcptr}\" TARGET=\"cont\">%s</A></TD>
                  ...
                  <TD CLASS=\"SZDATA\">%0s</TD>
                  <TD CLASS=\"SZDATA\">%0s</TD>
              </TR>\n",$ts_name,$ts_name,$block_size,$status,$contentc,$logging,$force_logging,$extent_management,$allocation_type,$plugged_in,$segment_space_management) ;
      }
