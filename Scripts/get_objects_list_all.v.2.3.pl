...
sub print_objects_navigation($) { $active_tab{1} = "" ; $active_tab{2} = "" ; $active_tab{3} = "" ; 
    ...
    print "<TABLE CELLSPACING=\"0\" CELLPADDING=\"0\" STYLE=\"border: 0pt none; width: 100%;\">
           ...
           <TD STYLE=\"border: 2pt navy; border-style: solid $active_tab{1}; text-align: center;\">
               <A HREF=\"$base_url/cgi/get_objects_list_all.cgi?order_field=OBJECT_NAME\" TARGET=\"cont\">Объекты<BR>все</A></TD>
           ...
           <TD STYLE=\"border: 2pt navy; border-style: solid $active_tab{3}; text-align: center;\">
               <A HREF=\"$base_url/cgi/get_data_from_table_view.cgi?order_field=VIEW_NAME&owner=SYS&name=DBA_VIEWS&is_second=yes\" TARGET=\"cont\">Представления</A></TD>
           <TD STYLE=\"border: 2pt navy; border-style: none none solid none;\">&nbsp;&nbsp;&nbsp;&nbsp;</TD>
           <TD STYLE=\"border: 2pt navy; border-style: solid $active_tab{3}; text-align: center;\">
               <A HREF=\"$base_url/cgi/get_data_from_table_view.cgi?order_field=INDEX_NAME&owner=SYS&name=DBA_INDEXES&is_second=yes\" TARGET=\"cont\">Индексы</A></TD>
           <TD STYLE=\"border: 2pt navy; border-style: none none solid none;\">&nbsp;&nbsp;&nbsp;&nbsp;</TD>
           <TD STYLE=\"border: 2pt navy; border-style: solid $active_tab{3}; text-align: center;\">
               <A HREF=\"$base_url/cgi/get_data_from_table_view.cgi?order_field=SYNONYM_NAME&owner=SYS&name=DBA_SYNONYMS&is_second=yes\" TARGET=\"cont\">Синонимы</A></TD>
           <TD STYLE=\"border: 2pt navy; border-style: none none solid none;\">&nbsp;&nbsp;&nbsp;&nbsp;</TD>
           <TD STYLE=\"border: 2pt navy; border-style: solid $active_tab{3}; text-align: center;\">
               <A HREF=\"$base_url/cgi/get_data_from_table_view.cgi?order_field=TRIGGER_NAME&owner=SYS&name=DBA_TRIGGERS&is_second=yes\" TARGET=\"cont\">Триггеры</A></TD>
           <TD STYLE=\"border: 2pt navy; border-style: none none solid none;\">&nbsp;&nbsp;&nbsp;&nbsp;</TD>
           <TD STYLE=\"border: 2pt navy; border-style: solid $active_tab{3}; text-align: center;\">
               <A HREF=\"$base_url/cgi/get_objects_list_sources.cgi?order_field=NAME&is_second=no\" TARGET=\"cont\">Sources</A></TD>
           <TD STYLE=\"border: 2pt navy; border-style: none none solid none;\">&nbsp;&nbsp;&nbsp;&nbsp;</TD>
           <TD STYLE=\"border: 2pt navy; border-style: solid $active_tab{3}; text-align: center;\">
               <A HREF=\"$base_url/cgi/get_data_from_table_view.cgi?order_field=JOB&owner=SYS&name=DBA_JOBS&is_second=yes\" TARGET=\"cont\">Jobs</A></TD>
           <TD STYLE=\"border: 2pt navy; border-style: none none solid none;\">&nbsp;&nbsp;&nbsp;&nbsp;</TD>
           <TD STYLE=\"border: 2pt navy; border-style: solid $active_tab{3}; text-align: center;\">
               <A HREF=\"$base_url/cgi/get_data_from_table_view.cgi?order_field=DB_LINK&owner=SYS&name=DBA_DB_LINKS&is_second=yes\" TARGET=\"cont\">DB<BR>линки</A></TD>
           <TD STYLE=\"border: 2pt navy; border-style: none none solid none;\">&nbsp;&nbsp;&nbsp;&nbsp;</TD>
           </TR></TABLE>" ;
    }
...
print "<FORM ACTION=\"$base_url/cgi/get_objects_list_all.cgi\" METHOD=\"GET\">
       ...
       <TR><TD>Тип объектов: </TD><TD STYLE=\"width: 85%\"><INPUT TYPE=\"input\" NAME=\"object_type\" VALUE=\"$pv{object_type}\" STYLE=\"width: 100%\"></TD></TR></TD></TR>
       <TR><TD>Дата: </TD>
           <TD><INPUT TYPE=\"radio\" NAME=\"period\" VALUE=\"default\" $is_checked_period_default>не важно&nbsp;
               <INPUT TYPE=\"radio\" NAME=\"period\" VALUE=\"created\" $is_checked_period_created>создание&nbsp;
               <INPUT TYPE=\"radio\" NAME=\"period\" VALUE=\"modified\" $is_checked_period_modified>модификация с 
...
# --- вычислить диапазон выводимых значений при необходимости
   $where_class = "" ; 
   if ( $pv{period} eq "created" ) { $where_class = ' WHERE CREATED >= ' . "TO_DATE('$pv{period_from} 00:00:00','YYYY-MM-DD HH24:MI:SS') AND " . 'CREATED < ' . "TO_DATE('$pv{period_to} 23:59:59','YYYY-MM-DD HH24:MI:SS') " ; }
   if ( $pv{period} eq "modified" ) { $where_class = ' WHERE LAST_DDL_TIME >= ' . "TO_DATE('$pv{period_from} 00:00:00','YYYY-MM-DD HH24:MI:SS') AND " . 'LAST_DDL_TIME < ' . "TO_DATE('$pv{period_to} 23:59:59','YYYY-MM-DD HH24:MI:SS') " ; }
   ...
   if ( $pv{object_type} ne "" ) { if ( $where_class ne "" ) { $where_class .= " AND " } else { $where_class .= " WHERE " } $where_class .= ' OBJECT_TYPE = ' . "'$pv{object_type}' " ; }
   if ( $pv{name_include} ne "" ) { if ( $where_class ne "" ) { $where_class .= " AND " } else { $where_class .= " WHERE " } $where_class .= " OBJECT_NAME LIKE '%$pv{name_include}%' " ; } 
   $request = 'select OWNER, OBJECT_NAME, SUBOBJECT_NAME, OBJECT_ID, DATA_OBJECT_ID, OBJECT_TYPE, TO_CHAR(CREATED,\'YYYY-MM-DD HH:MI:SS\'), TO_CHAR(LAST_DDL_TIME,\'YYYY-MM-DD HH:MI:SS\'), STATUS, TEMPORARY, GENERATED, SECONDARY from dba_objects ' . $where_class . 'ORDER BY ' . $pv{order_field} ;
...
# вывести непосредственно контент
   ...
   print "<TR><TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_objects_list_all.cgi?order_field=OWNER\&report_type=$pv{report_type}\&owner=$pv{owner}\&status=$pv{status}\&object_type=$pv{object_type}\&period=$pv{period}\&period_from=$pv{period_from}\&period_to=$pv{period_to}\&name_include=$pv{name_include}\&isfirst=$pv{isfirst}\" TARGET=\"cont\">Владелец</A></TD>
              <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_objects_list_all.cgi?order_field=OBJECT_NAME\&report_type=$pv{report_type}\&owner=$pv{owner}\&status=$pv{status}\&object_type=$pv{object_type}\&period=$pv{period}\&period_from=$pv{period_from}\&period_to=$pv{period_to}\&name_include=$pv{name_include}\&isfirst=$pv{isfirst}\" TARGET=\"cont\">Имя объекта</A></TD>
              <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_objects_list_all.cgi?order_field=SUBOBJECT_NAME\&report_type=$pv{report_type}\&owner=$pv{owner}\&status=$pv{status}\&object_type=$pv{object_type}\&period=$pv{period}\&period_from=$pv{period_from}\&period_to=$pv{period_to}\&name_include=$pv{name_include}\&isfirst=$pv{isfirst}\" TARGET=\"cont\">Имя подобъекта</A></TD>
              <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_objects_list_all.cgi?order_field=OBJECT_ID\&report_type=$pv{report_type}\&owner=$pv{owner}\&status=$pv{status}\&object_type=$pv{object_type}\&period=$pv{period}\&period_from=$pv{period_from}\&period_to=$pv{period_to}\&name_include=$pv{name_include}\&isfirst=$pv{isfirst}\" TARGET=\"cont\">ID объекта</A></TD>
              <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_objects_list_all.cgi?order_field=DATA_OBJECT_ID\&report_type=$pv{report_type}\&owner=$pv{owner}\&status=$pv{status}\&object_type=$pv{object_type}\&period=$pv{period}\&period_from=$pv{period_from}\&period_to=$pv{period_to}\&name_include=$pv{name_include}\&isfirst=$pv{isfirst}\" TARGET=\"cont\">ID сегмента</A></TD>
              <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_objects_list_all.cgi?order_field=OBJECT_TYPE\&report_type=$pv{report_type}\&owner=$pv{owner}\&status=$pv{status}\&object_type=$pv{object_type}\&period=$pv{period}\&period_from=$pv{period_from}\&period_to=$pv{period_to}\&name_include=$pv{name_include}\&isfirst=$pv{isfirst}\" TARGET=\"cont\">Тип объекта</A></TD>
              <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_objects_list_all.cgi?order_field=CREATED\&report_type=$pv{report_type}\&owner=$pv{owner}\&status=$pv{status}\&object_type=$pv{object_type}\&period=$pv{period}\&period_from=$pv{period_from}\&period_to=$pv{period_to}\&name_include=$pv{name_include}\&isfirst=$pv{isfirst}\" TARGET=\"cont\">Дата создания</A></TD>
              <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_objects_list_all.cgi?order_field=LAST_DDL_TIME\&report_type=$pv{report_type}\&owner=$pv{owner}\&status=$pv{status}\&object_type=$pv{object_type}\&period=$pv{period}\&period_from=$pv{period_from}\&period_to=$pv{period_to}\&name_include=$pv{name_include}\&isfirst=$pv{isfirst}\" TARGET=\"cont\">Модификация определения</A></TD>
              <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_objects_list_all.cgi?order_field=STATUS\&report_type=$pv{report_type}\&owner=$pv{owner}\&status=$pv{status}\&object_type=$pv{object_type}\&period=$pv{period}\&period_from=$pv{period_from}\&period_to=$pv{period_to}\&name_include=$pv{name_include}\&isfirst=$pv{isfirst}\" TARGET=\"cont\">Статус</A></TD>
              <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_objects_list_all.cgi?order_field=TEMPORARY\&report_type=$pv{report_type}\&owner=$pv{owner}\&status=$pv{status}\&object_type=$pv{object_type}\&period=$pv{period}\&period_from=$pv{period_from}\&period_to=$pv{period_to}\&name_include=$pv{name_include}\&isfirst=$pv{isfirst}\" TARGET=\"cont\">Временный</A></TD>
              <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_objects_list_all.cgi?order_field=GENERATED\&report_type=$pv{report_type}\&owner=$pv{owner}\&status=$pv{status}\&object_type=$pv{object_type}\&period=$pv{period}\&period_from=$pv{period_from}\&period_to=$pv{period_to}\&name_include=$pv{name_include}\&isfirst=$pv{isfirst}\" TARGET=\"cont\">Имя сгенерировано автоматом</A></TD>
              <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_objects_list_all.cgi?order_field=SECONDARY\&report_type=$pv{report_type}\&owner=$pv{owner}\&status=$pv{status}\&object_type=$pv{object_type}\&period=$pv{period}\&period_from=$pv{period_from}\&period_to=$pv{period_to}\&name_include=$pv{name_include}\&isfirst=$pv{isfirst}\" TARGET=\"cont\">SECONDARY</A></TD>
              </TR>" ;
   $count_rows = 0 ;
   while (my ($owner,$object_name,$subobject_name,$object_id,$data_object_id,$object_type,$created,$last_ddl_time,$status,$temporary,$generated,$secondary) = $sth->fetchrow_array() ) {
         $count_rows += 1 ;
         if ( $object_id eq "" ) { $object_id = "&nbsp;" ; }
         if ( $data_object_id eq "" ) { $data_object_id = "&nbsp;" ; }
         if ( $subobject_name eq "" ) { $subobject_name = "&nbsp;" ; }
         if ( $data_object_id eq "" ) { $data_object_id = "&nbsp;" ; }
         if ( $last_ddl_time eq "" ) { $last_ddl_time = "&nbsp;" ; } $last_ddl_time =~ s/\s/&nbsp;/ ;
         $created =~ s/\s/&nbsp;/ ;
         $last_ddl_time =~ s/\s/&nbsp;/ ;
         $object_type =~ s/\s/&nbsp;/ ;
         printf("<TR><TD CLASS=\"SZDATA\">%s</TD>
                     ...
                     <TD CLASS=\"CENTERDATA\">%s</TD>
                     <TD CLASS=\"CENTERDATA\">%s</TD>
                 </TR>\n",$owner,$object_name,$subobject_name,$object_id,$data_object_id,$object_type,$created,$last_ddl_time,$status,$temporary,$generated,$secondary) ;
         }
   print "<TR><TD CLASS=\"SZDATA\">Итого</TD><TD CLASS=\"NUMDATA\" COLSPAN=\"12\">$count_rows</TD></TR>\n" ;
   print "</TABLE>" ;
   $sth->finish();
   $dbh->disconnect();
   print "</TD></TR></TABLE>" ;
