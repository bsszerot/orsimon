...
&print_html_head("Информация об SGA") ;

# добавить определение навигационно - оформительского механизма закладок
&print_sga_navigation(1) ;

my $dbh = DBI->connect( $connector_definition{$current_connector}, $connector_credentials{$current_connector}, '');
#$request = 'select POOL,NAME,BYTES from v$sgastat ORDER BY ' . $pv{order_field} ;
$request = "select CASE WHEN NAME  IN ('dictionary cache','library cache','buffer_cache','log_buffer','fixed_sga','sql area') THEN '0 main' 
                        WHEN NAME = 'free memory' THEN '1 free' 
                        WHEN POOL = 'shared pool' AND NAME  IN ('Checkpoint queue','enqueue','errors','parameters','processes','session param values','sessions','trace buffer','transaction') THEN '2 sec' 
                        ELSE '3 other' END TYPE,POOL,NAME,BYTES from v\$sgastat ORDER BY $pv{order_field}" ;
...
print "<TR><TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_sga_stat.cgi?order_field=TYPE,NAME\" TARGET=\"cont\">Тип</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_sga_stat.cgi?order_field=NAME\" TARGET=\"cont\">Раздел</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_sga_stat.cgi?order_field=POOL\" TARGET=\"cont\">Пул</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_sga_stat.cgi?order_field=BYTES\" TARGET=\"cont\">Размер, байт</A></TD>
           </TR>" ;
$count_rows = 0 ;
while (my ($type,$pool,$name,$bytes) = $sth->fetchrow_array() ) {
      $count_rows += 1 ;
      if ( $pool eq "" ) { $pool = "&nbsp;" ; }
      $bgcolor = "" ; if ($type eq '0 main' || $type eq '2 sec') { $bgcolor = " STYLE=\"background-color: #e0f2ff;\"" ; }
      printf("<TR><TD CLASS=\"SZDATA\" $bgcolor>%s</TD>
                  <TD CLASS=\"SZDATA\" $bgcolor>%s</TD>
                  <TD CLASS=\"SZDATA\" $bgcolor>%s</TD>
                  <TD CLASS=\"NUMDATA\" $bgcolor>%s</TD>
              </TR>\n",$type,$name,$pool,&show_razryads($bytes)) ;
      }
print "<TR><TD CLASS=\"SZDATA\">Итого</TD><TD CLASS=\"NUMDATA\" COLSPAN=\"3\">$count_rows</TD></TR>\n" ;
print "</TABLE>" ;
print "</TD></TR></TABLE>" ;
$sth->finish();
$dbh->disconnect();                                        
