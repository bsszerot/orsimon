...
&print_html_head("Статистика пулов буферов") ;

# добавить определение навигационно - оформительского механизма закладок
&print_sga_navigation(3) ;

my $dbh = DBI->connect( $connector_definition{$current_connector}, $connector_credentials{$current_connector}, '');
$request = "SELECT ID, NAME, BLOCK_SIZE, SET_MSIZE, CNUM_REPL, CNUM_WRITE, CNUM_SET, BUF_GOT, SUM_WRITE, SUM_SCAN, FREE_BUFFER_WAIT, WRITE_COMPLETE_WAIT,
                   BUFFER_BUSY_WAIT, FREE_BUFFER_INSPECTED, DIRTY_BUFFERS_INSPECTED, DB_BLOCK_CHANGE, DB_BLOCK_GETS, CONSISTENT_GETS, PHYSICAL_READS, 
                   PHYSICAL_WRITES FROM V\$BUFFER_POOL_STATISTICS ORDER BY $pv{order_field}" ;
...
print "<TR><TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_sga_buffer_pool.cgi?order_field=ID\" TARGET=\"cont\">ID</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_sga_buffer_pool.cgi?order_field=NAME\" TARGET=\"cont\">Имя объекта</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_sga_buffer_pool.cgi?order_field=BLOCK_SIZE\" TARGET=\"cont\">Размер блока</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_sga_buffer_pool.cgi?order_field=SET_MSIZE\" TARGET=\"cont\">SET_MSIZE</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_sga_buffer_pool.cgi?order_field=CNUM_REPL\" TARGET=\"cont\">CNUM_REPL</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_sga_buffer_pool.cgi?order_field=CNUM_WRITE\" TARGET=\"cont\">CNUM_WRITE</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_sga_buffer_pool.cgi?order_field=CNUM_SET\" TARGET=\"cont\">CNUM_SET</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_sga_buffer_pool.cgi?order_field=BUF_GOT\" TARGET=\"cont\">BUF_GOT</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_sga_buffer_pool.cgi?order_field=SUM_WRITE\" TARGET=\"cont\">SUM_WRITE</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_sga_buffer_pool.cgi?order_field=SUM_SCAN\" TARGET=\"cont\">SUM_SCAN</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_sga_buffer_pool.cgi?order_field=FREE_BUFFER_WAIT\" TARGET=\"cont\">FREE_BUFFER_WAIT</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_sga_buffer_pool.cgi?order_field=WRITE_COMPLETE_WAIT\" TARGET=\"cont\">WRITE_COMPLETE_WAIT</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_sga_buffer_pool.cgi?order_field=BUFFER_BUSY_WAIT\" TARGET=\"cont\">BUFFER_BUSY_WAIT</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_sga_buffer_pool.cgi?order_field=FREE_BUFFER_INSPECTED\" TARGET=\"cont\">FREE_BUFFER_INSPECTED</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_sga_buffer_pool.cgi?order_field=DIRTY_BUFFERS_INSPECTED\" TARGET=\"cont\">DIRTY_BUFFERS_INSPECTED</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_sga_buffer_pool.cgi?order_field=DB_BLOCK_CHANGE\" TARGET=\"cont\">DB_BLOCK_CHANGE</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_sga_buffer_pool.cgi?order_field=DB_BLOCK_GETS\" TARGET=\"cont\">DB_BLOCK_GETS</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_sga_buffer_pool.cgi?order_field=CONSISTENT_GETS\" TARGET=\"cont\">CONSISTENT_GETS</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_sga_buffer_pool.cgi?order_field=PHYSICAL_READS\" TARGET=\"cont\">PHYSICAL_READS</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_sga_buffer_pool.cgi?order_field=PHYSICAL_WRITES\" TARGET=\"cont\">PHYSICAL_WRITES</A></TD>
           </TR>" ;
$count_rows = 0 ;
while (my ($id, $name, $block_size, $set_msize, $cnum_repl, $cnum_write, $cnum_set, $buf_got, $sum_write, $sum_scan, $free_buffer_wait, 
          $write_complete_wait, $buffer_busy_wait, $free_buffer_inspected, $dirty_buffer_inspected, $db_block_change, $db_block_gets, $consistent_gets, 
          $physical_reads, $physical_writes) = $sth->fetchrow_array() ) { $count_rows ++ ;
      if ( $pool eq "" ) { $pool = "&nbsp;" ; }
      printf("<TR><TD CLASS=\"NUMDATA\">%s</TD>
                  ...
                  <TD CLASS=\"NUMDATA\">%s</TD>
                  <TD CLASS=\"NUMDATA\">%s</TD>
              </TR>\n",$id, $name, &show_razryads($block_size), &show_razryads($set_msize), &show_razryads($cnum_repl), &show_razryads($cnum_write),
                       &show_razryads($cnum_set), &show_razryads($buf_got), &show_razryads($sum_write), &show_razryads($sum_scan), 
                       &show_razryads($free_buffer_wait), &show_razryads($write_complete_wait), &show_razryads($buffer_busy_wait), 
                       &show_razryads($free_buffer_inspected), &show_razryads($dirty_buffer_inspected), &show_razryads($db_block_change), 
                       &show_razryads($db_block_gets), &show_razryads($consistent_gets), &show_razryads($physical_reads), 
                       &show_razryads($physical_writes)) ;
      }
printf("<TR><TD CLASS=\"SZDATA\">Итого</TD><TD CLASS=\"NUMDATA\" COLSPAN=\"19\">%s</TD></TR>\n",$count_rows) ;
print "</TABLE>" ;
print "</TD></TR></TABLE>" ;
$sth->finish();
$dbh->disconnect();                                        
