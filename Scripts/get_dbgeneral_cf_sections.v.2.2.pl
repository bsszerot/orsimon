...
$request = 'select TYPE, RECORD_SIZE, RECORDS_TOTAL, RECORDS_USED, FIRST_INDEX, LAST_INDEX, LAST_RECID from V$CONTROLFILE_RECORD_SECTION ORDER BY ' . $order_field ;
                                 
my $sth = $dbh->prepare($request) ;
$sth->execute();
print "Content-Type: text/html\n\n"; 
&print_html_head("Информация о секциях контрольных файлов") ;

# добавить определение навигационно - оформительского механизма закладок
&print_dbgeneral_navigation(4) ;
...
print "<TR><TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_dbgeneral_cf_sections.cgi?order_field=TYPE\" TARGET=\"cont\">Тип</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_dbgeneral_cf_sections.cgi?order_field=RECORD_SIZE\" TARGET=\"cont\">Размер записи, байт</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_dbgeneral_cf_sections.cgi?order_field=RECORDS_TOTAL\" TARGET=\"cont\">Всего записей секции</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_dbgeneral_cf_sections.cgi?order_field=RECORDS_USED\" TARGET=\"cont\">Использовано записей секции</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_dbgeneral_cf_sections.cgi?order_field=FIRST_INDEX\" TARGET=\"cont\">Позиция первой записи</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_dbgeneral_cf_sections.cgi?order_field=LAST_INDEX\" TARGET=\"cont\">Позиция последней записи</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_dbgeneral_cf_sections.cgi?order_field=LAST_RECID\" TARGET=\"cont\">ID последней записи</A></TD>
           </TR>" ;
$count_rows = 0 ;
while (my ($type,$redo_size,$records_total,$records_used,$first_index,$last_index,$last_recid) = $sth->fetchrow_array() ) {
      $count_rows += 1 ; $type =~ s/\s+/&nbsp;/g ;
      printf("<TR><TD CLASS=\"SZDATA\">%s</TD>
                  ...
                  <TD CLASS=\"NUMDATA\">%s</TD>
                  <TD CLASS=\"NUMDATA\">%s</TD>
              </TR>\n",$type,&show_razryads($redo_size),&show_razryads($records_total),&show_razryads($records_used),&show_razryads($first_index),
                       &show_razryads($last_index),&show_razryads($last_recid)) ;
      }
