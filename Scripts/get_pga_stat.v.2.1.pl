...
print "<TR><TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_pga_stat.cgi?order_field=NAME\" TARGET=\"cont\">Параметр</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_pga_stat.cgi?order_field=VALUE\" TARGET=\"cont\">Значение</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_pga_stat.cgi?order_field=UNIT\" TARGET=\"cont\">Размерность</A></TD>
           </TR>" ;
$count_rows = 0 ;
while (my ($name,$value,$unit) = $sth->fetchrow_array() ) {
      $count_rows += 1 ;
      if ( $unit eq "" ) { $unit = "&nbsp;" ; }
      if ( $name eq "cache hit percentage" ) { $name = "cache hit percentage (обработки без one и multipass)" ; }
      printf("<TR><TD CLASS=\"SZDATA\">%s</TD>
                  <TD CLASS=\"NUMDATA\">%s</TD>
                  <TD CLASS=\"SZDATA\">%s</TD>
              </TR>\n",$name,&show_razryads($value),$unit) ;
      }
print "<TR><TD CLASS=\"SZDATA\">Итого строк</TD><TD CLASS=\"NUMDATA\" COLSPAN=\"2\">$count_rows</TD></TR>\n" ;
print "</TABLE>" ;
print "</TD></TR></TABLE>" ;
$sth->finish();
$dbh->disconnect();                                        
