...
&print_html_head("Прогностика buffer cache") ;

# добавить определение навигационно - оформительского механизма закладок
&print_sga_navigation(4) ;

my $dbh = DBI->connect( $connector_definition{$current_connector}, $connector_credentials{$current_connector}, '');
$request = "select name,size_for_estimate, buffers_for_estimate, estd_physical_read_factor, estd_physical_reads FROM V\$DB_CACHE_ADVICE WHERE name = 'DEFAULT' AND block_size = (SELECT value FROM V\$PARAMETER WHERE name = 'db_block_size') AND advice_status = 'ON' ORDER BY NAME,$pv{order_field}" ;
...
print "<TR><TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_sga_bufcache_advice.cgi?order_field=size_for_estimate\" TARGET=\"cont\">Название пула</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_sga_bufcache_advice.cgi?order_field=size_for_estimate\" TARGET=\"cont\">Размер кэша</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_sga_bufcache_advice.cgi?order_field=buffers_for_estimate\" TARGET=\"cont\">Количество буферов</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_sga_bufcache_advice.cgi?order_field=estd_physical_read_factor\" TARGET=\"cont\">Фактор</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_sga_bufcache_advice.cgi?order_field=estd_physical_reads\" TARGET=\"cont\">Ожидаемых физических чтений</A></TD>
           </TR>" ;
$count_rows = 0 ;
while (my ($name,$size,$buffers,$factor,$phreads) = $sth->fetchrow_array() ) {
      $count_rows += 1 ;
      if ( $pool eq "" ) { $pool = "&nbsp;" ; }
      printf("<TR><TD CLASS=\"SZDATA\">%s</TD>
                  ...
                  <TD CLASS=\"NUMDATA\">%s</TD>
                  <TD CLASS=\"NUMDATA\">%s</TD>
              </TR>\n",$name,&show_razryads($size),&show_razryads($buffers),$factor,&show_razryads($phreads)) ;
      }
print "<TR><TD CLASS=\"SZDATA\">Итого</TD><TD CLASS=\"NUMDATA\" COLSPAN=\"4\">$count_rows</TD></TR>\n" ;
print "</TABLE>" ;
print "</TD></TR></TABLE>" ;
$sth->finish();
$dbh->disconnect();                                        
