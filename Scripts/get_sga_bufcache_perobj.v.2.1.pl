...
$pv{order_field} = "NUMBER_OF_BLOCKS DESC" ;
&get_forms_param() ;

$ENV{ORACLE_SID} = $current_connector ;

print "Content-Type: text/html\n\n";
&print_html_head("Информация об утилизации buffer cache объектами") ;

# добавить определение навигационно - оформительского механизма закладок
&print_sga_navigation(5) ;

my $dbh = DBI->connect( $connector_definition{$current_connector}, $connector_credentials{$current_connector}, '');
$request = "SELECT o.owner owner, o.object_name object_name, o.object_type object_type, COUNT(2) number_of_blocks FROM DBA_OBJECTS o, V\$BH bh WHERE o.object_id  = bh.objd AND o.owner != 'SYS' GROUP BY o.owner,o.object_name,o.object_type ORDER BY $pv{order_field}" ;
...
print "<TR><TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_sga_bufcache_perobj.cgi?order_field=owner\" TARGET=\"cont\">Владелец</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_sga_bufcache_perobj.cgi?order_field=object_name\" TARGET=\"cont\">Имя объекта</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_sga_bufcache_perobj.cgi?order_field=object_type\" TARGET=\"cont\">Тип объекта</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_sga_bufcache_perobj.cgi?order_field=number_of_blocks DESC\" TARGET=\"cont\">Количество буферов</A></TD>
           </TR>" ;
$count_rows = 0 ;
$all_buffers = 0 ;
while (my ($owner,$object_name,$object_type,$number_of_blocks) = $sth->fetchrow_array() ) {
      $all_buffers += $number_of_blocks ;
      if ( $pool eq "" ) { $pool = "&nbsp;" ; }
      printf("<TR><TD CLASS=\"SZDATA\">%s</TD>
                  <TD CLASS=\"SZDATA\">%s</TD>
                  <TD CLASS=\"SZDATA\">%s</TD>
                  <TD CLASS=\"NUMDATA\">%s</TD>
              </TR>\n",$owner,$object_name,$object_type,&show_razryads($number_of_blocks)) ;
      }
printf("<TR><TD CLASS=\"SZDATA\">Итого</TD><TD CLASS=\"NUMDATA\" COLSPAN=\"3\">%s</TD></TR>\n",&show_razryads($all_buffers)) ;
print "</TABLE>" ;
print "</TD></TR></TABLE>" ;
$sth->finish();
$dbh->disconnect();                                        
