...
&print_html_head("Информация о процессах архивации") ;
...
&print_archivelogs_navigation(2) ;
...
print "<TR><TD>" ;
$request = 'select PROCESS,STATUS,LOG_SEQUENCE,STATE from V$ARCHIVE_PROCESSES ORDER BY ' . $order_field ;
my $sth = $dbh->prepare($request) ;
$sth->execute();
# вывести непосредственно контент
print "<TABLE BORDER=\"1\" CELLSPACING=\"0\" CELLPADDING=\"0\" STYLE=\"width: 100%;\">" ;
print "<TR><TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_archive_processes_status.cgi?order_field=PROCESS\" TARGET=\"cont\">Процесс</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_archive_processes_status.cgi?order_field=STATUS\" TARGET=\"cont\">Статус</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_archive_processes_status.cgi?order_field=LOG_SEQUENCE\" TARGET=\"cont\">LOG SEQUENCE</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_archive_processes_status.cgi?order_field=STATE\" TARGET=\"cont\">Режим</A></TD>
       </TR>" ;
$count_rows = 0 ;
while (my ($process,$status,$log_sequence,$state) = $sth->fetchrow_array() ) {
      $count_rows += 1 ;
      printf("<TR><TD CLASS=\"SZDATA\">%s</TD>
                  <TD CLASS=\"CENTERDATA\">%s</TD>
                  <TD CLASS=\"NUMDATA\">%s</TD>
                  <TD CLASS=\"CENTERDATA\">%s</TD>
              </TR>\n",$process,$status,&show_razryads($log_sequence),$state) ;
      }
