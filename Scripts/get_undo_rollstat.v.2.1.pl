...
$pv{order_field} = "USN" ; $pv{report_type} = "" ;
get_forms_param() ;
...
&print_html_head("Статистика сегментов отмены Rollstat") ;
...
print "<FORM ACTION=\"$base_url/cgi/get_undo_rollstat.cgi\" METHOD=\"GET\">
...
# === ОТЧЕТ - полный список полей v$archive_log
   ...
   $request = 'select rn.NAME, rs.USN, rs.LATCH, rs.EXTENTS, rs.RSSIZE, rs.WRITES, rs.XACTS, rs.GETS, rs.WAITS, rs.OPTSIZE, rs.HWMSIZE, rs.SHRINKS, 
                      rs.WRAPS, rs.EXTENDS, rs.AVESHRINK, rs.AVEACTIVE, rs.STATUS, rs.CUREXT, rs.CURBLK from v$rollname rn, v$rollstat rs WHERE rn.USN = rs.USN ORDER BY ' . $pv{order_field} ;
#      $request = 'select USN, LATCH, EXTENTS, RSSIZE, WRITES, XACTS, GETS, WAITS, OPTSIZE, HWMSIZE, SHRINKS, WRAPS, EXTENDS, AVESHRINK, AVEACTIVE, STATUS, 
#                         CUREXT, CURBLK from v$rollstat ' . $where_class . ' ORDER BY ' . $pv{order_field} ;
...
# вывести непосредственно контент
      ...
      print "<TR>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_undo_rollstat.cgi?order_field=rn.NAME\&report_type=$pv{report_type}\&period=$pv{period}\&period_from=$pv{period_from}\&period_to=$pv{period_to}\" TARGET=\"cont\">Имя сегмента</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_undo_rollstat.cgi?order_field=rs.USN\&report_type=$pv{report_type}\&period=$pv{period}\&period_from=$pv{period_from}\&period_to=$pv{period_to}\" TARGET=\"cont\">USN</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_undo_rollstat.cgi?order_field=rs.LATCH\&report_type=$pv{report_type}\&period=$pv{period}\&period_from=$pv{period_from}\&period_to=$pv{period_to}\" TARGET=\"cont\">LATCH</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_undo_rollstat.cgi?order_field=rs.EXTENTS\&report_type=$pv{report_type}\&period=$pv{period}\&period_from=$pv{period_from}\&period_to=$pv{period_to}\" TARGET=\"cont\">Экстентов&nbsp;в сегменте</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_undo_rollstat.cgi?order_field=rs.RSSIZE\&report_type=$pv{report_type}\&period=$pv{period}\&period_from=$pv{period_from}\&period_to=$pv{period_to}\" TARGET=\"cont\">Размер сегмента, байт</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_undo_rollstat.cgi?order_field=rs.WRITES\&report_type=$pv{report_type}\&period=$pv{period}\&period_from=$pv{period_from}\&period_to=$pv{period_to}\" TARGET=\"cont\">Записей отмены, байт</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_undo_rollstat.cgi?order_field=rs.XACTS\&report_type=$pv{report_type}\&period=$pv{period}\&period_from=$pv{period_from}\&period_to=$pv{period_to}\" TARGET=\"cont\">Количество активных транзакций</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_undo_rollstat.cgi?order_field=rs.GETS\&report_type=$pv{report_type}\&period=$pv{period}\&period_from=$pv{period_from}\&period_to=$pv{period_to}\" TARGET=\"cont\">Полученных заголовков сегмента</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_undo_rollstat.cgi?order_field=rs.WAITS\&report_type=$pv{report_type}\&period=$pv{period}\&period_from=$pv{period_from}\&period_to=$pv{period_to}\" TARGET=\"cont\">Ожиданий заголовков сегмента</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_undo_rollstat.cgi?order_field=rs.OPTSIZE\&report_type=$pv{report_type}\&period=$pv{period}\&period_from=$pv{period_from}\&period_to=$pv{period_to}\" TARGET=\"cont\">Оптимальный размер сегмента</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_undo_rollstat.cgi?order_field=rs.HWMSIZE\&report_type=$pv{report_type}\&period=$pv{period}\&period_from=$pv{period_from}\&period_to=$pv{period_to}\" TARGET=\"cont\">HWM сегмента</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_undo_rollstat.cgi?order_field=rs.SHRINKS\&report_type=$pv{report_type}\&period=$pv{period}\&period_from=$pv{period_from}\&period_to=$pv{period_to}\" TARGET=\"cont\">Усечений сегмента, раз</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_undo_rollstat.cgi?order_field=rs.WRAPS\&report_type=$pv{report_type}\&period=$pv{period}\&period_from=$pv{period_from}\&period_to=$pv{period_to}\" TARGET=\"cont\">WRAP сегмента, раз</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_undo_rollstat.cgi?order_field=rs.EXTENDS\&report_type=$pv{report_type}\&period=$pv{period}\&period_from=$pv{period_from}\&period_to=$pv{period_to}\" TARGET=\"cont\">Расширений сегмента, раз</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_undo_rollstat.cgi?order_field=rs.AVESHRINK\&report_type=$pv{report_type}\&period=$pv{period}\&period_from=$pv{period_from}\&period_to=$pv{period_to}\" TARGET=\"cont\">Размер среднего усечения</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_undo_rollstat.cgi?order_field=rs.AVEACTIVE\&report_type=$pv{report_type}\&period=$pv{period}\&period_from=$pv{period_from}\&period_to=$pv{period_to}\" TARGET=\"cont\">Средний размер экстентов</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_undo_rollstat.cgi?order_field=rs.STATUS\&report_type=$pv{report_type}\&period=$pv{period}\&period_from=$pv{period_from}\&period_to=$pv{period_to}\" TARGET=\"cont\">Статус</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_undo_rollstat.cgi?order_field=rs.CURREXT\&report_type=$pv{report_type}\&period=$pv{period}\&period_from=$pv{period_from}\&period_to=$pv{period_to}\" TARGET=\"cont\">Текущий экстент</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_undo_rollstat.cgi?order_field=rs.CURRBLK\&report_type=$pv{report_type}\&period=$pv{period}\&period_from=$pv{period_from}\&period_to=$pv{period_to}\" TARGET=\"cont\">Текущий блок</A></TD>
           </TR>" ;
      $count_rows = 0 ;
      while (my ($name,$usn,$latch,$extents,$rssize,$writes,$xacts,$gets,$waits,$optsize,$hwmsize,$shrinks,$wraps,$extends,$aveshrink,$aveactive,$status,$curext,$curblk) = $sth->fetchrow_array() ) {
            $count_rows += 1 ; if ( $optsize == "" ) { $optsize = "&nbsp;" ; } else { $optsize = &show_razryads($optsize) ; }
            printf("<TR><TD CLASS=\"NUMDATA\">%s</TD>
                        ...
                        <TD CLASS=\"NUMDATA\">%s</TD>
                        <TD CLASS=\"NUMDATA\">%s</TD>
                    </TR>\n",$name,$usn,$latch,&show_razryads($extents),&show_razryads($rssize),&show_razryads($writes),&show_razryads($xacts),
                             &show_razryads($gets),&show_razryads($waits),$optsize,&show_razryads($hwmsize),&show_razryads($shrinks),
                             &show_razryads($wraps),&show_razryads($extends),&show_razryads($aveshrink),&show_razryads($aveactive),$status,
                             &show_razryads($curext),&show_razryads($curblk)) ;
            }
        print "<TR><TD CLASS=\"SZDATA\">Итого</TD><TD CLASS=\"NUMDATA\" COLSPAN=\"18\">$count_rows</TD></TR>\n" ;
        print "</TABLE>" ;
        $sth->finish();
        $dbh->disconnect();
