...
#$request = 'select WAITING_SESSION, HOLDING_SESSION, LOCK_ID1, LOCK_ID2, LOCK_TYPE, 
#                   decode(MODE_HELD,0,\'none\',1,\'null (NULL)\',2,\'row-S (SS)\',3,\'row-X (SX)\',4,\'share (X)\',5,\'S/Row-X (SSX)\',6,\'executive (X)\',MODE_HELD), 
#                   decode(MODE_REQUESTED,0,\'none\',1,\'null (NULL)\',2,\'row-S (SS)\',3,\'row-X (SX)\',4,\'share (X)\',5,\'S/Row-X (SSX)\',6,\'executive (X)\',MODE_REQUESTED) from dba_waiters ORDER BY ' . $order_field ;

$request = 'select WAITING_SESSION, HOLDING_SESSION, LOCK_ID1, LOCK_ID2, LOCK_TYPE, MODE_HELD,MODE_REQUESTED from dba_waiters ORDER BY ' . $order_field ;

my $sth = $dbh->prepare($request) ;
$sth->execute();
print "Content-Type: text/html\n\n"; 
&print_html_head("Ждущие освобождения блокировок сессии") ;
# добавить определение навигационно - оформительского механизма закладок
&print_locks_navigation(5) ;
...
print "<TR><TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_waiters_session.cgi?order_field=WAITING_SESSION\" TARGET=\"cont\">SID ждущей сессии</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_waiters_session.cgi?order_field=HOLDING_SESSION\" TARGET=\"cont\">SID блокирующей сессии</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_waiters_session.cgi?order_field=LOCK_ID1\" TARGET=\"cont\">ID1</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_waiters_session.cgi?order_field=LOCK_ID2\" TARGET=\"cont\">ID2</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_waiters_session.cgi?order_field=LOCK_TYPE\" TARGET=\"cont\">Тип блокировки</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_waiters_session.cgi?order_field=MODE_HELD\" TARGET=\"cont\">Режим блокировки</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_waiters_session.cgi?order_field=MODE_REQUESTED\" TARGET=\"cont\">Режим ожидания</A></TD>
           </TR>" ;
$is_not_null_answer = "no" ; $sum_records = 0 ;
while (my ($waiter_sid,$holder_sid,$id1,$id2,$type,$lmode,$request) = $sth->fetchrow_array() ) { $sum_records += 1 ;
      ...
      printf("<TR><TD CLASS=\"SZDATA\">%s</TD>
                  ...
                  <TD CLASS=\"CENTERDATA\">%s</TD>
              </TR>\n",$waiter_sid,$holder_sid,$id1,$id2,$type,$lmode,$request) ;
      $is_not_null_answer = "yes" ;
      }
if ( $is_not_null_answer eq "no" ) { print "<TR><TD CLASS=\"SZDATA\" COLSPAN=\"9\">Ожидающие сессии отсутствуют в DBA_WAITERS</TD></TR>" ;}
