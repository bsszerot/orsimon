...
&print_pga_navigation(7) ;

my $dbh = DBI->connect( $connector_definition{$current_connector}, $connector_credentials{$current_connector}, '');
$request = "SELECT sql_text, sum(ONEPASS_EXECUTIONS) onepass_cnt, sum(MULTIPASSES_EXECUTIONS) mpass_cnt 
...
print "<TR><TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_pga_pass_cursors.cgi?order_field=sql_text\" TARGET=\"cont\">sql_text</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_pga_pass_cursors.cgi?order_field=onepass_cnt\" TARGET=\"cont\">onepass_cnt</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_pga_pass_cursors.cgi?order_field=mpass_cnt\" TARGET=\"cont\">mpass_cnt</A></TD>
           </TR>" ;
$count_rows = 0 ;
while (my ($sql_text,$onepass_cnt,$mpass_cnt) = $sth->fetchrow_array() ) { $count_rows += 1 ;
      printf("<TR><TD CLASS=\"SZDATA\">%s</TD>
                  <TD CLASS=\"NUMDATA\">%s</TD>
                  <TD CLASS=\"NUMDATA\">%s</TD>
              </TR>\n",$sql_text,$onepass_cnt,$mpass_cnt) ;
      }
printf("<TR><TD CLASS=\"SZDATA\" COLSPAN=\"2\">Итого строк: </TD><TD CLASS=\"NUMDATA\">%s</TD></TR>\n",$count_rows) ;
print "</TABLE>" ;
print "</TD></TR></TABLE>" ;
$sth->finish();
$dbh->disconnect();                                        
