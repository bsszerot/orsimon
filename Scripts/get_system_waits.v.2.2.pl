...
&print_html_head("Ожидания сессий экземпляра") ;
...
&print_system_waits_navigation(1) ;

print "</TD><TD STYLE=\" padding: 0pt; height: 100%;\">
       <TABLE CELLSPACING=\"0\" CELLPADDING=\"0\" WIDTH=\"100%\" STYLE=\"border: 0pt none; width: 100%; height: 100%;\">
              <TR><TD STYLE=\"border: 2pt navy; border-style: none none solid none; width: 100%; height: 100%;\">&nbsp;</TD></TR>
       </TABLE>
       </TD></TR>
...
print "<FORM ACTION=\"$base_url/cgi/get_system_waits.cgi\" METHOD=\"GET\">
...
$ext_where_term = "" ; 
if ( $pv{incltype} eq "group" && $pv{eventsinc} ne "all" && $pv{eventsinc} ne "" ) { $ext_where_term = " AND w.EVENT IN ($events_group_members{$pv{eventsinc}}) " ; }
if ( $pv{incltype} eq "single" && $pv{sngleventsinc} ne "all" && $pv{sngleventsinc} ne "" ) { $ext_where_term = ' AND en.EVENT# = ' . "$pv{sngleventsinc} " ; }
if ( $pv{excltype} eq "group" && $pv{eventsexcl} ne "none" && $pv{eventsexcl} ne "" ) { $ext_where_term .= " AND w.EVENT NOT IN ($events_group_members{$pv{eventsexcl}}) " ; } 
...
if ( $pv{fl_sid} ne "" ) { $ext_where_term .= ' AND w.SID = ' . "$pv{fl_sid} " ; } 

#################
if ( $pv{srcptr} eq "0" || $pv{srcptr} eq "" ) { 
   $request = 'select w.SID,s.SERIAL#,s.STATUS,w.SEQ# seq_num,w.EVENT,w.P1TEXT,w.P1,w.P1RAW,w.P2TEXT,w.P2,w.P2RAW,w.P3TEXT,w.P3,w.P3RAW,w.WAIT_TIME,w.SECONDS_IN_WAIT,
                      w.STATE from v$session_wait w, v$session s, v$event_name en where s.sid = w.sid AND w.EVENT = en.NAME ' . $ext_where_term . " order by $pv{order_field} asc" ; }
else { $request = 'select w.SID,s.SERIAL#,s.STATUS,w.SEQ# seq_num,w.EVENT,w.P1TEXT,w.P1,w.P1RAW,w.P2TEXT,w.P2,w.P2RAW,w.P3TEXT,w.P3,w.P3RAW,w.WAIT_TIME,w.SECONDS_IN_WAIT,
                          w.STATE from BESTAT_SESSION_WAITS w, BESTAT_SESSIONS s, v$event_name en where s.sid = w.sid AND w.POINT = s.POINT AND w.EVENT = en.NAME AND ' . " w.POINT = $pv{srcptr} AND s.POINT = $pv{srcptr} " . $ext_where_term . " order by $pv{order_field} asc" ; }

my $sth = $dbh->prepare($request) ;
$sth->execute();

print "<TABLE BORDER=\"1\" CELLSPACING=\"0\" CELLPADDING=\"0\" STYLE=\"width: 100%;\">" ;
print "<TR><TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_system_waits.cgi?order_field=s.SID&incltype=$pv{incltype}&sngleventsinc=$pv{sngleventsinc}&eventsinc=$pv{eventsinc}&excltype=$pv{excltype}&sngleventsexcl=$pv{sngleventsexcl}&eventsexcl=$pv{eventsexcl}&srcptr=$pv{srcptr}&is_no_idle=$pv{is_no_idle}\" TARGET=\"cont\">ID сессии</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_system_waits.cgi?order_field=s.STATUS&incltype=$pv{incltype}&sngleventsinc=$pv{sngleventsinc}&eventsinc=$pv{eventsinc}&excltype=$pv{excltype}&sngleventsexcl=$pv{sngleventsexcl}&eventsexcl=$pv{eventsexcl}&srcptr=$pv{srcptr}&is_no_idle=$pv{is_no_idle}\" TARGET=\"cont\">Статус сессии</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_system_waits.cgi?order_field=SEQ_NUM&incltype=$pv{incltype}&sngleventsinc=$pv{sngleventsinc}&eventsinc=$pv{eventsinc}&excltype=$pv{excltype}&sngleventsexcl=$pv{sngleventsexcl}&eventsexcl=$pv{eventsexcl}&srcptr=$pv{srcptr}&is_no_idle=$pv{is_no_idle}\" TARGET=\"cont\">Номер события</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_system_waits.cgi?order_field=w.EVENT&incltype=$pv{incltype}&sngleventsinc=$pv{sngleventsinc}&eventsinc=$pv{eventsinc}&excltype=$pv{excltype}&sngleventsexcl=$pv{sngleventsexcl}&eventsexcl=$pv{eventsexcl}&srcptr=$pv{srcptr}&is_no_idle=$pv{is_no_idle}\" TARGET=\"cont\">Событие ожидания</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_system_waits.cgi?order_field=w.P1TEXT&incltype=$pv{incltype}&sngleventsinc=$pv{sngleventsinc}&eventsinc=$pv{eventsinc}&excltype=$pv{excltype}&sngleventsexcl=$pv{sngleventsexcl}&eventsexcl=$pv{eventsexcl}&srcptr=$pv{srcptr}&is_no_idle=$pv{is_no_idle}\" TARGET=\"cont\">Описание P1/P2/P3</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_system_waits.cgi?order_field=w.P1&incltype=$pv{incltype}&sngleventsinc=$pv{sngleventsinc}&eventsinc=$pv{eventsinc}&excltype=$pv{excltype}&sngleventsexcl=$pv{sngleventsexcl}&eventsexcl=$pv{eventsexcl}&srcptr=$pv{srcptr}&is_no_idle=$pv{is_no_idle}\" TARGET=\"cont\">Значение P1/P2/P3</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_system_waits.cgi?order_field=w.P1RAW&incltype=$pv{incltype}&sngleventsinc=$pv{sngleventsinc}&eventsinc=$pv{eventsinc}&excltype=$pv{excltype}&sngleventsexcl=$pv{sngleventsexcl}&eventsexcl=$pv{eventsexcl}&srcptr=$pv{srcptr}&is_no_idle=$pv{is_no_idle}\" TARGET=\"cont\">RAW значение P1/P2/P3</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_system_waits.cgi?order_field=w.WAIT_TIME&incltype=$pv{incltype}&sngleventsinc=$pv{sngleventsinc}&eventsinc=$pv{eventsinc}&excltype=$pv{excltype}&sngleventsexcl=$pv{sngleventsexcl}&eventsexcl=$pv{eventsexcl}&srcptr=$pv{srcptr}&is_no_idle=$pv{is_no_idle}\" TARGET=\"cont\">Длительность последнего ожидания</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_system_waits.cgi?order_field=w.SECONDS_IN_WAIT&incltype=$pv{incltype}&sngleventsinc=$pv{sngleventsinc}&eventsinc=$pv{eventsinc}&excltype=$pv{excltype}&sngleventsexcl=$pv{sngleventsexcl}&eventsexcl=$pv{eventsexcl}&srcptr=$pv{srcptr}&is_no_idle=$pv{is_no_idle}\" TARGET=\"cont\">Секунд текущего ожидания</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_system_waits.cgi?order_field=w.STATE&incltype=$pv{incltype}&sngleventsinc=$pv{sngleventsinc}&eventsinc=$pv{eventsinc}&excltype=$pv{excltype}&sngleventsexcl=$pv{sngleventsexcl}&eventsexcl=$pv{eventsexcl}&srcptr=$pv{srcptr}&is_no_idle=$pv{is_no_idle}\" TARGET=\"cont\">Статус</A></TD>
           </TR>" ;

$system_waits_count_all = 0 ;
while (my ($sid,$serial,$session_status,$seq_num,$event,$p1text,$p1,$p1raw,$p2text,$p2,$p2raw,$p3text,$p3,$p3raw,$wait_time,$seconds_in_wait,$state) = $sth->fetchrow_array() ) {
      if ( $system_waits_count_by_status{$event} eq "" || $system_waits_count_by_status{$event} < 1 ) { $system_waits_count_by_status{$event} = 0 ; } 
      $system_waits_count_all += 1 ; $system_waits_count_by_status{$event} += 1 ;

      if ( $event eq "" ) { $event = "&nbsp;" ; } $event =~ s/\s/&nbsp;/g ; 
      if ( $p1raw eq "" ) { $p1raw = "&nbsp;" ; } if ( $p1 eq "" ) { $p1 = "&nbsp;" ; } if ( $p1text eq "" ) { $p1text = "&nbsp;" ; }
      if ( $p2raw eq "" ) { $p2raw = "&nbsp;" ; } if ( $p2 eq "" ) { $p2 = "&nbsp;" ; } if ( $p2text eq "" ) { $p2text = "&nbsp;" ; }
      if ( $p3raw eq "" ) { $p3raw = "&nbsp;" ; } if ( $p3 eq "" ) { $p3 = "&nbsp;" ; } if ( $p3text eq "" ) { $p3text = "&nbsp;" ; }
      if ( $wait_time eq "" ) { $wait_time = "&nbsp;" ; }
      if ( $seconds_in_wait eq "" ) { $seconds_in_wait = "&nbsp;" ; }
      if ( $state eq "" ) { $state = "&nbsp;" ; } $state =~ s/\s/&nbsp;/g ; 

      printf ("<TR><TD CLASS=\"SZDATA\"><A HREF=\"$base_url/cgi/get_session_info.cgi?sid=%d&serial=%d\" TARGET=\"cont\">%d,&nbsp;%d</TD>",$sid,$serial,$sid,$serial) ;
      print "      <TD CLASS=\"CENTERDATA\">$session_status</TD>
                   <TD CLASS=\"NUMDATA\">$seq_num</TD>
                   <TD CLASS=\"SZDATA\">$event</TD>
                   <TD CLASS=\"SZDATA\">$p1text<BR>$p2text<BR>$p3text</TD>
                   <TD CLASS=\"NUMDATA\">$p1<BR>$p2<BR>$p3</TD>
                   <TD CLASS=\"NUMDATA\">$p1raw<BR>$p2raw<BR>$p3raw</TD>
                   <TD CLASS=\"NUMDATA\">$wait_time</TD>
                   <TD CLASS=\"NUMDATA\">$seconds_in_wait</TD>
                   <TD CLASS=\"NUMDATA\">$state</TD>
                   </TR>\n" ;
      }
print "<TR><TD COLSPAN=\"10\">
           <TABLE BORDER=\"0\"><TR><TD>Текущих ожиданий всего:</TD><TD>$system_waits_count_all, в т.ч.</TD></TR>" ;
@system_waits_count_by_status_keys_nosort = keys %system_waits_count_by_status ; @system_waits_count_by_status_keys_sort = sort @system_waits_count_by_status_keys_nosort ;
for ($i=0;$i<=$#system_waits_count_by_status_keys_sort;$i++) {
    print "<TR><TD>&nbsp;&nbsp;&nbsp;$system_waits_count_by_status_keys_sort[$i]:</TD><TD>$system_waits_count_by_status{$system_waits_count_by_status_keys_sort[$i]}</TD></TR>" ;
    }
print "</TABLE></TD></TR>" ;
print "</TABLE>" ;
