...
print "<FORM ACTION=\"$base_url/cgi/get_files_iostat.cgi\" METHOD=\"GET\">
...
$request = 'select * from ( SELECT fs.FILE# FILE_ID, ts.NAME TABLESPACE_NAME, df.NAME FILE_NAME, fs.PHYRDS, fs.PHYWRTS, fs.PHYBLKRD, fs.PHYBLKWRT, fs.SINGLEBLKRDS, 
                                   fs.READTIM, fs.WRITETIM, fs.SINGLEBLKRDTIM, fs.AVGIOTIM, fs.LSTIOTIM, fs.MINIOTIM, fs.MAXIORTM, fs.MAXIOWTM
                                   FROM v$datafile df, v$filestat fs, v$tablespace ts
                                   WHERE df.FILE# = fs.FILE# AND df.TS# = ts.TS#
                            UNION ALL
                            SELECT fs.FILE# FILE_ID, ts.NAME TABLESPACE_NAME, tf.NAME FILE_NAME, fs.PHYRDS, fs.PHYWRTS, fs.PHYBLKRD, fs.PHYBLKWRT, fs.SINGLEBLKRDS, 
                                   fs.READTIM, fs.WRITETIM, fs.SINGLEBLKRDTIM, fs.AVGIOTIM, fs.LSTIOTIM, fs.MINIOTIM, fs.MAXIORTM, fs.MAXIOWTM
                                   FROM v$tempfile tf, v$tempstat fs, v$tablespace ts
                                   WHERE tf.FILE# = fs.FILE# AND tf.TS# = ts.TS# )  ' . $where_ext . " order by $pv{order_field} asc" ; }
else { $request = 'select * from ( SELECT fs.FILE#, ts.NAME TABLESPACE_NAME, df.NAME FILE_NAME, fs.PHYRDS, fs.PHYWRTS, fs.PHYBLKRD, fs.PHYBLKWRT, fs.SINGLEBLKRDS, fs.READTIM, fs.WRITETIM, 
                                          fs.SINGLEBLKRDTIM, fs.AVGIOTIM, fs.LSTIOTIM, fs.MINIOTIM, fs.MAXIORTM, fs.MAXIOWTM
                                          FROM BESTAT_v$datafile df, BESTAT_v$filestat fs, BESTAT_v$tablespace ts
                                          WHERE df.FILE# = fs.FILE# AND df.TS# = ts.TS# ' . " AND df.POINT = $pv{srcptr} AND fs.POINT = $pv{srcptr} AND ts.POINT = $pv{srcptr} " . ' UNION ALL
                                   SELECT fs.FILE#, ts.NAME TABLESPACE_NAME, tf.NAME FILE_NAME, fs.PHYRDS, fs.PHYWRTS, fs.PHYBLKRD, fs.PHYBLKWRT, fs.SINGLEBLKRDS, fs.READTIM,
                                          fs.WRITETIM, fs.SINGLEBLKRDTIM, fs.AVGIOTIM, fs.LSTIOTIM, fs.MINIOTIM, fs.MAXIORTM, fs.MAXIOWTM
                                          FROM BESTAT_v$tempfile tf, BESTAT_v$tempstat fs, BESTAT_v$tablespace ts
                                          WHERE tf.FILE# = fs.FILE# AND tf.TS# = ts.TS#  ' . " AND tf.POINT = $pv{srcptr} AND fs.POINT = $pv{srcptr} AND ts.POINT = $pv{srcptr} ) " . 
                                   $where_ext . " order by $pv{order_field} asc" ; }
my $sth = $dbh->prepare($request) ; $sth->execute() ;
print "<TABLE BORDER=\"1\" CELLSPACING=\"0\" CELLPADDING=\"0\" STYLE=\"width: 100%;\">" ;
print "<TR><TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_files_iostat.cgi?order_field=FILE_ID&srcptr=$pv{srcptr}&tbs_filter=$pv{tbs_filter}\" TARGET=\"cont\">ID файла</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_files_iostat.cgi?order_field=TABLESPACE_NAME&srcptr=$pv{srcptr}&tbs_filter=$pv{tbs_filter}\" TARGET=\"cont\">Имя TC</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_files_iostat.cgi?order_field=NAME&srcptr=$pv{srcptr}&tbs_filter=$pv{tbs_filter}\" TARGET=\"cont\">Имя файла</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_files_iostat.cgi?order_field=PHYRDS&srcptr=$pv{srcptr}&tbs_filter=$pv{tbs_filter}\" TARGET=\"cont\">Физических чтений</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_files_iostat.cgi?order_field=PHYWRTS&srcptr=$pv{srcptr}&tbs_filter=$pv{tbs_filter}\" TARGET=\"cont\">Физических записей</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_files_iostat.cgi?order_field=PHYBLKRD&srcptr=$pv{srcptr}&tbs_filter=$pv{tbs_filter}\" TARGET=\"cont\">Прочитано блоков</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_files_iostat.cgi?order_field=PHYBLKWRT&srcptr=$pv{srcptr}&tbs_filter=$pv{tbs_filter}\" TARGET=\"cont\">Записано блоков</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_files_iostat.cgi?order_field=SINGLEBLKRDS&srcptr=$pv{srcptr}&tbs_filter=$pv{tbs_filter}\" TARGET=\"cont\">Чтений одного блока</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_files_iostat.cgi?order_field=READTIM&srcptr=$pv{srcptr}&tbs_filter=$pv{tbs_filter}\" TARGET=\"cont\">Время чтения (сек/100)</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_files_iostat.cgi?order_field=WRITETIM&srcptr=$pv{srcptr}&tbs_filter=$pv{tbs_filter}\" TARGET=\"cont\">Время записи (сек/100)</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_files_iostat.cgi?order_field=SINGLEBLKRDTIM&srcptr=$pv{srcptr}&tbs_filter=$pv{tbs_filter}\" TARGET=\"cont\">SINGLE BLKRDTIM (сек/100)</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_files_iostat.cgi?order_field=AVGIOTIM&srcptr=$pv{srcptr}&tbs_filter=$pv{tbs_filter}\" TARGET=\"cont\">AVGIOTIM (сек/100)</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_files_iostat.cgi?order_field=LSTIOTIM&srcptr=$pv{srcptr}&tbs_filter=$pv{tbs_filter}\" TARGET=\"cont\">Последний IO (сек/100)</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_files_iostat.cgi?order_field=MINIOTIM&srcptr=$pv{srcptr}&tbs_filter=$pv{tbs_filter}\" TARGET=\"cont\">Минимальное время IO (сек/100)</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_files_iostat.cgi?order_field=MAXIORTM&srcptr=$pv{srcptr}&tbs_filter=$pv{tbs_filter}\" TARGET=\"cont\">Максимальное время чтения (сек/100)</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_files_iostat.cgi?order_field=MAXIOWTM&srcptr=$pv{srcptr}&tbs_filter=$pv{tbs_filter}\" TARGET=\"cont\">Максимальное время записи (сек/100)</A></TD>
           </TR>" ;

$count_all = 0 ;
while (my ($file_n,$tablespace_name,$file_name,$phyrds,$phywrts,$phyblkrd,$phyblkwrt,$singleblkrds,$readtim,$writetim,$singleblkrdtim,$avgiotim,$lstiotim,$miniotim,$maxiortm,$maxiowtm) = $sth->fetchrow_array() ) {
      ...
      printf("<TR><TD CLASS=\"NUMDATA\">%s</TD>
                  ...
                  <TD CLASS=\"NUMDATA\">%s</TD>
                  <TD CLASS=\"NUMDATA\">%s</TD>
                  </TR>\n",$file_n,$tablespace_name,$file_name,&show_razryads($phyrds),&show_razryads($phywrts),&show_razryads($phyblkrd),
                             &show_razryads($phyblkwrt),&show_razryads($singleblkrds),&show_razryads($readtim),&show_razryads($writetim),
                             &show_razryads($singleblkrdtim),&show_razryads($avgiotim),&show_razryads($lstiotim),&show_razryads($miniotim),
                             &show_razryads($maxiortm),&show_razryads($maxiowtm)) ;
      }
print "<TR><TD COLSPAN=\"16\">
