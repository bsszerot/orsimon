...
$pv{order_field} = "LOWKB" ;
&get_forms_param() ;

$ENV{ORACLE_SID} = $current_connector ;

print "Content-Type: text/html\n\n";
&print_html_head("Прогностика PGA") ;

# добавить определение навигационно - оформительского механизма закладок
&print_pga_navigation(8) ;

my $dbh = DBI->connect( $connector_definition{$current_connector}, $connector_credentials{$current_connector}, '');
$request = 'SELECT PGA_TARGET_FOR_ESTIMATE, PGA_TARGET_FACTOR, ADVICE_STATUS, LOW_OPTIMAL_SIZE/1024 lowkb, (HIGH_OPTIMAL_SIZE+1)/1024 highkb, estd_optimal_executions, estd_onepass_executions, estd_multipasses_executions 
                   FROM v$pga_target_advice_histogram WHERE estd_total_executions != 0 ORDER BY PGA_TARGET_FOR_ESTIMATE, ' . $pv{order_field} ;
...
print "<TR><TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_pga_advice.cgi?order_field=PGA_TARGET_FOR_ESTIMATE\" TARGET=\"cont\">PGA TARGET FOR ESTIMATE</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_pga_advice.cgi?order_field=PGA_TARGET_FACTOR\" TARGET=\"cont\">PGA TARGET FACTOR</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_pga_advice.cgi?order_field=ADVICE_STATUS\" TARGET=\"cont\">STATUS</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_pga_advice.cgi?order_field=LOW_KB\" TARGET=\"cont\">Low kb</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_pga_advice.cgi?order_field=HIGH_KB\" TARGET=\"cont\">High kb</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_pga_advice.cgi?order_field=OPTIMAL_EXECUTIONS\" TARGET=\"cont\">OPTIMAL_EXECUTIONS</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_pga_advice.cgi?order_field=ONEPASS_EXECUTIONS\" TARGET=\"cont\">ONEPASS_EXECUTIONS</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_pga_advice.cgi?order_field=MULTIPASSES_EXECUTIONS\" TARGET=\"cont\">MULTIPASSES_EXECUTIONS</A></TD>
           </TR>" ;
$all_optimal = 0 ; $all_onepass = 0 ; $all_multipass = 0 ; $count_rows = 0 ;
while (my ($ptfe,$factor,$status,$low_kb,$high_kb,$optimal,$onepass,$multipass) = $sth->fetchrow_array() ) {
      $all_optimal += $optimal ; $all_onepass += $onepass ; $all_multipass += $multipass ; $count_rows += 1 ;
      printf("<TR><TD CLASS=\"NUMDATA\">%s</TD>
                  ...
                  <TD CLASS=\"NUMDATA\">%s</TD>
                  <TD CLASS=\"NUMDATA\">%s</TD>
              </TR>\n",$ptfe,$factor,$status,&show_razryads($low_kb),&show_razryads($high_kb),&show_razryads($optimal),&show_razryads($onepass),&show_razryads($multipass)) ;
      }
printf("<TR><TD CLASS=\"SZDATA\" COLSPAN=\"2\">Итого строк: %s</TD><TD CLASS=\"NUMDATA\">%s</TD><TD CLASS=\"NUMDATA\">%s</TD><TD CLASS=\"NUMDATA\">%s</TD></TR>\n",$count_rows,&show_razryads($all_optimal),&show_razryads($all_onepass),&show_razryads($all_multipass)) ;
print "</TABLE>" ;
print "</TD></TR></TABLE>" ;
$sth->finish();
$dbh->disconnect();                                        
