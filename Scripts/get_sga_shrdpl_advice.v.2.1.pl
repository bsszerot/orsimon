...
&print_html_head("Прогностика shared pool") ;

# добавить определение навигационно - оформительского механизма закладок
&print_sga_navigation(7) ;

my $dbh = DBI->connect( $connector_definition{$current_connector}, $connector_credentials{$current_connector}, '');
$request = "SELECT SHARED_POOL_SIZE_FOR_ESTIMATE, SHARED_POOL_SIZE_FACTOR, ESTD_LC_SIZE, ESTD_LC_MEMORY_OBJECTS, ESTD_LC_TIME_SAVED, ESTD_LC_TIME_SAVED_FACTOR, ESTD_LC_MEMORY_OBJECT_HITS FROM V\$SHARED_POOL_ADVICE ORDER BY $pv{order_field}" ;
...
print "<TR><TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_sga_shrdpl_advice.cgi?order_field=SHARED_POOL_SIZE_FOR_ESTIMATE\" TARGET=\"cont\">SHARED_POOL_SIZE_FOR_ESTIMATE</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_sga_shrdpl_advice.cgi?order_field=SHARED_POOL_SIZE_FACTOR\" TARGET=\"cont\">SHARED_POOL_SIZE_FACTOR</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_sga_shrdpl_advice.cgi?order_field=ESTD_LC_SIZE\" TARGET=\"cont\">ESTD_LC_SIZE</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_sga_shrdpl_advice.cgi?order_field=ESTD_LC_MEMORY_OBJECTS\" TARGET=\"cont\">ESTD_LC_MEMORY_OBJECTS</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_sga_shrdpl_advice.cgi?order_field=ESTD_LC_TIME_SAVED\" TARGET=\"cont\">ESTD_LC_TIME_SAVED</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_sga_shrdpl_advice.cgi?order_field=ESTD_LC_TIME_SAVED_FACTOR\" TARGET=\"cont\">ESTD_LC_TIME_SAVED_FACTOR</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_sga_shrdpl_advice.cgi?order_field=ESTD_LC_MEMORY_OBJECT_HITS\" TARGET=\"cont\">ESTD_LC_MEMORY_OBJECT_HITS</A></TD>
           </TR>" ;
$count_rows = 0 ;
while (my ($shpl_size_fe,$shpl_size_factor,$lc_size,$lc_mem_obj,$ls_time_saved,$lc_time_saved_factor,$lc_mem_obj_hits) = $sth->fetchrow_array() ) {
      $count_rows += 1 ;
      if ( $pool eq "" ) { $pool = "&nbsp;" ; }
      printf("<TR><TD CLASS=\"NUMDATA\">%s</TD>
                  ...
                  <TD CLASS=\"NUMDATA\">%s</TD>
                  <TD CLASS=\"NUMDATA\">%s</TD>
              </TR>\n",&show_razryads($shpl_size_fe),$shpl_size_factor,&show_razryads($lc_size),&show_razryads($lc_mem_obj),$ls_time_saved,$lc_time_saved_factor,$lc_mem_obj_hits) ;
      }
print "<TR><TD CLASS=\"SZDATA\">Итого строк</TD><TD CLASS=\"NUMDATA\" COLSPAN=\"6\">$count_rows</TD></TR>\n" ;
print "</TABLE>" ;
print "</TD></TR></TABLE>" ;
$sth->finish();
$dbh->disconnect();                                        
