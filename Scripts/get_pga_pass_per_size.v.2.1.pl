...
&print_html_head("Итеративность использования PGA от размера") ;

# добавить определение навигационно - оформительского механизма закладок
&print_pga_navigation(3) ;

my $dbh = DBI->connect( $connector_definition{$current_connector}, $connector_credentials{$current_connector}, '');
$request = 'SELECT LOW_OPTIMAL_SIZE/1024 LOWKB, (HIGH_OPTIMAL_SIZE+1)/1024 HIGHKB, OPTIMAL_EXECUTIONS, ONEPASS_EXECUTIONS, MULTIPASSES_EXECUTIONS 
            FROM V$SQL_WORKAREA_HISTOGRAM WHERE TOTAL_EXECUTIONS != 0 ORDER BY ' . $pv{order_field} ;
...
print "<TR><TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_pga_pass_per_size.cgi?order_field=LOW_KB\" TARGET=\"cont\">Low kb</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_pga_pass_per_size.cgi?order_field=HIGH_KB\" TARGET=\"cont\">High kb</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_pga_pass_per_size.cgi?order_field=OPTIMAL_EXECUTIONS\" TARGET=\"cont\">OPTIMAL_EXECUTIONS</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_pga_pass_per_size.cgi?order_field=ONEPASS_EXECUTIONS\" TARGET=\"cont\">ONEPASS_EXECUTIONS</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_pga_pass_per_size.cgi?order_field=MULTIPASSES_EXECUTIONS\" TARGET=\"cont\">MULTIPASSES_EXECUTIONS</A></TD>
           </TR>" ;
$all_optimal = 0 ; $all_onepass = 0 ; $all_multipass = 0 ; $count_rows = 0 ;
while (my ($low_kb,$high_kb,$optimal,$onepass,$multipass) = $sth->fetchrow_array() ) {
      ...
      printf("<TR><TD CLASS=\"NUMDATA\">%s</TD>
                  ...
                  <TD CLASS=\"NUMDATA\">%s</TD>
                  <TD CLASS=\"NUMDATA\">%s</TD>
              </TR>\n",&show_razryads($low_kb),&show_razryads($high_kb),&show_razryads($optimal),&show_razryads($onepass),&show_razryads($multipass)) ;
      }
