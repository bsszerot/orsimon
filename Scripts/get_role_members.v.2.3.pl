...
&print_html_head("Роли (группы) и их члены") ;
...
print "<FORM ACTION=\"$base_url/cgi/get_role_members.cgi\" METHOD=\"GET\">
       <TABLE BORDER=\"0\" CELLSPACING=\"0\" CELLPADDING=\"0\" STYLE=\"width: 100%;\">" ;
print "<TR><TD>&nbsp;источник: </TD><TD><SELECT NAME=\"source\" STYLE=\"width: 100%;\">" ;
$is_selected = "" ; if ( $pv{source} eq "DBA_ROLE_PRIVS" ) { $is_selected = " SELECTED" ; } print "<OPTION VALUE=\"DBA_ROLE_PRIVS\" $is_selected>Роли грантованные пользователям и группам (ролям), а также никому</OPTION>" ;
$is_selected = "" ; if ( $pv{source} eq "USER_ROLE_PRIVS" ) { $is_selected = " SELECTED" ; } print "<OPTION VALUE=\"USER_ROLE_PRIVS\" $is_selected>Привелегии текущего пользователя</OPTION>" ;
$is_selected = "" ; if ( $pv{source} eq "ROLE_ROLE_PRIVS" ) { $is_selected = " SELECTED" ; } print "<OPTION VALUE=\"ROLE_ROLE_PRIVS\" $is_selected>Привелегии групп (ролей) текущего пользователя</OPTION>" ;
print "</SELECT></TD></TR>" ;
print "<TR><TD>&nbsp;Роль: </TD><TD><INPUT TYPE=\"INPUT\" NAME=\"granted_role\" VALUE=\"$pv{granted_role}\" STYLE=\"width: 100%\"></TD></TR>" ;
print "<TR><TD>&nbsp;Субъект: </TD><TD><INPUT TYPE=\"INPUT\" NAME=\"grantee\" VALUE=\"$pv{grantee}\" STYLE=\"width: 100%\"></TD></TR>" ;
...
$where_ext = "" ; 
if ( $pv{granted_role} ne "" ) { if ( $where_ext eq "" ) { $where_ext .= " WHERE " ; } else { $where_ext .= " AND " ; } $where_ext .= " GRANTED_ROLE = '$pv{granted_role}' " ; }
# актуально только для DBA_ROLE_PRIVS
if ( $pv{grantee} ne "" ) { if ( $where_ext eq "" ) { $where_ext .= " WHERE " ; } else { $where_ext .= " AND " ; }
   if ( $pv{source} eq "DBA_ROLE_PRIVS" ) { $where_ext .= " GRANTEE = '$pv{grantee}' " ; }
   if ( $pv{source} eq "USER_ROLE_PRIVS" ) { $where_ext .= " USERNAME = '$pv{grantee}' " ; }
   if ( $pv{source} eq "ROLE_ROLE_PRIVS" ) { $where_ext .= " ROLE = '$pv{grantee}' " ; }
   }

if ( $pv{source} eq "DBA_ROLE_PRIVS" ) { if ( $pv{order_field} eq "" ) { $pv{order_field} = "" ; } $request = "SELECT rls.ROLE ROLE,rls.PASSWORD_REQUIRED PASSWORD_REQUIRED,rprvs.GRANTEE GRANTEE, rprvs.ADMIN_OPTION ADMIN_OPTION, rprvs.DEFAULT_ROLE DEFAULT_ROLE, '' OS_GRANTED FROM DBA_ROLES rls LEFT OUTER JOIN DBA_ROLE_PRIVS rprvs ON rprvs.GRANTED_ROLE = rls.ROLE $where_ext order by $pv{order_field}" ; }
if ( $pv{source} eq "USER_ROLE_PRIVS" ) { 
   if ( $pv{order_field} eq "GRANTEE" ) { $pv{order_field} = "USERNAME" ; }
   if ( $where_ext eq "" ) { $where_ext .= " WHERE uprvs.GRANTED_ROLE = rls.ROLE " ; } else { $where_ext .= " AND uprvs.GRANTED_ROLE = rls.ROLE " ; }
   $request = "SELECT rls.ROLE ROLE,rls.PASSWORD_REQUIRED PASSWORD_REQUIRED, uprvs.USERNAME GRANTEE, uprvs.ADMIN_OPTION ADMIN_OPTION, uprvs.DEFAULT_ROLE DEFAULT_ROLE, uprvs.OS_GRANTED OS_GRANTED FROM DBA_ROLES rls, USER_ROLE_PRIVS uprvs $where_ext order by $pv{order_field}" ; }
if ( $pv{source} eq "ROLE_ROLE_PRIVS" ) { 
   if ( $pv{order_field} eq "GRANTEE" ) { $pv{order_field} = "ROLE" ; } 
   if ( $where_ext eq "" ) { $where_ext .= " WHERE rprvs.GRANTED_ROLE = rls.ROLE " ; } else { $where_ext .= " AND rprvs.GRANTED_ROLE = rls.ROLE " ; }
   $request = "SELECT rls.ROLE ROLE,rls.PASSWORD_REQUIRED PASSWORD_REQUIRED, rprvs.ROLE GRANTEE, rprvs.ADMIN_OPTION ADMIN_OPTION, '' DEFAULT_ROLE, '' OS_GRANTED FROM DBA_ROLES rls, ROLE_ROLE_PRIVS rprvs $where_ext order by $pv{order_field}" ; }
# старый вариант не отражал ролей которые никому не выданы
# if ( $pv{source} eq "DBA_ROLE_PRIVS" ) { if ( $pv{order_field} eq "" ) { $pv{order_field} = "" ; } $request = "SELECT GRANTEE, GRANTED_ROLE, ADMIN_OPTION, DEFAULT_ROLE FROM $pv{source} $where_ext order by $pv{order_field}" ; }
# if ( $pv{source} eq "USER_ROLE_PRIVS" ) { if ( $pv{order_field} eq "GRANTEE" ) { $pv{order_field} = "USERNAME" ; } $request = "SELECT USERNAME, GRANTED_ROLE, ADMIN_OPTION, DEFAULT_ROLE, OS_GRANTED FROM $pv{source} $where_ext order by $pv{order_field}" ; }
# if ( $pv{source} eq "ROLE_ROLE_PRIVS" ) { if ( $pv{order_field} eq "GRANTEE" ) { $pv{order_field} = "ROLE" ; } $request = "SELECT ROLE, GRANTED_ROLE, ADMIN_OPTION FROM $pv{source} $where_ext order by $pv{order_field}" ; }
...
sub print_head_ancor($$) { $a_order_field = $_[0] ; $a_title_field = $_[1] ; $order_symbol = "&nbsp;" ;
    ...
    print "<TABLE BORDER=\"0\" CELLSPACING=\"0\" CELLPADDING=\"0\" ALIGN=\"center\"><TR><TD>&nbsp;</TD>
           <TD STYLE=\"text-align: center;\"><A HREF=\"$base_url/cgi/get_role_members.cgi?order_field=$a_order_field&is_view_result=yes&source=$pv{source}&granted_role=$pv{granted_role}&grantee=$pv{grantee}\" TARGET=\"cont\">$a_title_field</A></TD>
...
print "<TR><TD CLASS=\"HEAD\">"; &print_head_ancor("ROLE", "Роль") ;
print "    <TD CLASS=\"HEAD\">"; &print_head_ancor("GRANTEE", "Кому<BR>грантована") ;
print "    <TD CLASS=\"HEAD\">"; &print_head_ancor("ADMIN_OPTION", "ADMIN<BR>OPTION") ;
print "    <TD CLASS=\"HEAD\">"; &print_head_ancor("DEFAULT_ROLE", "Роль по<BR> умолчанию") ;
print "    <TD CLASS=\"HEAD\">"; &print_head_ancor("PASSWORD_REQUIRED", "PASSWORD<BR>REQUIRED") ;
print "    <TD CLASS=\"HEAD\">"; &print_head_ancor("OS_GRANTED", "OS<BR>GRANTED") ;
print "</TR>" ;

$cnt_rows = 0 ;
while (my ($role,$password_required,$grantee,$admin_option,$default_role,$os_granted) = $sth->fetchrow_array() ) { $cnt_rows++ ;
      if ( $default_role eq "" ) { $default_role = "&nbsp;" ; } if ( $os_granted eq "" ) { $os_granted = "&nbsp;" ; }
      if ( $grantee eq "" ) { $grantee = "не&nbsp;грантована&nbsp;никому" ; } if ( $admin_option eq "" ) { $admin_option = "&nbsp;" ; }
# фильтры принудительно сбрасывают прежние значения ибо смысла в накоплении здесь нет
      printf("<TR><TD CLASS=\"SZDATA\"><A HREF=\"$base_url/cgi/get_role_members.cgi?order_field=$pv{order_field}&is_view_result=yes&source=$pv{source}&granted_role=%s&grantee=\" TARGET=\"cont\">F</A>&nbsp;%s</TD>
                  <TD CLASS=\"SZDATA\"><A HREF=\"$base_url/cgi/get_role_members.cgi?order_field=$pv{order_field}&is_view_result=yes&source=$pv{source}&granted_role=&grantee=%s\" TARGET=\"cont\">F</A>&nbsp;%s</TD>
                  <TD CLASS=\"SZDATA\">%s</TD>
                  <TD CLASS=\"SZDATA\">%s</TD>
                  <TD CLASS=\"SZDATA\">%s</TD>
                  <TD CLASS=\"SZDATA\">%s</TD>
              </TR>\n",$role,$role,$grantee,$grantee,$admin_option,$default_role,$password_required,$os_granted) ;
      }
printf ("<TR><TD CLASS=\"SZDATA\">Итого</TD><TD CLASS=\"NUMDATA\" COLSPAN=\"5\">%s</TD></TR>\n", $cnt_rows) ;
print "</TABLE>" ;
