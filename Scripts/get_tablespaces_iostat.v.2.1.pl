...
&print_html_head("Статистика ввода / вывода табличных пространств") ;
...
&print_tablespaces_navigation(3) ;
...
print "<FORM ACTION=\"$base_url/cgi/get_tablespaces_iostat.cgi\" METHOD=\"GET\">
...
$request = 'select TABLESPACE_NAME, SUM(PHYRDS) PHYRDS, SUM(PHYWRTS) PHYWRTS, SUM(PHYBLKRD) PHYBLKRD, SUM(PHYBLKWRT) PHYBLKWRT, 
                   SUM(SINGLEBLKRDS) SINGLEBLKRDS, SUM(READTIM) READTIM, SUM(WRITETIM) WRITETIM, SUM(SINGLEBLKRDTIM) SINGLEBLKRDTIM, 
                   AVG(AVGIOTIM) AVGIOTIM, MAX(LSTIOTIM) LSTIOTIM, MIN(MINIOTIM) MINIOTIM, MAX(MAXIORTM) MAXIORTM, MAX(MAXIOWTM) MAXIOWTM
                   from ( SELECT ts.NAME TABLESPACE_NAME, fs.PHYRDS, fs.PHYWRTS, fs.PHYBLKRD, fs.PHYBLKWRT, fs.SINGLEBLKRDS, 
                            ...
                            UNION ALL
                            SELECT ts.NAME TABLESPACE_NAME, fs.PHYRDS, fs.PHYWRTS, fs.PHYBLKRD, fs.PHYBLKWRT, fs.SINGLEBLKRDS, 
                                   fs.READTIM, fs.WRITETIM, fs.SINGLEBLKRDTIM, fs.AVGIOTIM, fs.LSTIOTIM, fs.MINIOTIM, fs.MAXIORTM, fs.MAXIOWTM
                                   FROM v$tempfile tf, v$filestat fs, v$tablespace ts
                                   WHERE tf.FILE# = fs.FILE# AND tf.TS# = ts.TS# )  ' . $where_ext . " GROUP BY TABLESPACE_NAME order by $pv{order_field} asc" ; }
else { $request = 'select TABLESPACE_NAME, SUM(PHYRDS) PHYRDS, SUM(PHYWRTS) PHYWRTS, SUM(PHYBLKRD) PHYBLKRD, SUM(PHYBLKWRT) PHYBLKWRT, 
                   SUM(SINGLEBLKRDS) SINGLEBLKRDS, SUM(READTIM) READTIM, SUM(WRITETIM) WRITETIM, SUM(SINGLEBLKRDTIM) SINGLEBLKRDTIM, 
                   AVG(AVGIOTIM) AVGIOTIM, MAX(LSTIOTIM) LSTIOTIM, MIN(MINIOTIM) MINIOTIM, MAX(MAXIORTM) MAXIORTM, MAX(MAXIOWTM) MAXIOWTM
                   from ( SELECT ts.NAME TABLESPACE_NAME, fs.PHYRDS, fs.PHYWRTS, fs.PHYBLKRD, fs.PHYBLKWRT, fs.SINGLEBLKRDS, 
                                   fs.READTIM, fs.WRITETIM, fs.SINGLEBLKRDTIM, fs.AVGIOTIM, fs.LSTIOTIM, fs.MINIOTIM, fs.MAXIORTM, fs.MAXIOWTM
                                   FROM BESTAT_v$datafile df, BESTAT_v$filestat fs, BESTAT_v$tablespace ts
                                   WHERE df.FILE# = fs.FILE# AND df.TS# = ts.TS# ' . " AND df.POINT = $pv{srcptr} AND fs.POINT = $pv{srcptr} AND ts.POINT = $pv{srcptr} " . '
                            UNION ALL
                            SELECT ts.NAME TABLESPACE_NAME, fs.PHYRDS, fs.PHYWRTS, fs.PHYBLKRD, fs.PHYBLKWRT, fs.SINGLEBLKRDS, 
                                   fs.READTIM, fs.WRITETIM, fs.SINGLEBLKRDTIM, fs.AVGIOTIM, fs.LSTIOTIM, fs.MINIOTIM, fs.MAXIORTM, fs.MAXIOWTM
                                   FROM BESTAT_v$tempfile tf, BESTAT_v$filestat fs, BESTAT_v$tablespace ts
                                   WHERE tf.FILE# = fs.FILE# AND tf.TS# = ts.TS# ' . " AND tf.POINT = $pv{srcptr} AND fs.POINT = $pv{srcptr} AND ts.POINT = $pv{srcptr} " . ')  ' . $where_ext . " GROUP BY TABLESPACE_NAME order by $pv{order_field} asc" ; }
my $sth = $dbh->prepare($request) ; $sth->execute() ;
print "<TABLE BORDER=\"1\" CELLSPACING=\"0\" CELLPADDING=\"0\" STYLE=\"width: 100%;\">" ;
print "<TR><TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_tablespaces_iostat.cgi?order_field=TABLESPACE_NAME&srcptr=$pv{srcptr}&tbs_filter=$pv{tbs_filter}\" TARGET=\"cont\">Имя TC</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_tablespaces_iostat.cgi?order_field=PHYRDS&srcptr=$pv{srcptr}&tbs_filter=$pv{tbs_filter}\" TARGET=\"cont\">Физических чтений</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_tablespaces_iostat.cgi?order_field=PHYWRTS&srcptr=$pv{srcptr}&tbs_filter=$pv{tbs_filter}\" TARGET=\"cont\">Физических записей</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_tablespaces_iostat.cgi?order_field=PHYBLKRD&srcptr=$pv{srcptr}&tbs_filter=$pv{tbs_filter}\" TARGET=\"cont\">Прочитано блоков</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_tablespaces_iostat.cgi?order_field=PHYBLKWRT&srcptr=$pv{srcptr}&tbs_filter=$pv{tbs_filter}\" TARGET=\"cont\">Записано блоков</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_tablespaces_iostat.cgi?order_field=SINGLEBLKRDS&srcptr=$pv{srcptr}&tbs_filter=$pv{tbs_filter}\" TARGET=\"cont\">Чтений одного блока</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_tablespaces_iostat.cgi?order_field=READTIM&srcptr=$pv{srcptr}&tbs_filter=$pv{tbs_filter}\" TARGET=\"cont\">Время чтения (сек/100)</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_tablespaces_iostat.cgi?order_field=WRITETIM&srcptr=$pv{srcptr}&tbs_filter=$pv{tbs_filter}\" TARGET=\"cont\">Время записи (сек/100)</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_tablespaces_iostat.cgi?order_field=SINGLEBLKRDTIM&srcptr=$pv{srcptr}&tbs_filter=$pv{tbs_filter}\" TARGET=\"cont\">SINGLE BLKRDTIM (сек/100)</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_tablespaces_iostat.cgi?order_field=AVGIOTIM&srcptr=$pv{srcptr}&tbs_filter=$pv{tbs_filter}\" TARGET=\"cont\">AVGIOTIM (сек/100)</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_tablespaces_iostat.cgi?order_field=LSTIOTIM&srcptr=$pv{srcptr}&tbs_filter=$pv{tbs_filter}\" TARGET=\"cont\">Последний min IO (сек/100)</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_tablespaces_iostat.cgi?order_field=MINIOTIM&srcptr=$pv{srcptr}&tbs_filter=$pv{tbs_filter}\" TARGET=\"cont\">Минимальное время IO (сек/100)</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_tablespaces_iostat.cgi?order_field=MAXIORTM&srcptr=$pv{srcptr}&tbs_filter=$pv{tbs_filter}\" TARGET=\"cont\">Максимальное время чтения (сек/100)</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_tablespaces_iostat.cgi?order_field=MAXIOWTM&srcptr=$pv{srcptr}&tbs_filter=$pv{tbs_filter}\" TARGET=\"cont\">Максимальное время записи (сек/100)</A></TD>
           </TR>" ;

$count_all = 0 ;
while (my ($tablespace_name,$phyrds,$phywrts,$phyblkrd,$phyblkwrt,$singleblkrds,$readtim,$writetim,$singleblkrdtim,$avgiotim,$lstiotim,$miniotim,$maxiortm,$maxiowtm) = $sth->fetchrow_array() ) {
      ...
      if ( $class eq "" ) { $class = "&nbsp;" ; }
      if ( $value eq "" ) { $value = "&nbsp;" ; }
      printf("<TR><TD CLASS=\"SZDATA\"><A HREF=\"$base_url/cgi/get_files_iostat.cgi?order_field=FILE_NAME&tbs_filter=%s&srcptr=$pv{srcptr}\" TARGET=\"cont\">%s</A></TD>
                  ...
                  <TD CLASS=\"NUMDATA\">%s</TD>
                  <TD CLASS=\"NUMDATA\">%s</TD>
                  </TR>\n",$tablespace_name,$tablespace_name,&show_razryads($phyrds),&show_razryads($phywrts),&show_razryads($phyblkrd),
...
print "<TR><TD>Итого</TD><TD COLSPAN=\"13\" STYLE=\"text-align: right;\">$count_all</TD></TR>" ;
print "</TABLE>" ;
