...
$request = 'SELECT /*+ ORDERED */ l.sid SID, s.serial#, r.NAME, TRUNC(l.ID1/POWER(2,16)) ROLLID, BITAND(l.ID1,POWER(2,16)-1)+0 SLOT, l.ID2 NUMCHANGE, 
            decode(l.LMODE,0,\'none\',1,\'null (NULL)\',2,\'row-S (SS)\',3,\'row-X (SX)\',4,\'share (X)\',5,\'S/Row-X (SSX)\',6,\'executive (X)\',l.LMODE), 
            decode(l.REQUEST,0,\'none\',1,\'null (NULL)\',2,\'row-S (SS)\',3,\'row-X (SX)\',4,\'share (X)\',5,\'S/Row-X (SSX)\',6,\'executive (X)\',l.REQUEST), l.TYPE, decode(block,0,\'нет\',1,\'блокирует\',2,\'глобально\',to_char(block)) BLOCKING_OTHERS 
            FROM V$LOCK l, V$SESSION s, V$ROLLNAME r WHERE l.SID = s.SID (+) AND TRUNC(l.ID1/65536) = r.USN (+) AND l.TYPE = \'TX\' ORDER BY ' . $order_field ;

my $sth = $dbh->prepare($request) ;
$sth->execute();
print "Content-Type: text/html\n\n"; 
&print_html_head("Текущие TX DML блокировки") ;
# добавить определение навигационно - оформительского механизма закладок
&print_locks_navigation(3) ;
...
print "<TR><TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_dml_tx_locks.cgi?order_field=l.SID\" TARGET=\"cont\">Сессия</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_dml_tx_locks.cgi?order_field=o.OBJECT_NAME\" TARGET=\"cont\">Имя RBS сегмента</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_dml_tx_locks.cgi?order_field=o.OBJECT_NAME\" TARGET=\"cont\">USN RBS сегмента</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_dml_tx_locks.cgi?order_field=o.OBJECT_NAME\" TARGET=\"cont\">SLOT RBS сегмента</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_dml_tx_locks.cgi?order_field=o.OBJECT_NAME\" TARGET=\"cont\">CHANGENUM RBS сегмента</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_dml_tx_locks.cgi?order_field=l.LMODE\" TARGET=\"cont\">Режим блокировки</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_dml_tx_locks.cgi?order_field=l.REQUEST\" TARGET=\"cont\">Режим ожидания</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_dml_tx_locks.cgi?order_field=l.TYPE\" TARGET=\"cont\">Тип блокировки</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_dml_tx_locks.cgi?order_field=l.TYPE\" TARGET=\"cont\">Блокирует ли других</A></TD>
           </TR>" ;
$is_not_null_answer = "no" ; $sum_records = 0 ;
while (my ($sid,$serial,$rbsname,$rbsusn,$rbsslot,$rbschangenum,$lmode,$request,$type,$block) = $sth->fetchrow_array() ) { $sum_records += 1 ;
      if ( $id1 eq "" ) { $id1 = "&nbsp;" ; } if ( $id2 eq "" ) { $id2 = "&nbsp;" ; }
      if ( $lmode eq "" ) { $lmode = "&nbsp;" ; } if ( $request eq "" ) { $request = "&nbsp;" ; } if ( $type eq "" ) { $type = "&nbsp;" ; }
      printf("<TR><TD CLASS=\"SZDATA\"><A HREF=\"$base_url/cgi/get_session_info.cgi?sid=%s&serial=%s\" TARGET=\"cont\">%s,%s</A></TD>
                  ...
                  <TD CLASS=\"CENTERDATA\">%s</TD>
                  <TD CLASS=\"CENTERDATA\">%s</TD>
              </TR>\n",$sid,$serial,$sid,$serial,$rbsname,$rbsusn,$rbsslot,$rbschangenum,$lmode,$request,$type,$block) ;
      $is_not_null_answer = "yes" ;
      }
if ( $is_not_null_answer eq "no" ) { print "<TR><TD CLASS=\"SZDATA\" COLSPAN=\"9\">Блокировки TX DML</TD></TR>" ;}
else { print "<TR><TD CLASS=\"SZDATA\">Итого</TD><TD COLSPAN=\"8\" CLASS=\"NUMDATA\">$sum_records</TD></TR>" ; }
print "</TABLE>" ;
print "</TD></TR></TABLE>" ;
$sth->finish();
$dbh->disconnect();                                        
