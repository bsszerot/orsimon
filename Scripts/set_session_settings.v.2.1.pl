...
if ( $ENV{REQUEST_METHOD} eq "GET" && $ENV{QUERY_STRING} =~ /.*current_connector=([^&]*).*/) { $current_connector = $1 ;
    $current_connector =~ s/[\r\n]+//g ; $current_connector =~ s/%(..)/pack("c",hex($1))/ge ;
    print "Content-Type: text/html\n"; 
    print "Set-cookie: current_connector=$current_connector; path=/; domain=$ENV{HTTP_HOST}\n\n";
    &print_html_head("Установка значений текущей сессии") ; 
    print "Отправлен запрос на установку новых параметров сессии. Для корректной работы ваш браузер должен принимать Cookies\n<BR>&nbsp;" ;
    }
else {
   print "Content-Type: text/html\n\n"; 
   &print_html_head("Установка значений текущей сессии") ;
   print "<FORM ACTION=\"$base_url/cgi/set_session_settings.cgi\" METHOD=\"GET\">
          <TABLE CELLPADDING=\"0\" CELLSPACING=\"3pt\" BORDER=\"0\" STYLE=\"width: 100%;\">
            <TR><TD>Текущий коннектор:</TD><TD STYLE=\"width: 70%\">
            <SELECT NAME=\"current_connector\" STYLE=\"width: 100%\">
            <OPTION VALUE=\"none\">Не выбран</OPTION>" ;
   @dbconn__nosort_keys = keys %connector_definition ; @dbconn__sort_keys = sort @dbconn__nosort_keys ;
   foreach $i (@dbconn__sort_keys) { $is_selected = "" ; if ( $current_connector eq "$i" ) { $is_selected = "SELECTED" ; } print "<OPTION VALUE=\"$i\" $is_selected>$i - $connector_description{$i}</OPTION>" ; }
   print "</SELECT></TD></TR>
          <TR><TD COLSPAN=\"2\" STYLE=\"text-align: center;\"><BR>&nbsp;<INPUT TYPE=\"submit\" VALUE=\"Установить новые значения\"></TD></TR>
          </TABLE></FORM>" ;
   }

print "<HR COLOR=\"navy\"><P STYLE=\"font-size 8pt; text-align: center;\">(C) 2000-2010 Sergey S. Belonin. All rights reserved</P>" ;
print "</BODY></HTML>" ;
