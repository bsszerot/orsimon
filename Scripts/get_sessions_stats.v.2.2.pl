...
&print_html_head("Статистики текущих сессий") ;
...
&print_sessions_stats_navigation(1) ;
...
print "<FORM ACTION=\"$base_url/cgi/get_sessions_stats.cgi\" METHOD=\"GET\">
...
print "<TR><TD><INPUT TYPE=\"radio\" NAME=\"incltype\" VALUE=\"single\" $is_incltype_single_checked>&nbsp;Включить&nbsp;опцию: </TD>
           <TD STYLE=\"width: 75%\">
               <SELECT NAME=\"snglstatsinc\" STYLE=\"width: 100%\" VALUE=\"$pv{statsinc}\">
...
print "</SELECT>
       ...
       <TR><TD><INPUT TYPE=\"radio\" NAME=\"incltype\" VALUE=\"group\" $is_incltype_group_checked>&nbsp;Включить&nbsp;группу: </TD>
           <TD STYLE=\"width: 75%\">
               <SELECT NAME=\"statsinc\" STYLE=\"width: 100%\" VALUE=\"$pv{statsinc}\">
...
$ext_where_term = "" ; 
if ( $pv{incltype} eq "group" && $pv{statsinc} ne "all" && $pv{statsinc} ne "" ) { $ext_where_term = " AND sn.NAME IN ($statistics_group_members{$pv{statsinc}}) " ; }
if ( $pv{incltype} eq "single" && $pv{snglstatsinc} ne "all" && $pv{snglstatsinc} ne "" ) { $ext_where_term = ' AND ss.STATISTIC# = ' . "'$pv{snglstatsinc}' " ; }
if ( $pv{excltype} eq "group" && $pv{statsexcl} ne "none" && $pv{statsexcl} ne "" ) { 
   if ( $ext_where_term eq "" ) { $ext_where_term .= " AND sn.NAME NOT IN ($statistics_group_members{$pv{statsexcl}}) " ; } 
   else { $ext_where_term .= " AND sn.NAME NOT IN ($statistics_group_members{$pv{statsexcl}}) " ; } }
if ( $pv{excltype} eq "single" && $pv{snglstatsexcl} ne "none" && $pv{snglstatsexcl} ne "" ) { 
   if ( $ext_where_term eq "" ) { $ext_where_term .= ' AND sn.STATISTIC# <> ' . "$pv{snglstatsexcl} " ; }
   else { $ext_where_term .= ' AND ss.STATISTIC# <> ' . "$pv{snglstatsexcl} " ; } }
if ( $pv{fl_sid} ne "" ) { $ext_where_term .= ' AND s.SID = ' . "$pv{fl_sid} " ; }

if ( $pv{srcptr} eq "0" || $pv{srcptr} eq "" ) { $first_ord_field = "s.SID" ; 
   $request = 'select s.SID, s.SERIAL# SERIAL, ss.STATISTIC# id_stat,sn.NAME,sn.CLASS,ss.VALUE from v$sesstat ss, v$statname sn, v$session s WHERE ss.STATISTIC# = sn.STATISTIC# AND ss.SID = s.SID ' . $ext_where_term . " order by $pv{order_field} asc" ; }
else { $request = 'select s.SID, s.SERIAL# SERIAL, ss.STATISTIC# id_stat,sn.NAME,sn.CLASS,ss.VALUE from BESTAT_SESSION_STATS ss, BESTAT_SESSIONS s, v$statname sn WHERE ss.POINT = s.POINT AND ss.SID = s.SID AND ss.STATISTIC# = sn.STATISTIC# AND ' . " ss.POINT = $pv{srcptr} " . $ext_where_term . " order by $pv{order_field} asc" ; }
...
sub print_head_ancor($$) { $a_order_field = $_[0] ; $a_title_field = $_[1] ;
    print "<A HREF=\"$base_url/cgi/get_sessions_stats.cgi?order_field=$a_order_field&incltype=$pv{incltype}&snglstatsinc=$pv{snglstatsinc}&statsinc=$pv{statsinc}&excltype=$pv{excltype}&snglstatsexcl=$pv{snglstatsexcl}&statsexcl=$pv{statsexcl}&srcptr=$pv{srcptr}&fl_sid=$pv{fl_sid}\" TARGET=\"cont\">$a_title_field</A>" ;
    }

print "<TR><TD CLASS=\"HEAD\">" ; &print_head_ancor("$first_ord_field","ID сессии (SID,SERIAL#)") ;
print "</TD><TD CLASS=\"HEAD\">" ; &print_head_ancor("ID_STAT","ID статистики") ;
print "</TD><TD CLASS=\"HEAD\">" ; &print_head_ancor("sn.NAME","Статистика") ;
print "</TD><TD CLASS=\"HEAD\">" ; &print_head_ancor("sn.CLASS","Класс статистики") ;
print "</TD><TD CLASS=\"HEAD\">" ; &print_head_ancor("ss.VALUE","Значение счетчика") ;
print "</TR>" ;

$system_stat_count_all = 0 ;
while (my ($sid,$serial,$stat_id,$name,$class,$value) = $sth->fetchrow_array() ) {
...
# --- вычислить символьное отображение классов статистики путем перебора вариантов вычисляются попадения и инициализируется эелмент хэша
      ...
      printf("<TR><TD CLASS=\"SZDATA\"><A HREF=\"$base_url/cgi/get_session_info.cgi?sid=%d&serial=%d\" TARGET=\"cont\">%d,&nbsp;%d</A></TD>
                  ...
                  </TR>\n",$sid,$serial,$sid,$serial,$stat_id,$name,$stat_classes{$class},&show_razryads($value)) ;
      }
