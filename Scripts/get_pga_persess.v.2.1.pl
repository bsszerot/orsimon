...
&print_html_head("Утилизация PGA сессиями") ;

# добавить определение навигационно - оформительского механизма закладок
&print_pga_navigation(2) ;

my $dbh = DBI->connect( $connector_definition{$current_connector}, $connector_credentials{$current_connector}, '');
$request = 'SELECT s.SID,s.SERIAL#,s.PROCESS,p.SPID,s.USERNAME,s.OSUSER,s.TERMINAL,s.PROGRAM,p.PGA_USED_MEM,p.PGA_ALLOC_MEM,p.PGA_MAX_MEM FROM V$SESSION s,V$PROCESS p WHERE s.PADDR = p.ADDR ORDER BY ' . $pv{order_field} ;
...
print "<TR><TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_pga_persess.cgi?order_field=SID\" TARGET=\"cont\">SID, serial</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_pga_persess.cgi?order_field=PROCESS\" TARGET=\"cont\">pid server proc</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_pga_persess.cgi?order_field=SPID\" TARGET=\"cont\">pid user proc</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_pga_persess.cgi?order_field=USERNAME\" TARGET=\"cont\">USERNAME</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_pga_persess.cgi?order_field=OSUSER\" TARGET=\"cont\">OSUSER</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_pga_persess.cgi?order_field=TERMINAL\" TARGET=\"cont\">Терминал</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_pga_persess.cgi?order_field=PROGRAM\" TARGET=\"cont\">PROGRAM</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_pga_persess.cgi?order_field=PGA_USED_MEM\" TARGET=\"cont\">PGA используется</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_pga_persess.cgi?order_field=PGA_ALLOC_MEM\" TARGET=\"cont\">PGA выделено</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_pga_persess.cgi?order_field=PGA_MAX_MEM\" TARGET=\"cont\">PGA выделялось максимально</A></TD>
           </TR>" ;
$pga_all_use = 0 ; $pga_all_alloc = 0 ; $count_rows = 0 ;
while (my ($sid,$serial_n,$proc_pid,$sess_pid,$username,$osuser,$terminal,$program,$pga_used_mem,$pga_alloc_mem,$pga_max_mem) = $sth->fetchrow_array() ) {
      $pga_all_use += $pga_used_mem ; $pga_all_alloc += $pga_alloc_mem ; $count_rows += 1 ;
      if ( $username eq "" ) { $username = "&nbsp;" ; } if ( $osuser eq "" ) { $osuser = "&nbsp;" ; } if ( $terminal eq "" ) { $terminal = "&nbsp;" ; }
      if ( $proc_pid eq "" ) { $proc_pid = "&nbsp;" ; } if ( $sess_pid eq "" ) { $sess_pid = "&nbsp;" ; }
      if ( $program eq "" ) { $program = "&nbsp;" ; } $program =~ s/\s/&nbsp;/g ; 
      printf("<TR><TD CLASS=\"SZDATA\"><A HREF=\"$base_url/cgi/get_session_info.cgi?sid=%d&serial=%d\" TARGET=\"cont\">%s,%s</A></TD>
                  ...
                  <TD CLASS=\"NUMDATA\">%s</TD>
                  <TD CLASS=\"NUMDATA\">%s</TD>
              </TR>\n",$sid,$serial_n,$sid,$serial_n,$proc_pid,$sess_pid,$username,$osuser,$terminal,$program,&show_razryads($pga_used_mem),&show_razryads($pga_alloc_mem),&show_razryads($pga_max_mem)) ;
      }
printf("<TR><TD CLASS=\"SZDATA\" COLSPAN=\"7\">Итого строк: %s</TD><TD CLASS=\"NUMDATA\">%s</TD><TD CLASS=\"NUMDATA\">%s</TD><TD>&nbsp;</TD></TR>\n",&show_razryads($count_rows),&show_razryads($pga_all_use),&show_razryads($pga_all_alloc)) ;
print "</TABLE>" ;
print "</TD></TR></TABLE>" ;
$sth->finish();
$dbh->disconnect();                                        
