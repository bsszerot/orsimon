...
$pv{order_field} = "PROFILE" ;
&get_forms_param() ;

$ENV{ORACLE_SID} = $current_connector ;

print "Content-Type: text/html\n\n";
&print_html_head("Количество work area по итеративности") ;

# добавить определение навигационно - оформительского механизма закладок
&print_pga_navigation(9) ;

my $dbh = DBI->connect( $connector_definition{$current_connector}, $connector_credentials{$current_connector}, '');
$request = "SELECT name profile, cnt, decode(total, 0, 0, round(cnt*100/total)) percentage
                   FROM (SELECT name, value cnt, (sum(value) over ()) total
                                FROM V\$SYSSTAT WHERE name like 'workarea exec%') ORDER BY $pv{order_field}" ;
...
print "<TR><TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_pga_advice.cgi?order_field=PROFILE\" TARGET=\"cont\">Тип статистики</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_pga_advice.cgi?order_field=CNT\" TARGET=\"cont\">Единиц</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_pga_advice.cgi?order_field=percentage\" TARGET=\"cont\">%</A></TD>
           </TR>" ;

$count_rows = 0 ;
while (my ($profile,$cnt,$percent) = $sth->fetchrow_array() ) {
      $count_rows += 1 ;
      printf("<TR><TD CLASS=\"SZDATA\">%s</TD>
                  <TD CLASS=\"NUMDATA\">%s</TD>
                  <TD CLASS=\"NUMDATA\">%s</TD>
              </TR>\n",$profile,&show_razryads($cnt),$percent) ;
      }
