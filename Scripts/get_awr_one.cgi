...
print "<TR><TD><DIV STYLE=\"width: 900pt; x-overflow: auto;\">" ;

# вывести форму выбора дополнительного фильтра отчета 

$start_one_snap = $pv{stop_snap_id} -1  ;
#$request = "select * from (select dbms_workload_repository.awr_report_html($pv{dbid},$pv{instance_num},$pv{start_snap_id},$stop) from dual) a " ;
$request = "select dbms_workload_repository.awr_report_html($pv{dbid},$pv{instance_num},$start_one_snap,$pv{stop_snap_id}) from dual " ;
...
#print $request ;
   ...
   while (my $sz_result = $sth->fetchrow() ) { foreach(@$sz_result) { print @$_; } }
   $sth->finish() ;
   $dbh->disconnect() ;
...
# это конец общей контейнерной таблицы
   print "</TD></TR></TABLE>" ;
  if ( $is_view_request eq "yes" ) { print "<BR><HR COLOR=\"navy\"><P STYLE=\"font-size 10pt; text-align: left; text-decoration: none;\">Справочно - запрос к базе:</P>\n$request\n" ; }
# это конец общей контейнерной таблицы
print "</TD></TR></TABLE>" ;
print "<BR>&nbsp;<HR COLOR=\"navy\"><P STYLE=\"font-size 8pt; text-align: center; text-decoration: none;\">(C) 2000-2010 Sergey S. Belonin. All rights reserved</P>" ;
print "</BODY></HTML>" ;
