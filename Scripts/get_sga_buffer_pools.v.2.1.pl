...
&print_html_head("Пулы буферов") ;

# добавить определение навигационно - оформительского механизма закладок
&print_sga_navigation(2) ;

my $dbh = DBI->connect( $connector_definition{$current_connector}, $connector_credentials{$current_connector}, '');
$request = "SELECT ID, NAME, BLOCK_SIZE, RESIZE_STATE, CURRENT_SIZE, BUFFERS, TARGET_SIZE, TARGET_BUFFERS, PREV_SIZE, PREV_BUFFERS, LO_BNUM, HI_BNUM, 
                   LO_SETID, HI_SETID, SET_COUNT FROM V\$BUFFER_POOL ORDER BY $pv{order_field}" ;
...
print "<TR><TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_sga_buffer_pool.cgi?order_field=ID\" TARGET=\"cont\">ID</A></TD>
           ...
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_sga_buffer_pool.cgi?order_field=BLOCK_SIZE\" TARGET=\"cont\">Размер блока, байт</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_sga_buffer_pool.cgi?order_field=RESIZE_STAT\" TARGET=\"cont\">RESIZE статус</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_sga_buffer_pool.cgi?order_field=CURRENT_SIZE\" TARGET=\"cont\">Текущий размер, Мбайт</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_sga_buffer_pool.cgi?order_field=BUFFERS\" TARGET=\"cont\">Количество буферов</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_sga_buffer_pool.cgi?order_field=TARGET_SIZE\" TARGET=\"cont\">TARGET SIZE</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_sga_buffer_pool.cgi?order_field=TARGET_BUFFERS\" TARGET=\"cont\">TARGET BUFFERS</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_sga_buffer_pool.cgi?order_field=PREV_SIZE\" TARGET=\"cont\">Прежний размер, Мбайт</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_sga_buffer_pool.cgi?order_field=PREV_BUFFERS\" TARGET=\"cont\">Прежнее количество буферов</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_sga_buffer_pool.cgi?order_field=LO_BNUM\" TARGET=\"cont\">LOW BNUM</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_sga_buffer_pool.cgi?order_field=HI_BNUM\" TARGET=\"cont\">HIGH BNUM</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_sga_buffer_pool.cgi?order_field=LO_SETID\" TARGET=\"cont\">LOW SETID</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_sga_buffer_pool.cgi?order_field=HI_SETID\" TARGET=\"cont\">HIGH SETID</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_sga_buffer_pool.cgi?order_field=SET_COUNT\" TARGET=\"cont\">SET COUNT</A></TD>
           </TR>" ;
$count_rows = 0 ;
while (my ($id, $name, $block_size, $resize_state, $current_size, $buffers, $target_size, $target_buffers, $prev_size, $prev_buffers, $lo_bnum, $hi_bnum, 
          $lo_setid,$hi_setid,$set_count) = $sth->fetchrow_array() ) { $count_rows ++ ;
      if ( $pool eq "" ) { $pool = "&nbsp;" ; }
      printf("<TR><TD CLASS=\"NUMDATA\">%s</TD>
                  ...
                  <TD CLASS=\"NUMDATA\">%s</TD>
                  <TD CLASS=\"NUMDATA\">%s</TD>
              </TR>\n",$id, $name, &show_razryads($block_size), $resize_state, &show_razryads($current_size), &show_razryads($buffers), 
                       &show_razryads($target_size), &show_razryads($target_buffers), &show_razryads($prev_size), &show_razryads($prev_buffers), 
                       &show_razryads($lo_bnum), &show_razryads($hi_bnum), &show_razryads($lo_setid), &show_razryads($hi_setid), 
                       &show_razryads($set_count)) ;
      }
printf("<TR><TD CLASS=\"SZDATA\">Итого</TD><TD CLASS=\"NUMDATA\" COLSPAN=\"14\">%s</TD></TR>\n",$count_rows) ;
print "</TABLE>" ;
print "</TD></TR></TABLE>" ;
$sth->finish();
$dbh->disconnect();                                        
