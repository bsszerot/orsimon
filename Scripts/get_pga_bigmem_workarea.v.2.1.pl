...
$pv{order_field} = "estimated_optimal_size" ;
&get_forms_param() ;
...
&print_pga_navigation(6) ;

my $dbh = DBI->connect( $connector_definition{$current_connector}, $connector_credentials{$current_connector}, '');
$request = "SELECT workarea_address, operation_type, policy, estimated_optimal_size
                   FROM V\$SQL_WORKAREA WHERE ROWNUM <= 50 ORDER BY estimated_optimal_size DESC " ;
...
print "<TR><TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_pga_bigmem_workarea.cgi?order_field=workarea_address\" TARGET=\"cont\">workarea address</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_pga_bigmem_workarea.cgi?order_field=operation_type\" TARGET=\"cont\">operation type</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_pga_bigmem_workarea.cgi?order_field=policy\" TARGET=\"cont\">policy</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_pga_bigmem_workarea.cgi?order_field=estimated_optimal_size\" TARGET=\"cont\">estimated_optimal_size</A></TD>
           </TR>" ;
$count_rows = 0 ;
while (my ($workarea_address,$operation_type,$policy,$estimated_optimal_size) = $sth->fetchrow_array() ) { $count_rows += 1 ;
      printf("<TR><TD CLASS=\"SZDATA\">%s</TD>
                  <TD CLASS=\"SZDATA\">%s</TD>
                  <TD CLASS=\"SZDATA\">%s</TD>
                  <TD CLASS=\"NUMDATA\">%s</TD>
              </TR>\n",$workarea_address,$operation_type,$policy,&show_razryads($estimated_optimal_size)) ;
      }
printf("<TR><TD CLASS=\"SZDATA\" COLSPAN=\"3\">Итого строк: </TD><TD CLASS=\"NUMDATA\">%s</TD></TR>\n",$count_rows) ;
print "</TABLE>" ;
print "</TD></TR></TABLE>" ;
$sth->finish();
$dbh->disconnect();                                        
