...
&print_html_head("Итеративность PGA в процентах") ;

# добавить определение навигационно - оформительского механизма закладок
&print_pga_navigation(4) ;

my $dbh = DBI->connect( $connector_definition{$current_connector}, $connector_credentials{$current_connector}, '');
$request = 'SELECT optimal_count, optimal_count/total optimal_perc, onepass_count, onepass_count/total onepass_perc,
                   multipass_count, multipass_count/total multipass_perc
                   FROM (SELECT decode(sum(total_executions), 0, 1, sum(total_executions)) total, sum(OPTIMAL_EXECUTIONS) optimal_count,
                                sum(ONEPASS_EXECUTIONS) onepass_count, sum(MULTIPASSES_EXECUTIONS) multipass_count
                                FROM v$sql_workarea_histogram WHERE low_optimal_size > 64*1024)' ;
...
while (my ($optimal_count, $optimal_perc, $onepass_count, $onepass_perc, $multipass_count, $multipass_perc) = $sth->fetchrow_array() ) {
      print "<TABLE BORDER=\"1\" CELLSPACING=\"0\" CELLPADDING=\"0\" STYLE=\"width: 100%;\">" ;
      printf("<TR><TD CLASS=\"HEAD\">Тип обработки</TD><TD CLASS=\"HEAD\">единиц</TD><TD CLASS=\"HEAD\">%</TD></TR>
              <TR><TD CLASS=\"SZDATA\">Оптимальных обработок</TD><TD CLASS=\"NUMDATA\">%s</TD><TD CLASS=\"NUMDATA\">%0.2f</TD></TR>
              <TR><TD CLASS=\"SZDATA\">Однопроходных обработок</TD><TD CLASS=\"NUMDATA\">%s</TD><TD CLASS=\"NUMDATA\">%0.2f</TD></TR>
              <TR><TD CLASS=\"SZDATA\">Многопроходных обработок</TD><TD CLASS=\"NUMDATA\">%s</TD><TD CLASS=\"NUMDATA\">%0.2f</TD></TR>",
              &show_razryads($optimal_count),$optimal_perc,&show_razryads($onepass_count),$onepass_perc,&show_razryads($multipass_count),$multipass_perc) ;
      print "</TABLE>" ;
      }
print "</TD></TR></TABLE>" ;
$sth->finish();
