...
&print_html_head("Привелегии - системные") ;
...
print "<FORM ACTION=\"$base_url/cgi/get_privs_system.cgi\" METHOD=\"GET\">
       <TABLE BORDER=\"0\" CELLSPACING=\"0\" CELLPADDING=\"0\" STYLE=\"width: 100%;\">" ;
print "<TR><TD>&nbsp;источник: </TD><TD><SELECT NAME=\"source\" STYLE=\"width: 100%;\">" ;
$is_selected = "" ; if ( $pv{source} eq "DBA_SYS_PRIVS" ) { $is_selected = " SELECTED" ; } print "<OPTION VALUE=\"DBA_SYS_PRIVS\" $is_selected>Пользователи и группы (роли)</OPTION>" ;
$is_selected = "" ; if ( $pv{source} eq "USER_SYS_PRIVS" ) { $is_selected = " SELECTED" ; } print "<OPTION VALUE=\"USER_SYS_PRIVS\" $is_selected>Привелегии текущего пользователя</OPTION>" ;
$is_selected = "" ; if ( $pv{source} eq "ROLE_SYS_PRIVS" ) { $is_selected = " SELECTED" ; } print "<OPTION VALUE=\"ROLE_SYS_PRIVS\" $is_selected>Привелегии групп (ролей) текущего пользователя</OPTION>" ;
print "</SELECT></TD></TR>" ;
...
# актуально только для DBA_SYS_PRIVS
if ( $pv{grantee} ne "" ) { if ( $where_ext eq "" ) { $where_ext .= " WHERE " ; } else { $where_ext .= " AND " ; }
   if ( $pv{source} eq "DBA_SYS_PRIVS" ) { $where_ext .= " GRANTEE = '$pv{grantee}' " ; }
   if ( $pv{source} eq "USER_SYS_PRIVS" ) { $where_ext .= " USERNAME = '$pv{grantee}' " ; }
   if ( $pv{source} eq "ROLE_SYS_PRIVS" ) { $where_ext .= " ROLE = '$pv{grantee}' " ; }
   }

if ( $pv{source} eq "DBA_SYS_PRIVS" ) { if ( $pv{order_field} eq "" ) { $pv{order_field} = "" ; } $request = "SELECT GRANTEE,PRIVILEGE,ADMIN_OPTION FROM $pv{source} $where_ext order by $pv{order_field}" ; }
if ( $pv{source} eq "USER_SYS_PRIVS" ) { if ( $pv{order_field} eq "GRANTEE" ) { $pv{order_field} = "USERNAME" ; } $request = "SELECT USERNAME,PRIVILEGE,ADMIN_OPTION FROM $pv{source} $where_ext order by $pv{order_field}" ; }
if ( $pv{source} eq "ROLE_SYS_PRIVS" ) { if ( $pv{order_field} eq "GRANTEE" ) { $pv{order_field} = "ROLE" ; } $request = "SELECT ROLE,PRIVILEGE,ADMIN_OPTION FROM $pv{source} $where_ext order by $pv{order_field}" ; }
...
print "<TR><TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_privs_system.cgi?order_field=GRANTEE&source=$pv{source}&privilege=$pv{privilege}&grantee=$pv{grantee}\" TARGET=\"cont\">Субъект</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_privs_system.cgi?order_field=PRIVILEGE&source=$pv{source}&privilege=$pv{privilege}&grantee=$pv{grantee}\" TARGET=\"cont\">Привелегия</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_privs_system.cgi?order_field=ADMIN_OPTION&source=$pv{source}&privilege=$pv{privilege}&grantee=$pv{grantee}\" TARGET=\"cont\">Опция администратора</A></TD>
           </TR>" ;
$cnt_rows = 0 ;
while (my ($grantee,$privilege,$admin_option) = $sth->fetchrow_array() ) { $cnt_rows++ ;
      printf("<TR><TD CLASS=\"SZDATA\">%s</TD>
                  <TD CLASS=\"SZDATA\">%s</TD>
                  <TD CLASS=\"SZDATA\">%s</TD>
              </TR>\n",$grantee,$privilege,$admin_option) ;
      }
printf ("<TR><TD CLASS=\"SZDATA\">Итого</TD><TD CLASS=\"NUMDATA\" COLSPAN=\"2\">%s</TD></TR>\n", $cnt_rows) ;
print "</TABLE>" ;
