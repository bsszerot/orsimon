...
$request = 'select STATUS, NAME from v$controlfile' ;
                                 
my $sth = $dbh->prepare($request) ;
$sth->execute();
print "Content-Type: text/html\n\n"; 
&print_html_head("Информация о контрольных файлах") ;

# добавить определение навигационно - оформительского механизма закладок
&print_dbgeneral_navigation(3) ;
...
$count_rows = 0 ;
      printf("<TR><TD CLASS=\"CENTERDATA\">Статус</TD><TD CLASS=\"CENTERDATA\">Имя файла</TD></TR>\n") ;
while (my ($status,$name) = $sth->fetchrow_array() ) { if ( $status eq "" ) { $status = "&nbsp;" ; }
      printf("<TR><TD CLASS=\"SZDATA\">%s</TD><TD CLASS=\"SZDATA\">%s</TD></TR>\n",$status,$name) ;
      }
print "</TABLE>" ;
print "</TD></TR></TABLE>" ;
$sth->finish();
