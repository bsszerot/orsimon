...
$request = 'select GROUP# GROUPNUM,THREAD# THREAD,SEQUENCE# SEQUENCENUM,BYTES,MEMBERS,ARCHIVED,STATUS,FIRST_CHANGE# FIRST_CHANGE,TO_CHAR(FIRST_TIME,\'YYYY-MM-DD HH:MI:SS\') from v$log ORDER BY ' . $order_field ;
                                 
my $sth = $dbh->prepare($request) ;
$sth->execute();
print "Content-Type: text/html\n\n"; 
&print_html_head("Информация о группах оперативных журналов") ;

# добавить определение навигационно - оформительского механизма закладок
&print_redologs_navigation(1) ;
...
print "<TR><TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_redolog_group_info.cgi?order_field=GROUPNUM\" TARGET=\"cont\">GROUP\#</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_redolog_group_info.cgi?order_field=THREAD\" TARGET=\"cont\">THREAD\#</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_redolog_group_info.cgi?order_field=SEQUENCENUM\" TARGET=\"cont\">SEQUENCE\#</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_redolog_group_info.cgi?order_field=BYTES\" TARGET=\"cont\">BYTES</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_redolog_group_info.cgi?order_field=MEMBERS\" TARGET=\"cont\">MEMBERS</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_redolog_group_info.cgi?order_field=ARCHIVED\" TARGET=\"cont\">ARCHIVED</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_redolog_group_info.cgi?order_field=STATUS\" TARGET=\"cont\">STATUS</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_redolog_group_info.cgi?order_field=FIRST_CHANGE\" TARGET=\"cont\">FIRST CHANGE\#</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_redolog_group_info.cgi?order_field=FIRST_TIME\" TARGET=\"cont\">FIRST TIME</A></TD>
           </TR>" ;
$count_rows = 0 ;
while (my ($group,$thread,$sequence,$bytes,$members,$archived,$status,$first_change,$first_time) = $sth->fetchrow_array() ) {
      $count_rows += 1 ;
      printf("<TR><TD CLASS=\"NUMDATA\">%s</TD>
              ...
              </TR>\n",$group,$thread,&show_razryads($sequence),&show_razryads($bytes),$members,$archived,$status,&show_razryads($first_change),$first_time) ;
      }
