...
&print_html_head("Привелегии - колоночные") ;
...
print "<FORM ACTION=\"$base_url/cgi/get_privs_columns.cgi\" METHOD=\"GET\">
       <TABLE BORDER=\"0\" CELLSPACING=\"0\" CELLPADDING=\"0\" STYLE=\"width: 100%;\">" ;
print "<TR><TD>&nbsp;источник: </TD><TD><SELECT NAME=\"source\" STYLE=\"width: 100%;\">" ;
$is_selected = "" ; if ( $pv{source} eq "DBA_COL_PRIVS" ) { $is_selected = " SELECTED" ; } print "<OPTION VALUE=\"DBA_COL_PRIVS\" $is_selected>Пользователи и группы (роли)</OPTION>" ;
$is_selected = "" ; if ( $pv{source} eq "USER_COL_PRIVS" ) { $is_selected = " SELECTED" ; } print "<OPTION VALUE=\"USER_COL_PRIVS\" $is_selected>Привелегии текущего пользователя</OPTION>" ;
...
if ( $pv{grantee} ne "" && $pv{source} eq "ROLE_TAB_PRIVS" ) { if ( $where_ext eq "" ) { $where_ext .= " WHERE " ; } else { $where_ext .= " AND " ; } $where_ext .= " ROLE = '$pv{grantee}' " ; }
if ( $pv{grantee} ne "" && $pv{source} ne "ROLE_TAB_PRIVS" ) { if ( $where_ext eq "" ) { $where_ext .= " WHERE " ; } else { $where_ext .= " AND " ; } $where_ext .= " GRANTEE = '$pv{grantee}' " ; }

if ( $pv{order_field} eq "GRANTEE" && $pv{source} eq "ROLE_TAB_PRIVS" ) { $pv{order_field} = "ROLE" ; $request = "SELECT ROLE, OWNER, TABLE_NAME, COLUMN_NAME, PRIVILEGE, GRANTABLE FROM $pv{source} $where_ext order by $pv{order_field}" ; }
else { $request = "SELECT GRANTEE, OWNER, TABLE_NAME, COLUMN_NAME, PRIVILEGE, GRANTABLE FROM $pv{source} $where_ext order by $pv{order_field}" ; }
...
print "<TR><TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_privs_columns.cgi?order_field=ROLE&source=$pv{source}\&privilege=$pv{privilege}\&grantee=$pv{grantee}\&owner=$pv{owner}\&table_name=$pv{table_name}\" TARGET=\"cont\">Роль</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_privs_columns.cgi?order_field=OWNER&source=$pv{source}\&privilege=$pv{privilege}\&grantee=$pv{grantee}\&owner=$pv{owner}\&table_name=$pv{table_name}\" TARGET=\"cont\">Владелец</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_privs_columns.cgi?order_field=TABLE_NAME&source=$pv{source}\&privilege=$pv{privilege}\&grantee=$pv{grantee}\&owner=$pv{owner}\&table_name=$pv{table_name}\" TARGET=\"cont\">Таблица</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_privs_columns.cgi?order_field=COLUMN_NAME&source=$pv{source}\&privilege=$pv{privilege}\&grantee=$pv{grantee}\&owner=$pv{owner}\&table_name=$pv{table_name}\" TARGET=\"cont\">Колонка</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_privs_columns.cgi?order_field=PRIVILEGE&source=$pv{source}\&privilege=$pv{privilege}\&grantee=$pv{grantee}\&owner=$pv{owner}\&table_name=$pv{table_name}\" TARGET=\"cont\">Привелегия</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_privs_columns.cgi?order_field=ADMIN_OPTION&source=$pv{source}\&privilege=$pv{privilege}\&grantee=$pv{grantee}\&owner=$pv{owner}\&table_name=$pv{table_name}\" TARGET=\"cont\">GRANTABLE</A></TD>
           </TR>" ;
$cnt_rows = 0 ;
while (my ($grantee,$owner,$table_name,$column_name,$privilege,$grantable) = $sth->fetchrow_array() ) { $cnt_rows++ ;
      printf("<TR><TD CLASS=\"SZDATA\">%s</TD>
                  ...
                  <TD CLASS=\"SZDATA\">%s</TD>
                  <TD CLASS=\"SZDATA\">%s</TD>
              </TR>\n",$grantee,$owner,$table_name,$column_name,$privilege,$grantable) ;
      }
