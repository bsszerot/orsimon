...
$order_field = "RESOURCE_TYPE" ;
if ( $ENV{REQUEST_METHOD} eq "GET" && $ENV{QUERY_STRING} =~ /profile=(.*)&order_field=(.*)/) { $profile = $1 ; $order_field = $2 ;
    $profile =~ s/[\r\n]+//g ; $profile =~ s/%(..)/pack("c",hex($1))/ge ;
...
$request = "select RESOURCE_NAME, RESOURCE_TYPE, LIMIT from dba_profiles where profile = '$profile' order by $order_field asc" ;
my $db_conn_ext_req = DBI->connect( $connector_definition{$current_connector}, $connector_credentials{$current_connector}, '');
                                 
print "Content-Type: text/html\n\n"; 
&print_html_head("Информация о профиля $profile") ;

my $sth = $dbh->prepare($request) ;
$sth->execute();
print "<TABLE BORDER=\"1\" CELLSPACING=\"0\" CELLPADDING=\"0\" STYLE=\"width: 100%;\">" ;
print "<TR><TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_profile_info.cgi?profile=$profile&order_field=RESOURCE_NAME\" TARGET=\"cont\">Ресурс</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_profile_info.cgi?profile=$profile&order_field=RESOURCE_TYPE\" TARGET=\"cont\">Тип ресурса</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_profile_info.cgi?profile=$profile&order_field=LIMIT\" TARGET=\"cont\">Лимит</A></TD>
           </TR>" ;

$records_count_all = 0 ;
while (my ($resource_name,$resource_type,$limit) = $sth->fetchrow_array() ) {
      $records_count_all += 1 ;
      if ( $resource_type eq "" ) { $resource_type = "&nbsp;" ; }
      if ( $limit eq "" ) { $limit = "&nbsp;" ; }
      $limit_default_value = "" ;
      if ( $profile ne "DEFAULT" &&  $limit eq "DEFAULT" ) {
         $ext_request = "select ' ('||limit||')' from dba_profiles where profile = 'DEFAULT' and RESOURCE_NAME = '$resource_name' and RESOURCE_TYPE = '$resource_type'" ;
         $ext_reqest_handler = $dbh->prepare($ext_request) ; $ext_reqest_handler->execute();
         ( $limit_default_value ) = $ext_reqest_handler->fetchrow_array() ;
         $ext_reqest_handler->finish();
         }
      printf ("<TR><TD CLASS=\"SZDATA\">%s</TD>
               <TD CLASS=\"SZDATA\">%s</TD>
               <TD CLASS=\"SZDATA\">%s %s</TD>",$resource_name,$resource_type,$limit,$limit_default_value) ;
      }
# форматирование вложенной таблицей оставлено для единобразия
print "<TR><TD COLSPAN=\"3\">
           <TABLE BORDER=\"0\"><TR><TD>Записей лимитов профилей всего:</TD><TD>$records_count_all</TD></TR>" ;
print "</TABLE></TD></TR>" ;

print "</TABLE>" ;
$sth->finish();
$db_conn_ext_req->disconnect();
$dbh->disconnect();
