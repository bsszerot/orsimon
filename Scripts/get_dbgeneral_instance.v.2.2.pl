...
$request = 'select INSTANCE_NUMBER, INSTANCE_NAME, HOST_NAME, VERSION, TO_CHAR(STARTUP_TIME,\'YYYY-MM-DD HH24:MI:SS\') STARTUP_TIME, STATUS, PARALLEL, 
                   THREAD# THREAD_N, ARCHIVER, LOG_SWITCH_WAIT, LOGINS, SHUTDOWN_PENDING, DATABASE_STATUS, INSTANCE_ROLE, ACTIVE_STATE
                   from v$instance' ;
                                 
my $sth = $dbh->prepare($request) ;
$sth->execute();
print "Content-Type: text/html\n\n"; 
&print_html_head("Информация об экземпляре") ;

# добавить определение навигационно - оформительского механизма закладок
&print_dbgeneral_navigation(2) ;
...
$count_rows = 0 ;
while (my ($instznce_number,$instance_name,$hostname,$version,$startup_time,$status,$parallel,$thread_n,$archiver,$log_switch_wait,$logins,
           $shutdown_pending, $database_status,$instance_role,$active_state) = $sth->fetchrow_array() ) {
      if ( $status eq "" ) { $status = "&nbsp;" ; } if ( $type eq "" ) { $type = "&nbsp;" ; }

      printf("<TR><TD CLASS=\"CENTERDATA\" COLSPAN=\"3\">Общие сведения</TD></TR>\n") ;
      printf("<TR><TD CLASS=\"SZDATA\">Имя экземпляра</TD><TD CLASS=\"NUMDATA\">%s</TD></TR>\n",$instance_name) ;
      printf("<TR><TD CLASS=\"SZDATA\">Номер экземпляра</TD><TD CLASS=\"NUMDATA\">%s</TD></TR>\n",$instznce_number) ;
      printf("<TR><TD CLASS=\"SZDATA\">Имя хоста</TD><TD CLASS=\"NUMDATA\">%s</TD></TR>\n",$hostname) ;
      printf("<TR><TD CLASS=\"SZDATA\">Версия</TD><TD CLASS=\"NUMDATA\">%s</TD></TR>\n",$version) ;
      printf("<TR><TD CLASS=\"SZDATA\">Время старта экземпляра</TD><TD CLASS=\"NUMDATA\">%s</TD></TR>\n",$startup_time) ;
      printf("<TR><TD CLASS=\"SZDATA\">Статус</TD><TD CLASS=\"NUMDATA\">%s</TD></TR>\n",$status) ;
      printf("<TR><TD CLASS=\"SZDATA\">Parallel</TD><TD CLASS=\"NUMDATA\">%s</TD></TR>\n",$parallel) ;
      printf("<TR><TD CLASS=\"SZDATA\">Redo thread opened by the instance</TD><TD CLASS=\"NUMDATA\">%s</TD></TR>\n",$thread_n) ;
      printf("<TR><TD CLASS=\"SZDATA\">Archiver</TD><TD CLASS=\"NUMDATA\">%s</TD></TR>\n",$archiver) ;
      if ( $log_switch_wait eq "" ) { $log_switch_wait = "&nbsp;" ; } printf("<TR><TD CLASS=\"SZDATA\">Ожидание переключения журнала</TD><TD CLASS=\"NUMDATA\">%s</TD></TR>\n",$log_switch_wait) ;
      printf("<TR><TD CLASS=\"SZDATA\">Разрешен ли логин</TD><TD CLASS=\"NUMDATA\">%s</TD></TR>\n",$logins) ;
      printf("<TR><TD CLASS=\"SZDATA\">Shutdown pending</TD><TD CLASS=\"NUMDATA\">%s</TD></TR>\n",$shutdown_pending) ;
      printf("<TR><TD CLASS=\"SZDATA\">Статус базы</TD><TD CLASS=\"NUMDATA\">%s</TD></TR>\n",$database_status) ;
      printf("<TR><TD CLASS=\"SZDATA\">Роль экземпляра</TD><TD CLASS=\"NUMDATA\">%s</TD></TR>\n",$instance_role) ;
      printf("<TR><TD CLASS=\"SZDATA\">Active state</TD><TD CLASS=\"NUMDATA\">%s</TD></TR>\n",$active_state) ;
      }
print "</TABLE>" ;
print "</TD></TR></TABLE>" ;
$sth->finish();
