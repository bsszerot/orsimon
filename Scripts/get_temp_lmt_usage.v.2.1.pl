...
$request = 'select h.TABLESPACE_NAME,h.FILE_ID,f.FILE_NAME,h.BYTES_USED,h.BLOCKS_USED,h.BYTES_FREE,h.BLOCKS_FREE,h.RELATIVE_FNO from V$TEMP_SPACE_HEADER h, DBA_TEMP_FILES f WHERE h.FILE_ID = f.FILE_ID ORDER BY ' . $order_field ;
                                 
my $sth = $dbh->prepare($request) ;
$sth->execute();
print "Content-Type: text/html\n\n"; 
&print_html_head("Временные LMT файлы") ;

# добавить определение навигационно - оформительского механизма закладок
&print_temp_navigation(1) ;
...
print "<TR><TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_temp_lmt_usage.cgi?order_field=TABLESPACE_NAME\" TARGET=\"cont\">TABLESPACE NAME\#</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_temp_lmt_usage.cgi?order_field=RELATIVE_FNO\" TARGET=\"cont\">RELATIVE FNO</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_temp_lmt_usage.cgi?order_field=FILE_ID\" TARGET=\"cont\">FILE ID</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_temp_lmt_usage.cgi?order_field=FILE_NAME\" TARGET=\"cont\">FILE NAME</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_temp_lmt_usage.cgi?order_field=BYTES_USED\" TARGET=\"cont\">BYTES USED</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_temp_lmt_usage.cgi?order_field=BLOCKS_USED\" TARGET=\"cont\">BLOCKS USED</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_temp_lmt_usage.cgi?order_field=BYTES_FREE\" TARGET=\"cont\">BYTES FREE</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_temp_lmt_usage.cgi?order_field=BLOCKS_FREE\" TARGET=\"cont\">BLOCKS FREE</A></TD>
           </TR>" ;
$count_rows = 0 ;
while (my ($tablespace_name,$file_id,$name,$bytes_used,$blocks_used,$bytes_free,$blocks_free,$relative_fno) = $sth->fetchrow_array() ) {
      $count_rows += 1 ;
      ...
      printf("<TR><TD CLASS=\"SZDATA\">%s</TD>
                  ...
                  <TD CLASS=\"NUMDATA\">%s</TD>
                  <TD CLASS=\"NUMDATA\">%s</TD>
              </TR>\n",$tablespace_name,$relative_fno,$file_id,$name,$bytes_used,$blocks_used,$bytes_free,$blocks_free) ;
      }
