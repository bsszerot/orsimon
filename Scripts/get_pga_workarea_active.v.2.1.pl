...
&print_pga_navigation(5) ;

my $dbh = DBI->connect( $connector_definition{$current_connector}, $connector_credentials{$current_connector}, '');
$request = 'SELECT to_number(decode(SID, 65535, NULL, SID)) sid, operation_type OPERATION, trunc(EXPECTED_SIZE/1024) ESIZE,
                   trunc(ACTUAL_MEM_USED/1024) MEM, trunc(MAX_MEM_USED/1024) MAXMEM, NUMBER_PASSES PASS, trunc(TEMPSEG_SIZE/1024) TSIZE
                   FROM V$SQL_WORKAREA_ACTIVE ORDER BY 1, ' . $pv{order_field} ;
...
print "<TR><TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_pga_workarea_active.cgi?order_field=sid\" TARGET=\"cont\">Sid</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_pga_workarea_active.cgi?order_field=OPERATION\" TARGET=\"cont\">OPERATION</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_pga_workarea_active.cgi?order_field=ESIZE\" TARGET=\"cont\">ESIZE</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_pga_workarea_active.cgi?order_field=MEM\" TARGET=\"cont\">MEM</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_pga_workarea_active.cgi?order_field=MAXMEM\" TARGET=\"cont\">MAXMEM</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_pga_workarea_active.cgi?order_field=TSIZE\" TARGET=\"cont\">TSIZE</A></TD>
           </TR>" ;
$count_rows = 0 ;
while (my ($sid,$operation,$esize,$mem,$maxmem,$tsize) = $sth->fetchrow_array() ) { $count_rows += 1 ;
      printf("<TR><TD CLASS=\"SZDATA\">%s</TD>
                  ...
                  <TD CLASS=\"NUMDATA\">%s</TD>
                  <TD CLASS=\"NUMDATA\">%s</TD>
              </TR>\n",$sid,$operation,&show_razryads($esize),&show_razryads($mem),&show_razryads($maxmem),&show_razryads($tsize)) ;
      }
printf("<TR><TD CLASS=\"SZDATA\" COLSPAN=\"5\">Итого строк: </TD><TD CLASS=\"NUMDATA\">%s</TD></TR>\n",$count_rows) ;
print "</TABLE>" ;
print "</TD></TR></TABLE>" ;
$sth->finish();
$dbh->disconnect();                                        
