...
$request = 'select GROUP# GROUPNUM,STATUS,TYPE,MEMBER from v$logfile ORDER BY ' . $order_field ;
                                 
my $sth = $dbh->prepare($request) ;
$sth->execute();
print "Content-Type: text/html\n\n"; 
&print_html_head("Информация о членах групп оперативных журналов") ;

# добавить определение навигационно - оформительского механизма закладок
&print_redologs_navigation(2) ;
...
print "<TR><TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_redolog_file_info.cgi?order_field=GROUPNUM\" TARGET=\"cont\">GROUP\#</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_redolog_file_info.cgi?order_field=STATUS\" TARGET=\"cont\">STATUS</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_redolog_file_info.cgi?order_field=TYPE\" TARGET=\"cont\">TYPE</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_redolog_file_info.cgi?order_field=MEMBER\" TARGET=\"cont\">Имя файла</A></TD>
           </TR>" ;
$count_rows = 0 ;
while (my ($group,$status,$type,$member) = $sth->fetchrow_array() ) {
      $count_rows += 1 ;
