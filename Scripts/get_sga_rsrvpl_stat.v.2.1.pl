...
$pv{order_field} = "SHARED_POOL_SIZE_FOR_ESTIMATE" ;
&get_forms_param() ;

$ENV{ORACLE_SID} = $current_connector ;

print "Content-Type: text/html\n\n";
&print_html_head("Статистика резервного пула") ;

# добавить определение навигационно - оформительского механизма закладок
&print_sga_navigation(9) ;

my $dbh = DBI->connect( $connector_definition{$current_connector}, $connector_credentials{$current_connector}, '');
$request = 'SELECT FREE_SPACE, AVG_FREE_SIZE, FREE_COUNT, MAX_FREE_SIZE, USED_SPACE, AVG_USED_SIZE, USED_COUNT, MAX_USED_SIZE, REQUESTS, REQUEST_MISSES, LAST_MISS_SIZE, MAX_MISS_SIZE, REQUEST_FAILURES, LAST_FAILURE_SIZE, ABORTED_REQUEST_THRESHOLD, ABORTED_REQUESTS, LAST_ABORTED_SIZE FROM V$SHARED_POOL_RESERVED' ;
my $sth = $dbh->prepare($request) ; $sth->execute();
      print "<TABLE CELLSPACING=\"0\" CELLPADDING=\"0\" STYLE=\"border: 2pt navy; border-style: none solid solid solid; width: 100%;\"><TR><TD>" ;
while (my ($free_space,$avg_free_size,$free_count,$max_free_size,$used_space,$avg_user_size,$used_count,$max_used_size,$requests,$request_misses,$last_miss_size,$max_miss_size,$request_filures,$last_filure_size,$aborted_request_threshold,$aborted_requests,$last_aborted_size) = $sth->fetchrow_array() ) { 
# вывести непосредственно контент
      print "<TABLE BORDER=\"1\" CELLSPACING=\"0\" CELLPADDING=\"0\" STYLE=\"width: 100%;\">" ;
      printf("<TR><TD CLASS=\"SZDATA\">FREE_SPACE</TD><TD CLASS=\"NUMDATA\">%s</TD></TR>
              <TR><TD CLASS=\"SZDATA\">AVG_FREE_SIZE</TD><TD CLASS=\"NUMDATA\">%0.2f</TD></TR>
              <TR><TD CLASS=\"SZDATA\">FREE_COUNT</TD><TD CLASS=\"NUMDATA\">%s</TD></TR>
              <TR><TD CLASS=\"SZDATA\">MAX_FREE_SIZE</TD><TD CLASS=\"NUMDATA\">%s</TD></TR>
              <TR><TD CLASS=\"SZDATA\">USED_SPACE</TD><TD CLASS=\"NUMDATA\">%s</TD></TR>
              <TR><TD CLASS=\"SZDATA\">AVG_USED_SIZE</TD><TD CLASS=\"NUMDATA\">%0.2f</TD></TR>
              <TR><TD CLASS=\"SZDATA\">USED_COUNT</TD><TD CLASS=\"NUMDATA\">%s</TD></TR>
              <TR><TD CLASS=\"SZDATA\">MAX_USED_SIZE</TD><TD CLASS=\"NUMDATA\">%s</TD></TR>
              <TR><TD CLASS=\"SZDATA\">REQUESTS</TD><TD CLASS=\"NUMDATA\">%s</TD></TR>
              <TR><TD CLASS=\"SZDATA\">REQUEST_MISSES (должно стремиться к 0)</TD><TD CLASS=\"NUMDATA\">%s</TD></TR>
              <TR><TD CLASS=\"SZDATA\">LAST_MISS_SIZE</TD><TD CLASS=\"NUMDATA\">%s</TD></TR>
              <TR><TD CLASS=\"SZDATA\">MAX_MISS_SIZE</TD><TD CLASS=\"NUMDATA\">%s</TD></TR>
              <TR><TD CLASS=\"SZDATA\">REQUEST_FAILURES (должно стремиться к 0)</TD><TD CLASS=\"NUMDATA\">%s</TD></TR>
              <TR><TD CLASS=\"SZDATA\">LAST_FAILURE_SIZE</TD><TD CLASS=\"NUMDATA\">%s</TD></TR>
              <TR><TD CLASS=\"SZDATA\">ABORTED_REQUEST_THRESHOLD</TD><TD CLASS=\"NUMDATA\">%s</TD></TR>
              <TR><TD CLASS=\"SZDATA\">ABORTED_REQUESTS</TD><TD CLASS=\"NUMDATA\">%s</TD></TR>
              <TR><TD CLASS=\"SZDATA\">LAST_ABORTED_SIZE</TD><TD CLASS=\"NUMDATA\">%s</TD></TR>",&show_razryads($free_space),$avg_free_size,&show_razryads($free_count),&show_razryads($max_free_size),&show_razryads($used_space),$avg_user_size,&show_razryads($used_count),&show_razryads($max_used_size),&show_razryads($requests),&show_razryads($request_misses),&show_razryads($last_miss_size),&show_razryads($max_miss_size),&show_razryads($request_filures),&show_razryads($last_filure_size),&show_razryads($aborted_request_threshold),&show_razryads($aborted_requests),&show_razryads($last_aborted_size)) ;
      print "</TABLE>" ;
      }
      print "</TD></TR></TABLE>" ;
$sth->finish();
