...
$pv{order_field} = "NAMESPACE" ;
&get_forms_param() ;

$ENV{ORACLE_SID} = $current_connector ;

print "Content-Type: text/html\n\n";
&print_html_head("Статистика библиотечного кэша") ;

# добавить определение навигационно - оформительского механизма закладок
&print_sga_navigation(6) ;

my $dbh = DBI->connect( $connector_definition{$current_connector}, $connector_credentials{$current_connector}, '');
# gets говорит о свопинге памяти, pins - сессия не выполняет курсор несколько раз или не может найти разделяемый курсор (из руководства тюнинга 9 версии)
$request = "SELECT namespace, gets, gethits, gethitratio * 100, pins, pinhits, pinhitratio * 100, reloads, invalidations FROM V\$LIBRARYCACHE ORDER BY $pv{order_field}" ;
...
print "<TR><TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_sga_libcache_stat.cgi?order_field=namespace\" TARGET=\"cont\">Тип объектов</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_sga_libcache_stat.cgi?order_field=gets\" TARGET=\"cont\">Обращений</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_sga_libcache_stat.cgi?order_field=gethits\" TARGET=\"cont\">Попаданий</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_sga_libcache_stat.cgi?order_field=gethitratio\" TARGET=\"cont\">Коэффициент попадания</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_sga_libcache_stat.cgi?order_field=pins\" TARGET=\"cont\">Обращений PIN</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_sga_libcache_stat.cgi?order_field=pinhits\" TARGET=\"cont\">Попаданий PIN</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_sga_libcache_stat.cgi?order_field=pinhitratio\" TARGET=\"cont\">Коэффициент попадания PIN</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_sga_libcache_stat.cgi?order_field=reloads\" TARGET=\"cont\">Перегрузок</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_sga_libcache_stat.cgi?order_field=invalidations\" TARGET=\"cont\">Устареваний</A></TD>
           </TR>" ;
$count_rows = 0 ;
$all_buffers = 0 ;
while (my ($namespace, $gets, $gethits, $gethitratio, $pins, $pinhits, $pinhitratio, $reloads, $invalidations) = $sth->fetchrow_array() ) {
      $rows_count += 1 ;
      if ( $pool eq "" ) { $pool = "&nbsp;" ; }
      printf("<TR><TD CLASS=\"SZDATA\">%s</TD>
              ...
              $namespace, &show_razryads($gets), &show_razryads($gethits), $gethitratio, &show_razryads($pins), &show_razryads($pinhits), 
              $pinhitratio, &show_razryads($reloads), &show_razryads($invalidations)) ;
      }
printf("<TR><TD CLASS=\"SZDATA\">Итого строк</TD><TD CLASS=\"NUMDATA\" COLSPAN=\"8\">%s</TD></TR>\n",$rows_count) ;
print "</TABLE>" ;
print "</TD></TR></TABLE>" ;
$sth->finish();
$dbh->disconnect();                                        
