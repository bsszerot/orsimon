...
$sid = 1 ; $serial = 1 ;
if ( $ENV{REQUEST_METHOD} eq "GET" && $ENV{QUERY_STRING} =~ /sid=(\d+)&serial=(\d+)/) { 
   $sid = $1 ; $serial = $2 ; $sid =~ s/%(..)/pack("c",hex($1))/ge ; $serial =~ s/%(..)/pack("c",hex($1))/ge ; }
else { die "No valid request data" ; }
#$sid = 72 ; $serial = 37188 ; #72, SERIAL# = 37188 
...
#$s_sql_address,$s_sql_hash_value,$s_prev_sql_addr,$s_prev_hash_value,
#$s_row_wait_obj,$s_row_wait_file,$s_row_wait_block,#$s_row_wait_row,
      }
print "</TABLE>" ;
