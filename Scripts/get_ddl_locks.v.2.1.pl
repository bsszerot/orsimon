...
&get_forms_param() ;
if ( $pv{order_field} eq "" ) { $pv{order_field} = "l.SESSION_ID" ; }
...
print "<FORM ACTION=\"$base_url/cgi/get_ddl_locks.cgi\" METHOD=\"GET\">                                                                                        
...
print "<TR><TD>&nbsp;Пользователь: </TD><TD><INPUT TYPE=\"INPUT\" NAME=\"owner\" VALUE=\"$pv{owner}\" STYLE=\"width: 100%\"></TD></TR>" ;
print "<TR><TD>&nbsp;Объект LIKE: </TD><TD><INPUT TYPE=\"INPUT\" NAME=\"object_name\" VALUE=\"$pv{object_name}\" STYLE=\"width: 100%\"></TD></TR>" ;
print "<TR><TD>&nbsp;Тип LIKE: </TD><TD><INPUT TYPE=\"INPUT\" NAME=\"object_type\" VALUE=\"$pv{object_type}\" STYLE=\"width: 100%\"></TD></TR>" ;
...
if ( $pv{is_view_result} eq "yes" ) {
   $where_object_name = "" ; if ( $pv{object_name} ne "" ) { $where_object_name = " AND l.NAME LIKE '%$pv{object_name}%' " ; }
   $where_object_type = "" ; if ( $pv{object_type} ne "" ) { $where_object_type = " AND l.TYPE LIKE '%$pv{object_type}%' " ; }
   $where_owner = "" ; if ( $pv{owner} ne "" ) { $where_owner = " AND l.OWNER LIKE '%$pv{owner}%' " ; }
   $request = 'SELECT l.SESSION_ID SID, s.serial#, l.OWNER, l.NAME, l.TYPE, l.MODE_HELD, l.MODE_REQUESTED FROM DBA_DDL_LOCKS l, V$SESSION s WHERE l.SESSION_ID = s.sid ' . " $where_owner $where_object_name $where_object_type ORDER BY $pv{order_field} " ;
...
# вывести непосредственно контент
   ...
   print "<TR><TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_ddl_locks.cgi?order_field=l.SESSION_ID\" TARGET=\"cont\">Сессия</A></TD>
              <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_ddl_locks.cgi?order_field=l.OWNER\" TARGET=\"cont\">Владелец</A></TD>
              <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_ddl_locks.cgi?order_field=l.NAME\" TARGET=\"cont\">Имя объекта</A></TD>
              <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_ddl_locks.cgi?order_field=l.TYPE\" TARGET=\"cont\">Тип объекта</A></TD>
              <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_ddl_locks.cgi?order_field=l.MODE_HELD\" TARGET=\"cont\">Тип блокировки</A></TD>
              <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_ddl_locks.cgi?order_fiудв=l.MODE_REQUESTED\" TARGET=\"cont\">Тип ожидания</A></TD>
              </TR>" ;
   
   $is_not_null_answer = "no" ; $sum_records = 0 ;
   while (my ($sid,$serial,$owner,$name,$type,$mode_held,$mode_requested) = $sth->fetchrow_array() ) { $sum_records += 1 ;
         printf("<TR><TD CLASS=\"SZDATA\"><A HREF=\"$base_url/cgi/get_session_info.cgi?sid=%s&serial=%s\" TARGET=\"cont\">%s,%s</A></TD>
                     ...
                     <TD CLASS=\"SZDATA\">%s</TD>
                     <TD CLASS=\"SZDATA\">%s</TD>
                 </TR>\n",$sid,$serial,$sid,$serial,$owner,$name,$type,$mode_held,$mode_requested) ;
         $is_not_null_answer = "yes" ;
         }
   if ( $is_not_null_answer eq "no" ) { print "<TR><TD CLASS=\"SZDATA\" COLSPAN=\"6\">Блокировок DML отсутствуют</TD></TR>" ;}
   else { print "<TR><TD CLASS=\"SZDATA\">Итого</TD><TD COLSPAN=\"5\" CLASS=\"NUMDATA\">$sum_records</TD></TR>" ; }
   print "</TABLE>" ;
   print "</TD></TR>" ;
   $sth->finish();
   }
