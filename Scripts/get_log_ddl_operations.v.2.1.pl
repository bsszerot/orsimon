...
$pv{order_field} = "TMST_RECORD" ;
&get_forms_param() ;

$ENV{ORACLE_SID} = $current_connector ;

print "Content-Type: text/html\n\n"; 
&print_html_head("Журнал DDL операций") ;
...
print "<FORM ACTION=\"$base_url/cgi/get_log_ddl_operations.cgi\" METHOD=\"GET\">
       <TABLE BORDER=\"0\" CELLSPACING=\"0\" CELLPADDING=\"0\" STYLE=\"width: 100%;\">" ;
print "<TR><TD STYLE=\"width: 171pt;\">&nbsp;Владелец: </TD><TD><INPUT TYPE=\"INPUT\" NAME=\"owner_filter\" VALUE=\"$pv{owner_filter}\" STYLE=\"width: 100%\"></TD></TR>" ;
print "<TR><TD STYLE=\"width: 171pt;\">&nbsp;Имя&nbsp;содержит: </TD><TD><INPUT TYPE=\"INPUT\" NAME=\"name_filter\" VALUE=\"$pv{name_filter}\" STYLE=\"width: 100%\"></TD></TR>" ;
print "<TR><TD STYLE=\"width: 171pt;\">&nbsp;Тип&nbsp;сегмента: </TD><TD><INPUT TYPE=\"INPUT\" NAME=\"type_filter\" VALUE=\"$pv{type_filter}\" STYLE=\"width: 100%\"></TD></TR>" ;
print "<TR><TD STYLE=\"width: 171pt;\">&nbsp;Табличные&nbsp;пространства: </TD><TD><INPUT TYPE=\"INPUT\" NAME=\"tbs_filter\" VALUE=\"$pv{tbs_filter}\" STYLE=\"width: 100%\"></TD></TR>" ;
...
# сформировать уточняющие фильтры для запроса статистики
   ...
   $request = 'select TO_CHAR(TMST_RECORD,\'YYYY-MM-DD HH24:MI:SS\'), EVENT_NAME, LOGIN_USER, OBJECT_OWNER, OBJECT_NAME, OBJECT_TYPE FROM BESTAT_DDL_LOGS ' . $where_ext . " order by $pv{order_field} asc" ;
   ...
   print "<TR><TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_log_ddl_operations.cgi?order_field=TMST_RECORD&isfirst=$pv{isfirst}&srcptr=$pv{srcptr}&tbs_filter=$pv{tbs_filter}&name_filter=$pv{name_filter}&owner_filter=$pv{owner_filter}&type_filter=$pv{type_filter}\" TARGET=\"cont\">Дата ошибки</A></TD>
              <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_log_ddl_operations.cgi?order_field=EVENT_NAME&isfirst=$pv{isfirst}&srcptr=$pv{srcptr}&tbs_filter=$pv{tbs_filter}&name_filter=$pv{name_filter}&owner_filter=$pv{owner_filter}&type_filter=$pv{type_filter}\" TARGET=\"cont\">Событие</A></TD>
              <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_log_ddl_operations.cgi?order_field=LOGIN_USER&isfirst=$pv{isfirst}&srcptr=$pv{srcptr}&tbs_filter=$pv{tbs_filter}&name_filter=$pv{name_filter}&owner_filter=$pv{owner_filter}&type_filter=$pv{type_filter}\" TARGET=\"cont\">Инициатор</A></TD>
              <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_log_ddl_operations.cgi?order_field=OBJECT_OWNER&isfirst=$pv{isfirst}&srcptr=$pv{srcptr}&tbs_filter=$pv{tbs_filter}&name_filter=$pv{name_filter}&owner_filter=$pv{owner_filter}&type_filter=$pv{type_filter}\" TARGET=\"cont\">Владелец</A></TD>
              <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_log_ddl_operations.cgi?order_field=OBJECT_NAME&isfirst=$pv{isfirst}&srcptr=$pv{srcptr}&tbs_filter=$pv{tbs_filter}&name_filter=$pv{name_filter}&owner_filter=$pv{owner_filter}&type_filter=$pv{type_filter}\" TARGET=\"cont\">Имя объекта</A></TD>
              <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_log_ddl_operations.cgi?order_field=OBJECT_TYPE&isfirst=$pv{isfirst}&srcptr=$pv{srcptr}&tbs_filter=$pv{tbs_filter}&name_filter=$pv{name_filter}&owner_filter=$pv{owner_filter}&type_filter=$pv{type_filter}\" TARGET=\"cont\">Тип объекта</A></TD>
              </TR>" ;

   $count_all = 0 ;
   while (my ( $tmst_record,$event_name,$login_user,$object_owner,$object_name,$object_type ) = $sth->fetchrow_array() ) {
         if ( $count_by_users{$login_user} eq "" || $count_by_users{$login_user} < 1 ) { $count_by_users{$login_user} = 0 ; } 
         $count_by_users{$login_user} += 1 ; $count_all += 1 ;
         printf("<TR><TD CLASS=\"SZDATA\">%s</TD>
                     ...
                     <TD CLASS=\"SZDATA\">%s</TD>
                     <TD CLASS=\"SZDATA\">%s</TD>
                    </TR>\n",$tmst_record,$event_name,$login_user,$object_owner,$object_name,$object_type ) ;
         }
#                    <TD CLASS=\"SZDATA\"><PRE STYLE=\"font-family: sans-serif; font_size: 10pt ;\">%s</PRE></TD>
   print "<TR><TD COLSPAN=\"19\">
              <TABLE BORDER=\"0\"><TR><TD>Файлов всего:</TD><TD>$count_all, в т.ч.</TD></TR>" ;
   @count_by_users_keys_nosort = keys %count_by_users ; @count_by_users_keys_sort = sort @count_by_users_keys_nosort ;
   for ($i=0;$i<=$#count_by_users_keys_sort;$i++) {
       print "<TR><TD>&nbsp;&nbsp;&nbsp;$count_by_users_keys_sort[$i]:</TD><TD>$count_by_users{$count_by_users_keys_sort[$i]}</TD></TR>" ;
       }
   print "</TABLE>" ;
   $sth->finish();
   $dbh->disconnect();                                        
