...
$request = 'SELECT /*+ ORDERED */ l.sid SID, s.serial#, l.ID1, l.ID2, decode(l.LMODE,0,\'none\',1,\'null (NULL)\',2,\'row-S (SS)\',3,\'row-X (SX)\',4,\'share (X)\',5,\'S/Row-X (SSX)\',6,\'executive (X)\',l.LMODE), decode(l.REQUEST,0,\'none\',1,\'null (NULL)\',2,\'row-S (SS)\',3,\'row-X (SX)\',4,\'share (X)\',5,\'S/Row-X (SSX)\',6,\'executive (X)\',l.REQUEST), l.TYPE, decode(block,0,\'нет\',1,\'блокирует\',2,\'глобально\',to_char(block)) BLOCKING_OTHERS FROM V$LOCK l, V$SESSION s WHERE l.SID = s.SID (+) ORDER BY ' . $order_field ;
...
print "<TR><TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_dml_locks.cgi?order_field=l.SID\" TARGET=\"cont\">Сессия</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_dml_locks.cgi?order_field=l.ID1\" TARGET=\"cont\">ID1</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_dml_locks.cgi?order_field=l.ID2\" TARGET=\"cont\">ID2</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_dml_locks.cgi?order_field=l.LMODE\" TARGET=\"cont\">Режим блокировки</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_dml_locks.cgi?order_field=l.REQUEST\" TARGET=\"cont\">Режим ожидания</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_dml_locks.cgi?order_field=l.TYPE\" TARGET=\"cont\">Тип блокировки</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_dml_locks.cgi?order_field=l.ИДЩСЛ\" TARGET=\"cont\">Блокирует ли других</A></TD>
           </TR>" ;
$is_not_null_answer = "no" ; $sum_records = 0 ;
while (my ($sid,$serial,$id1,$id2,$lmode,$request,$type,$block) = $sth->fetchrow_array() ) { $sum_records += 1 ;
      ...
      printf("<TR><TD CLASS=\"SZDATA\"><A HREF=\"$base_url/cgi/get_session_info.cgi?sid=%s&serial=%s\" TARGET=\"cont\">%s,%s</A></TD>
                  ...
                  <TD CLASS=\"CENTERDATA\">%s</TD>
                  <TD CLASS=\"CENTERDATA\">%s</TD>
              </TR>\n",$sid,$serial,$sid,$serial,$id1,$id2,$lmode,$request,$type,$block) ;
      $is_not_null_answer = "yes" ;
      }
if ( $is_not_null_answer eq "no" ) { print "<TR><TD CLASS=\"SZDATA\" COLSPAN=\"7\">Блокировки DML отсутствуют</TD></TR>" ;}
else { print "<TR><TD CLASS=\"SZDATA\">Итого</TD><TD COLSPAN=\"6\" CLASS=\"NUMDATA\">$sum_records</TD></TR>" ; }
print "</TABLE>" ;
print "</TD></TR></TABLE>" ;
$sth->finish();
$dbh->disconnect();                                        
