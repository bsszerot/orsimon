...
sub print_sga_navigation($) { $active_tab{1} = "" ; $active_tab{2} = "" ; $active_tab{3} = "" ; 
    ...
    print "<TABLE CELLSPACING=\"0\" CELLPADDING=\"0\" STYLE=\"border: 0pt none; width: 100%;\">
           ...
           <TD STYLE=\"border: 2pt navy; border-style: solid $active_tab{2}; text-align: center;\">
               <A HREF=\"$base_url/cgi/get_sga_buffer_pools.cgi?order_field=NAME\" TARGET=\"cont\">Пулы буферов</A></TD>
           <TD STYLE=\"border: 2pt navy; border-style: none none solid none;\">&nbsp;&nbsp;&nbsp;&nbsp;</TD>
           <TD STYLE=\"border: 2pt navy; border-style: solid $active_tab{3}; text-align: center;\">
               <A HREF=\"$base_url/cgi/get_sga_buffer_pool_stat.cgi?order_field=NAME\" TARGET=\"cont\">Статистика пулов буферов</A></TD>
           <TD STYLE=\"border: 2pt navy; border-style: none none solid none;\">&nbsp;&nbsp;&nbsp;&nbsp;</TD>
...
$pv{order_field} = "PARAMETER" ;
&get_forms_param() ;

$ENV{ORACLE_SID} = $current_connector ;

print "Content-Type: text/html\n\n";
&print_html_head("Статистика кэша словаря") ;

# добавить определение навигационно - оформительского механизма закладок
&print_sga_navigation(8) ;

my $dbh = DBI->connect( $connector_definition{$current_connector}, $connector_credentials{$current_connector}, '');

# приём "+1" используется для исключения деления на 0
$request = "SELECT cache# cache_n, type, subordinate# subordinate_n, parameter, sum(count) count, sum(usage) usage, sum(fixed) fixed,  sum(gets) gets, sum(getmisses) getmisses, 100*sum(gets - getmisses) / sum(gets) pct_succ_gets, 
                   sum(scans) scans, sum(scanmisses) scanmisses, 100*sum(scans - scanmisses) / (sum(scans)+1) pct_succ_scans, sum(scancompletes) scancompletes, sum(modifications) updates, sum(flushes) flushes, 
                   sum(dlm_requests) dlm_requests, sum(dlm_conflicts) dlm_conflicts, sum(dlm_releases) dlm_releases 
                   FROM V\$ROWCACHE WHERE gets > 0 GROUP BY cache#,type,subordinate#,parameter ORDER BY $pv{order_field}" ;
...
print "<TR><TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_sga_dictcache_stat.cgi?order_field=cache_n\" TARGET=\"cont\">ID кэша</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_sga_dictcache_stat.cgi?order_field=type\" TARGET=\"cont\">Тип</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_sga_dictcache_stat.cgi?order_field=subordinate_n\" TARGET=\"cont\">Подчинённость</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_sga_dictcache_stat.cgi?order_field=parameter\" TARGET=\"cont\">Параметр</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_sga_dictcache_stat.cgi?order_field=count\" TARGET=\"cont\">Элементов кэша всего</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_sga_dictcache_stat.cgi?order_field=usage\" TARGET=\"cont\">Элементов кэша валидных</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_sga_dictcache_stat.cgi?order_field=fixed\" TARGET=\"cont\">Элементов кэша зафиксированных</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_sga_dictcache_stat.cgi?order_field=gets\" TARGET=\"cont\">Запросов всего</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_sga_dictcache_stat.cgi?order_field=getmisses\" TARGET=\"cont\">Запросов не в кэше </A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_sga_dictcache_stat.cgi?order_field=pct_succ_gets\" TARGET=\"cont\">Запросы, коэффициент попаданий</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_sga_dictcache_stat.cgi?order_field=scans\" TARGET=\"cont\">Сканирований всего</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_sga_dictcache_stat.cgi?order_field=scanmisses\" TARGET=\"cont\">Сканирований не в кэше </A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_sga_dictcache_stat.cgi?order_field=pct_succ_scans\" TARGET=\"cont\">Сканирований, коэффициент попаданий</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_sga_dictcache_stat.cgi?order_field=scancompletes\" TARGET=\"cont\">Сканирований завершено</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_sga_dictcache_stat.cgi?order_field=updates\" TARGET=\"cont\">Обновлений</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_sga_dictcache_stat.cgi?order_field=updates\" TARGET=\"cont\">Сбросов на диск</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_sga_dictcache_stat.cgi?order_field=DLM_REQUESTS\" TARGET=\"cont\">Запросов DLM</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_sga_dictcache_stat.cgi?order_field=DLM_CONFLICTS\" TARGET=\"cont\">Конфликтов DLM</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_sga_dictcache_stat.cgi?order_field=DLM_RELEASES\" TARGET=\"cont\">DLM releases</A></TD>
           </TR>" ;

$count_rows = 0 ;
while (my ($cache_n,$type,$subordinate_n,$parameter,$count,$usage,$fixed,$gets,$getmisses,$pct_succ_gets,$scans,$scanmisses,$pct_succ_scans,$scancompletes,$updates,$flushes,$dml_requests,$dml_conflicts,$dml_releases) = $sth->fetchrow_array() ) {
      $rows_count += 1 ;
      if ( $subordinate_n eq "" ) { $subordinate_n = "&nbsp;" ; }
      printf("<TR><TD CLASS=\"NUMDATA\">%s</TD>
                  ...
                  <TD CLASS=\"NUMDATA\">%s</TD>
                  <TD CLASS=\"NUMDATA\">%s</TD>
              </TR>\n",$cache_n,$type,$subordinate_n,$parameter,&show_razryads($count),&show_razryads($usage),&show_razryads($fixed),&show_razryads($gets),
                       &show_razryads($getmisses),$pct_succ_gets,&show_razryads($scans),&show_razryads($scanmisses),$pct_succ_scans,
                       &show_razryads($scancompletes),&show_razryads($updates),&show_razryads($flushes),&show_razryads($dml_requests),
                       &show_razryads($dml_conflicts),&show_razryads($dml_releases)) ;
      }
printf("<TR><TD CLASS=\"SZDATA\" COLSPAN=\"4\">Итого строк</TD><TD CLASS=\"NUMDATA\" COLSPAN=\"15\">%s</TD></TR>\n",$rows_count) ;
print "</TABLE>" ;
print "</TD></TR></TABLE>" ;
$sth->finish();
$dbh->disconnect();                                        
