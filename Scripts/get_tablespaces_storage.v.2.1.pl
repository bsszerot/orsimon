...
&print_html_head("Параметры хранения объектов по умолчанию") ;
...
&print_tablespaces_navigation(2) ;
...
print "<FORM ACTION=\"$base_url/cgi/get_tablespaces_storage.cgi\" METHOD=\"GET\">
...
if ( $pv{srcptr} eq "0" || $pv{srcptr} eq "" ) { $request = "select TABLESPACE_NAME, EXTENT_MANAGEMENT, ALLOCATION_TYPE, SEGMENT_SPACE_MANAGEMENT, INITIAL_EXTENT, NEXT_EXTENT, MIN_EXTENTS, MAX_EXTENTS, PCT_INCREASE, MIN_EXTLEN
                                                             from dba_tablespaces " . $where_ext . " order by $pv{order_field}" ; }
else { if ( $where_ext ne "" ) { $where_ext =~ s/WHERE/AND/g ; } 
       $request = "select TABLESPACE_NAME, EXTENT_MANAGEMENT, ALLOCATION_TYPE, SEGMENT_SPACE_MANAGEMENT, INITIAL_EXTENT, NEXT_EXTENT, MIN_EXTENTS, MAX_EXTENTS, PCT_INCREASE, MIN_EXTLEN
                          from BESTAT_dba_tablespaces WHERE POINT = $pv{srcptr} " . $where_ext . " order by $pv{order_field}" ; }
...
print "<TABLE BORDER=\"1\" CELLSPACING=\"0\" CELLPADDING=\"0\" STYLE=\"width: 100%;\"
       <TR><TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_tablespaces_storage.cgi?order_field=TABLESPACE_NAME&srcptr=$pv{srcptr}&tbs_filter=$pv{tbs_filter}&status_filter=$pv{status_filter}&type_filter=$pv{type_filter}\" TARGET=\"cont\">Имя ТС</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_tablespaces_storage.cgi?order_field=EXTENT_MANAGEMENT&srcptr=$pv{srcptr}&tbs_filter=$pv{tbs_filter}&status_filter=$pv{status_filter}&type_filter=$pv{type_filter}\" TARGET=\"cont\">Управление экстентами</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_tablespaces_storage.cgi?order_field=ALLOCATION_TYPE&srcptr=$pv{srcptr}&tbs_filter=$pv{tbs_filter}&status_filter=$pv{status_filter}&type_filter=$pv{type_filter}\" TARGET=\"cont\">ALLOCATION TYPE</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_tablespaces_storage.cgi?order_field=SEGMENT_SPACE_MANAGEMENT&srcptr=$pv{srcptr}&tbs_filter=$pv{tbs_filter}&status_filter=$pv{status_filter}&type_filter=$pv{type_filter}\" TARGET=\"cont\">Управление сегментами</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_tablespaces_storage.cgi?order_field=INITIAL_EXTENT&srcptr=$pv{srcptr}&tbs_filter=$pv{tbs_filter}&status_filter=$pv{status_filter}&type_filter=$pv{type_filter}\" TARGET=\"cont\">Начальный экстент</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_tablespaces_storage.cgi?order_field=NEXT_EXTENT&srcptr=$pv{srcptr}&tbs_filter=$pv{tbs_filter}&status_filter=$pv{status_filter}&type_filter=$pv{type_filter}\" TARGET=\"cont\">Следующий экстент</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_tablespaces_storage.cgi?order_field=MIN_EXTENTS&srcptr=$pv{srcptr}&tbs_filter=$pv{tbs_filter}&status_filter=$pv{status_filter}&type_filter=$pv{type_filter}\" TARGET=\"cont\">Экстентов минимум</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_tablespaces_storage.cgi?order_field=MAX_EXTENTS&srcptr=$pv{srcptr}&tbs_filter=$pv{tbs_filter}&status_filter=$pv{status_filter}&type_filter=$pv{type_filter}\" TARGET=\"cont\">Экстентов максимум</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_tablespaces_storage.cgi?order_field=PCT_INCREASE&srcptr=$pv{srcptr}&tbs_filter=$pv{tbs_filter}&status_filter=$pv{status_filter}&type_filter=$pv{type_filter}\" TARGET=\"cont\">PCT INCREASE</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_tablespaces_storage.cgi?order_field=MIN_EXTLEN&srcptr=$pv{srcptr}&tbs_filter=$pv{tbs_filter}&status_filter=$pv{status_filter}&type_filter=$pv{type_filter}\" TARGET=\"cont\">Мин. длина экстента</A></TD>
       </TR>" ;
$count_rows = 0 ;
while (my ($ts_name,$extent_management,$allocation_type,$segment_space_management,$ininial_extent,$next_extent,$min_extents,$max_extents,$pct_increase,$min_extlen) = $sth->fetchrow_array() ) {
      $count_rows += 1 ;
      if ( $ts_name eq "" ) { $ts_name = "&nbsp;" ; } if ( $extent_management eq "" ) { $extent_management = "&nbsp;" ; }
      if ( $allocation_type eq "" ) { $allocation_type = "&nbsp;" ; } if ( $segment_space_management eq "" ) { $segment_space_management = "&nbsp;" ; }
      if ( $ininial_extent eq "" ) { $ininial_extent = "&nbsp;" ; } else { $ininial_extent = &show_razryads($ininial_extent) ; }
      if ( $next_extent eq "" ) { $next_extent = "&nbsp;" ; } else { $next_extent =  &show_razryads($next_extent) ; } 
      if ( $min_extents eq "" ) { $min_extents = "&nbsp;" ; } else { $min_extents =  &show_razryads($min_extents) ; }
      if ( $max_extents eq "" ) { $max_extents = "&nbsp;" ; } else { $max_extents =  &show_razryads($max_extents) ; }
      if ( $pct_increase eq "" ) { $pct_increase = "&nbsp;" ; } else { $pct_increase = &show_razryads($pct_increase) ; }
      if ( $min_extlen eq "" ) { $min_extlen = "&nbsp;" ; } else { $min_extlen = &show_razryads($min_extlen) ; }
      printf("<TR><TD CLASS=\"SZDATA\"><A HREF=\"$base_url/cgi/get_files_definition.cgi?order_field=FILE_NAME&tbs_filter=%s&srcptr=$pv{srcptr}\" TARGET=\"cont\">%s</A></TD>
                  <TD CLASS=\"SZDATA\">%0s</TD>
                  <TD CLASS=\"SZDATA\">%0s</TD>
                  <TD CLASS=\"SZDATA\">%0s</TD>
                  <TD CLASS=\"NUMDATA\">%0s</TD>
                  <TD CLASS=\"NUMDATA\">%0s</TD>
                  <TD CLASS=\"NUMDATA\">%0s</TD>
                  <TD CLASS=\"NUMDATA\">%0s</TD>
                  <TD CLASS=\"NUMDATA\">%0s</TD>
                  <TD CLASS=\"NUMDATA\">%0s</TD>
              </TR>\n",$ts_name,$ts_name,$extent_management,$allocation_type,$segment_space_management,$ininial_extent,$next_extent,$min_extents,
                       $max_extents,$pct_increase,$min_extlen) ;
      }
print "<TR><TD CLASS=\"SZDATA\">Итого</TD><TD CLASS=\"NUMDATA\" COLSPAN=\"9\">$count_rows</TD></TR>\n" ;
print "</TABLE>" ;
