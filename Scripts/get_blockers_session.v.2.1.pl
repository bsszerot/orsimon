...
$request = 'select b.HOLDING_SESSION SID, s.SERIAL# SERIAL, s.USERNAME, s.OSUSER, s.PROCESS, s.TERMINAL 
            from dba_blockers b, v$session s WHERE b.HOLDING_SESSION = s.SID ORDER BY ' . $order_field ;
...
print "<TR><TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_dml_tx_locks.cgi?order_field=HOLDING_SESSION\" TARGET=\"cont\">Блокирующая сессия</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_dml_tx_locks.cgi?order_field=o.OBJECT_NAME\" TARGET=\"cont\">Oracle user</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_dml_tx_locks.cgi?order_field=o.OBJECT_NAME\" TARGET=\"cont\">ОС user</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_dml_tx_locks.cgi?order_field=o.OBJECT_NAME\" TARGET=\"cont\">OS PID client process</A></TD>
           <TD CLASS=\"HEAD\"><A HREF=\"$base_url/cgi/get_dml_tx_locks.cgi?order_field=l.TYPE\" TARGET=\"cont\">Терминал</A></TD>
           </TR>" ;
$is_not_null_answer = "no" ; $sum_records = 0 ;
while (my ($sid,$serial,$username,$osuser,$process,$terminal) = $sth->fetchrow_array() ) { $sum_records += 1 ;
      if ( $sid eq "" ) { $sid = "&nbsp;" ; } if ( $serial eq "" ) { $serial = "&nbsp;" ; }
      if ( $username eq "" ) { $username = "&nbsp;" ; } if ( $osuser eq "" ) { $osuser = "&nbsp;" ; } 
      if ( $process eq "" ) { $process = "&nbsp;" ; } if ( $terminal eq "" ) { $terminal = "&nbsp;" ; }
      printf("<TR><TD CLASS=\"SZDATA\"><A HREF=\"$base_url/cgi/get_session_info.cgi?sid=%s&serial=%s\" TARGET=\"cont\">%s,%s</A></TD>
                  ...
                  <TD CLASS=\"NUMDATA\">%s</TD>
                  <TD CLASS=\"NUMDATA\">%s</TD>
              </TR>\n",$sid,$serial,$sid,$serial,$username,$osuser,$process,$terminal) ;
      $is_not_null_answer = "yes" ;
      }
if ( $is_not_null_answer eq "no" ) { print "<TR><TD CLASS=\"SZDATA\" COLSPAN=\"9\">Блокирующие сессии отсутствуют в DBA_BLOCKERS</TD></TR>" ;}
else { print "<TR><TD CLASS=\"SZDATA\">Итого</TD><TD COLSPAN=\"4\" CLASS=\"NUMDATA\">$sum_records</TD></TR>" ; }
print "</TABLE>" ;
print "</TD></TR></TABLE>" ;
$sth->finish();
$dbh->disconnect();                                        
