...
# отразить общую статистику по SQL_AREA
# отразить текст SQL запроса
# отразить статистику каждого курсора и соответствующий ему план исполнения

@column_aggregation = ( "ADDRESS","HASH_VALUE","VERSION_COUNT","LOADED_VERSIONS","OPEN_VERSIONS","CPU_TIME","ELAPSED_TIME","SHARABLE_MEM","PERSISTENT_MEM",
                        "RUNTIME_MEM","SORTS","DISK_READS","BUFFER_GETS","ROWS_PROCESSED","PARSE_CALLS","EXECUTIONS","FETCHES","PARSING_USER",
                        "PARSING_SCHEMA","KEPT_VERSIONS","USERS_OPENING","USERS_EXECUTING","FIRST_LOAD_TIME","LOADS","INVALIDATIONS","COMMAND_TYPE",
                        "OPTIMIZER_MODE","MODULE","ACTION","SERIALIZABLE_ABORTS","IS_OBSOLETE","CHILD_LATCH" ) ;

$request_aggregation = 'select s.ADDRESS,s.HASH_VALUE,s.VERSION_COUNT,s.LOADED_VERSIONS,s.OPEN_VERSIONS,s.CPU_TIME,s.ELAPSED_TIME,s.SHARABLE_MEM,s.PERSISTENT_MEM,
                               s.RUNTIME_MEM,s.SORTS, s.DISK_READS,s.BUFFER_GETS,s.ROWS_PROCESSED,s.PARSE_CALLS,s.EXECUTIONS,s.FETCHES,lu.USERNAME PARSING_USER,
                               lc.USERNAME PARSING_SCHEMA,s.KEPT_VERSIONS,s.USERS_OPENING,s.USERS_EXECUTING, s.FIRST_LOAD_TIME,s.LOADS,s.INVALIDATIONS,
                               s.COMMAND_TYPE,s.OPTIMIZER_MODE,s.MODULE,s.ACTION,s.SERIALIZABLE_ABORTS,s.IS_OBSOLETE,s.CHILD_LATCH 
                               from v$sqlarea s, dba_users lu, dba_users lc 
                               where s.PARSING_USER_ID = lu.USER_ID AND s.PARSING_SCHEMA_ID = lc.USER_ID' . " and s.ADDRESS = '$pv{address}' and s.hash_value = '$pv{hash_value}'" ;

$request_sql_text = "select SQL_TEXT from v\$sqltext_with_newlines where ADDRESS = '$pv{address}' and hash_value = '$pv{hash_value}' ORDER BY PIECE" ;

@column_name_cursors = ( "ADDRESS","HASH_VALUE","OBJECT_STATUS","REMOTE","PLAN_HASH_VALUE","CHILD_NUMBER","OUTLINE_CATEGORY","OUTLINE_SID",
                           "CHILD_ADDRESS","SQLTYPE","LITERAL_HASH_VALUE","LOADED_VERSIONS","OPEN_VERSIONS","CPU_TIME","ELAPSED_TIME","SHARABLE_MEM",
                           "PERSISTENT_MEM","RUNTIME_MEM","SORTS","DISK_READS","BUFFER_GETS","ROWS_PROCESSED","PARSE_CALLS","EXECUTIONS","FETCHES",
                           "PARSING_USER","PARSING_SCHEMA","KEPT_VERSIONS","USERS_OPENING","USERS_EXECUTING","FIRST_LOAD_TIME",
                           "LOADS","INVALIDATIONS","LAST_LOAD_TIME","COMMAND_TYPE","OPTIMIZER_MODE","OPTIMIZER_COST","MODULE","ACTION",
                           "SERIALIZABLE_ABORTS","IS_OBSOLETE","CHILD_LATCH" ) ;

$request_cursors = 'select s.ADDRESS,s.HASH_VALUE,s.OBJECT_STATUS,s.REMOTE,s.PLAN_HASH_VALUE,s.CHILD_NUMBER,s.OUTLINE_CATEGORY,s.OUTLINE_SID,
                           s.CHILD_ADDRESS,s.SQLTYPE,s.LITERAL_HASH_VALUE, s.LOADED_VERSIONS,s.OPEN_VERSIONS,s.CPU_TIME,s.ELAPSED_TIME,s.SHARABLE_MEM,
                           s.PERSISTENT_MEM,s.RUNTIME_MEM,s.SORTS,s.DISK_READS,s.BUFFER_GETS,s.ROWS_PROCESSED, s.PARSE_CALLS,s.EXECUTIONS,s.FETCHES,
                           lu.USERNAME PARSING_USER,lc.USERNAME PARSING_SCHEMA,s.KEPT_VERSIONS,s.USERS_OPENING,s.USERS_EXECUTING, s.FIRST_LOAD_TIME,
                           s.LOADS,s.INVALIDATIONS,s.LAST_LOAD_TIME,s.COMMAND_TYPE,s.OPTIMIZER_MODE,s.OPTIMIZER_COST,s.MODULE,s.ACTION,
                           s.SERIALIZABLE_ABORTS,s.IS_OBSOLETE,s.CHILD_LATCH 
                           from v$sql s, dba_users lu, dba_users lc 
                           where s.PARSING_USER_ID = lu.USER_ID AND s.PARSING_SCHEMA_ID = lc.USER_ID ' . " and s.ADDRESS = '$pv{address}' and s.hash_value = '$pv{hash_value}'" . ' ORDER BY s.CHILD_NUMBER' ;
 
@column_name_plans = ( "CARDINALITY", "OPERATION", "OPTIONS", "OBJECT NAME", "SEARCH COLUMN", "COST", "CPU_COST", "IO_COST", "TEMP_SPACE" ) ;

#,SUBSTR(ACCESS_PREDICATES,0,50),SUBSTR(FILTER_PREDICATES,0,50)
...
&print_html_head("Информация о SQL запросе ADDRESS = $pv{address}, HASH_VALUE = $pv{hash_value}") ;
...
# вывести содержимое
my $cursor_handler_main = $dbconn_main->prepare($request_aggregation) ; $cursor_handler_main->execute();
print "<P><FONT COLOR=\"navy\"><B>Агрегированная информация SQL запроса:</B></FONT></P>" ;
print "<TABLE BORDER=\"1\">" ; 
print "<TR>" ; for ($i=0;$i<=$#column_aggregation;$i++) { print "<TD STYLE=\"text-align: center; color: navy;\">$column_aggregation[$i]</TD>" ; } print "</TR>" ;
# получить очередную строку ответа
@request_result = $cursor_handler_main->fetchrow_array() ;
print "<TR>" ; 
for ($i=0;$i<=$#request_result;$i++) { my $column_value = $request_result[$i]; 
#    $column_value = s/[^\w\d\s<>:()\+=\/\\\-_\.,\'\"#\*%!]+//g ;
$column_value =~ s/\s/&nbsp;/g ; if ( $column_value eq "" ) { $column_value = "&nbsp;" ; }
    print "<TD>$column_value</TD>" ; }
print "</TR>" ;
print "</TABLE>" ;

my $cursor_handler_main = $dbconn_main->prepare($request_cursors) ; $cursor_handler_main->execute();
@filter_for_plan = ("") ; $#filter_for_plan = -1 ;
# вывести содержимое
print "<P><FONT COLOR=\"navy\"><B>Информация отдельных курсоров:</B></FONT></P>" ;
print "<TABLE BORDER=\"1\">" ; 
print "<TR>" ; for ($i=0;$i<=$#column_name_cursors;$i++) { print "<TD STYLE=\"text-align: center; color: navy;\">$column_name_cursors[$i]</TD>" ; } print "</TR>" ;
# получить очередную строку ответа
while (@request_result = $cursor_handler_main->fetchrow_array()) {
...
if ( $is_view_request eq "yes" ) { 
   print "<BR><HR COLOR=\"navy\">
              <P><FONT COLOR=\"navy\"><B>Справочно: запрос о статистике текущего SQL</B></FONT> <BR><BR>$request_aggregation</P>\n
              <P><FONT COLOR=\"navy\"><B>Справочно: запрос о тексте SQL</B></FONT> <BR><BR>$request_sql_text</P>\n
              <P><FONT COLOR=\"navy\"><B>Справочно: запрос о статистиках курсоров текущего SQL</B></FONT> <BR><BR>$request_cursors</P>\n" ; }
